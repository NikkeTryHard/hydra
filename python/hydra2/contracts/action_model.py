...
class CanonicalAction:
    ...
    def __post_init__(self) -> None:
        ...
        actor: Seat = _bridge_contracts.make_seat(self.actor)
        object.__setattr__(self, "actor", actor)
        tile: TileId | None = (
            None if self.tile is None else _bridge_contracts.make_tile_id(self.tile)
        )
        called: TileId | None = (
            None if self.called_tile is None else _bridge_contracts.make_tile_id(self.called_tile)
        )
        source: Seat | None = (
            None if self.source_seat is None else _bridge_contracts.make_seat(self.source_seat)
        )
        consumed: tuple[TileId, ...] = tuple(
            _bridge_contracts.make_tile_id(t) for t in self.consumed_tiles
        )
...
def template_sort_key(template: CanonicalActionTemplate) -> tuple:
    ...
    Thin hard-Rust delegate: the key assembly lives in the
    ``hydra2._native.contracts`` ``template_sort_key_for`` leaf
    (``crates/bridge/src/action_model.rs``); this translator passes the
    seven record parts positionally. ``ImportError`` on a stale ``.so``
    (no oracle fallback); bridge ``ValueError``/``TypeError`` propagates
    fail-closed.
    """
    try:
        leaf: Callable[..., tuple] = _bridge_contracts.template_sort_key_for
    except AttributeError as exc:
        raise ImportError(
            "hydra2._native.contracts.template_sort_key_for missing (stale .so); "
