...
def _require_actor_bridge() -> Any:
    ...
    if _bridge_contracts is None:
        raise ImportError(
            "hydra2 observation authority requires the hydra2._native bridge; "
            "run `pixi run build-ext` to build the extension before use"
        )
    return _bridge_contracts


def _check_dora_indicators(indicators: Sequence[Any]) -> tuple[Any, ...]:
    """Validate dora indicators through the bridge; return the stored tuple.

    TOTAL mirror of the G3 gates in :meth:`ActorObservation.__post_init__`:
    shape, per-slot int/tile checks, and revealed-prefix contiguity. The
    bridge raises ``ValueError`` with byte-identical oracle text, mapped here
    to :class:`ContractError`; storage stays the validated original tuple.
    """
    try:
        _require_actor_bridge().check_dora_indicators(indicators)
...
class ActorObservation:
    ...
    def __post_init__(self) -> None:
        ...
        actor: Seat = _require_actor_bridge().make_seat(self.actor)
        ...
        for name in (
            ...
            digest: DigestText = _require_actor_bridge().make_digest_text(getattr(self, name))
        ...
        dealer: Seat = _require_actor_bridge().make_seat(self.dealer)
        object.__setattr__(self, "dealer", dealer)
        object.__setattr__(
            self,
            "scores",
            _quad(
        ...
        turn_actor: Seat = _require_actor_bridge().make_seat(self.turn_actor)
        object.__setattr__(self, "turn_actor", turn_actor)
        ...
        if self.pending_declaration_discard is not None:
            pending: TileId = _require_actor_bridge().make_tile_id(self.pending_declaration_discard)
            object.__setattr__(self, "pending_declaration_discard", pending)
        ...
        if self.own_drawn_tile is not None:
            drawn: TileId = _require_actor_bridge().make_tile_id(self.own_drawn_tile)
            object.__setattr__(self, "own_drawn_tile", drawn)
        if (
        ...
        object.__setattr__(self, "dora_indicators", _check_dora_indicators(self.dora_indicators))
        ...
        if self.observation_hash is not None:
            recorded: DigestText = _require_actor_bridge().make_digest_text(self.observation_hash)
            object.__setattr__(self, "observation_hash", recorded)
...
def observation_identity_document(observation: ActorObservation) -> dict[str, object]:
    ...
    return _require_actor_bridge().observation_identity_projection(observation.to_json())
