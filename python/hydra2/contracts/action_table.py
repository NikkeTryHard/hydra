...
try:
    from hydra2._native import contracts as bridge  # pyrefly: ignore[missing-import]
...
def _require_bridge() -> Any:
    ...
    if bridge is None:
        raise ImportError(
            "hydra2 codec authority requires the hydra2._native bridge; "
...
class ActionTable:
    ...
    def __post_init__(self) -> None:
        ...
        digest: DigestText = _require_bridge().make_digest_text(self.digest)
    ...
    def index_of(self, template: CanonicalActionTemplate) -> int | None:
        ...
        if bridge is None:
            raise ImportError(
                "hydra2 census authority requires the hydra2._native bridge; "
                "run `pixi run build-ext` to build the extension before use"
            )
        try:
            result: int | None = bridge.census_index_of(  # type: ignore[attr-defined]
                template.kind,
    ...
    def __post_init__(self) -> None:
        try:
            ...
            offered_tile: TileId | None = (
                ...
                else _require_bridge().make_tile_id(self.offered_tile)
            )
            offered_by: Seat | None = (
                None if self.offered_by is None else _require_bridge().make_seat(self.offered_by)
            )
            concealed: tuple[TileId, ...] = tuple(
                _require_bridge().make_tile_id(t) for t in self.own_concealed_tiles
            )
...
def _offset_from_source(source: Seat | None, actor: Seat) -> SourceOffset | None:
    ...
    The gate lives in ``hydra2._native.contracts.offset_from_source``;
    ...
    try:
        result: int | None = bridge.offset_from_source(source, actor)  # type: ignore[attr-defined]
    except (ValueError, TypeError) as exc:
        raise InvalidActionError(f"source offset rejected: {exc}") from exc
    if result is None:
        return None
    return cast("SourceOffset", result)
...
def _resolve_source(offset: SourceOffset | None, actor: Seat) -> Seat | None:
    ...
    The gate lives in ``hydra2._native.contracts.resolve_source``;
    ...
    try:
        result: int | None = bridge.resolve_source(offset, actor)  # type: ignore[attr-defined]
    except (ValueError, TypeError) as exc:
        raise InvalidActionError(f"source resolve rejected: {exc}") from exc
    if result is None:
        return None
    resolved: Seat = _require_bridge().make_seat(result)
    return resolved
...
class CanonicalActionCodec(ActionCodec):
    ...
    def encode(
        ...
        *,
        ...
        action_id: ActionId = _require_bridge().make_action_id(index)
        return action_id
