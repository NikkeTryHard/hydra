...
:mod:`hydra2.artifacts.canonical` so identical keys hash identically on every
...
RANDOM_PURPOSES: tuple[str, ...] = (
    tuple(_bridge_contracts.RANDOM_PURPOSES)  # pyrefly: ignore[unknown-argument-type] # bridge vocab with stale-.so literal fallback; tuple() copies
    if hasattr(_bridge_contracts, "RANDOM_PURPOSES")
    else (
        "wall",
        "belief_natural_sample",
        "belief_proposal_sample",
...
FINAL_EVALUATION_PURPOSES: frozenset[str] = (
    frozenset(_bridge_contracts.FINAL_EVALUATION_PURPOSES)  # pyrefly: ignore[unknown-argument-type] # bridge vocab with stale-.so literal fallback; frozenset() copies
    if hasattr(_bridge_contracts, "FINAL_EVALUATION_PURPOSES")
    else frozenset({"confirmation", "evaluation_schedule"})
)
...
def semantic_seed(master_seed: bytes, *, key: RandomStreamKey) -> bytes:
    ...
    Rust-first via the ``hydra2._native.contracts`` bridge
    (``semantic_seed_from_key_doc``): the payload ``{"protocol":
    "hydra2_rng_v1", "master_seed": <hex>, "key": <key json>}`` is hashed
    with SHA-256 over RFC 8785 canonical bytes. Evidence: bridge canon
    arrays 2.8-3.8x + flats 1.4x Rust-faster linear with digest parity on
    every doc; seed derivation byte-identical across the 3 master-seed
    fixtures (``seed_map_fixture``). The Python oracle
    (``canonical_json_bytes`` + ``hashlib.sha256``) is recomputed in Rust
    via the ``hydra-search`` formula owner and byte-compared; mismatch
    raises ``DigestMismatchError`` (fail-closed, never silent).
    Stale-``.so`` oracle fallback: without the randomness leaf the oracle
    decides alone.
    ...
    try:
        key_doc = canonical_json_bytes(key_to_json(key))
        rust_seed = bytes(
            _bridge_contracts.semantic_seed_from_key_doc(bytes(master_seed), key_doc)  # type: ignore[attr-defined]  # reason: randomness leaf lands with MAIN wiring; hasattr fallback below covers stale .so
        )
    except AttributeError:
        return oracle
    except (ValueError, TypeError, OverflowError) as exc:
        raise ContractError(f"semantic_seed bridge rejected input: {exc}") from exc
    if rust_seed != oracle:
        raise DigestMismatchError(
            "semantic_seed: bridge canon+digest != python oracle "
            f"(sha256:{rust_seed.hex()} != sha256:{oracle.hex()})"
        )
    return rust_seed
...
class RandomStream:
    ...
    def get_bytes(self, count: int) -> bytes:
        ...
        Byte assembly rides the ``hydra2._native.contracts.ctr_stream_bytes``
...
def derive_scope_material(root_material: bytes, scope: str) -> MasterSeedMaterial:
    ...
    Rust-first via the ``hydra2._native.contracts`` bridge
    (``derive_scope_material``): raw SHA-256 over the domain-separated
    preimage, byte-compared to the Python oracle; mismatch raises
    ``DigestMismatchError``. Stale-``.so`` fallback: without the randomness
    leaf the oracle decides alone.
    """
    ...
    try:
        bridged = bytes(
            _bridge_contracts.derive_scope_material(bytes(root_material), scope)  # type: ignore[attr-defined]  # reason: randomness leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        )
    except AttributeError:
        return MasterSeedMaterial(material=oracle, scope=scope)  # type: ignore[arg-type]  # reason: scope narrowed by the membership check above; checker keeps the str width
    except (ValueError, TypeError, OverflowError) as exc:
        raise ContractError(f"scope derivation bridge rejected input: {exc}") from exc
    if bridged != oracle:
        raise DigestMismatchError(
            "derive_scope_material: bridge != python oracle "
            f"(sha256:{bridged.hex()} != sha256:{oracle.hex()})"
        )
    return MasterSeedMaterial(material=bridged, scope=scope)  # type: ignore[arg-type]  # reason: scope narrowed by the membership check above; checker keeps the str width
...
def authority_stream(domain: MasterSeedMaterial, key: RandomStreamKey) -> RandomStream:
    ...
    The scope gate rides the ``hydra2._native.contracts`` bridge
    (``check_authority_scope``); bridge rejections surface as ContractError
    with byte-identical oracle text. Stale-``.so`` fallback: the inline
    oracle checks decide alone.
    """
    _ = RandomStreamSchema.validate_key(key)
    key_is_final = key.purpose in FINAL_EVALUATION_PURPOSES
    try:
        _bridge_contracts.check_authority_scope(domain.scope, key.purpose)  # type: ignore[attr-defined]  # reason: randomness leaf lands with MAIN wiring; AttributeError fallback covers stale .so
    except AttributeError:
