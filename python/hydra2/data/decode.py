...
def decode_json_line(line: str | bytes) -> object:
    """Parse one JSON line (bridge-first transport parse, fallback identical)."""
    try:
        from hydra2 import _native as _ext  # pyrefly: ignore[missing-import]
    except ImportError:
        return json.loads(line)
    bridge = getattr(getattr(_ext, "contracts", None), "record_decode_json_line", None)
    if bridge is None:
        return json.loads(line)
    try:
        return bridge(line)
    except ValueError:
        # serde/CPython error texts diverge; re-parse for the oracle text.
        # Failure path only — success rides the single bridge call.
        return json.loads(line)
...
def decode_game_object(
    *, object_id: str, packaged_object_id: str, decoded_bytes: bytes
    ...
    wall_raw: list[int] | tuple[int, ...] | None = slot.get("wall_tiles")
    wall: tuple[int, ...] | None = tuple(wall_raw) if wall_raw is not None else None
    return GameRecord(
