...
The Rust walk (``hydra2._native.expand_games`` /
...
class ReplayExpander:
    ...
    def _window_claim(
        ...
        if kind in _CLAIM_TYPES:
            ...
            claim_seat: Seat = _bridge_contracts.make_seat(actor)
            legals = sim.legal_actions(claim_seat)
        ...
        if kind == "hora" and not bool(ev.get("tsumo", False)):
            ...
            if actor not in pending:
                return None
            ron_seat: Seat = _bridge_contracts.make_seat(actor)
            legals = sim.legal_actions(ron_seat)
    ...
    def _step_draw(
        ...
        *,
        ...
        if kind == "reach":
            ...
            riichi_seat: Seat = _bridge_contracts.make_seat(expected)
            legals = sim.legal_actions(riichi_seat)
        ...
        if kind == "dahai":
            ...
            dahai_seat: Seat = _bridge_contracts.make_seat(expected)
            legals = sim.legal_actions(dahai_seat)
        ...
        if kind in ("ankan", "kakan"):
            ...
            kan_seat: Seat = _bridge_contracts.make_seat(expected)
            legals = sim.legal_actions(kan_seat)
        ...
        if kind == "hora":
            ...
            tsumo_seat: Seat = _bridge_contracts.make_seat(expected)
            legals = sim.legal_actions(tsumo_seat)
    ...
    def _apply_pass(self, seat: int) -> None:
        sim = self._sim
        pass_seat: Seat = _bridge_contracts.make_seat(seat)
        legals = sim.legal_actions(pass_seat)
    ...
    def _select_for_apply(self, seat: int, action: CanonicalAction) -> CanonicalAction:
        ...
        sim = self._sim
        apply_seat: Seat = _bridge_contracts.make_seat(seat)
        legals = sim.legal_actions(apply_seat)
        for legal in legals:
            if legal == action:
                return legal
        return action
    ...
    def _emit_row(
        ...
        *,
        ...
        sim = self._sim
        emit_seat: Seat = _bridge_contracts.make_seat(seat)
        observation = sim.actor_observation(emit_seat)
