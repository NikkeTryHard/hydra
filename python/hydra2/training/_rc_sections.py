...
_RC_CONFIG_SECTIONS: list[str] = _bridge_contracts.RC_CONFIG_SECTIONS
CONFIG_SECTIONS: tuple[str, ...] = tuple(_RC_CONFIG_SECTIONS)
...
_RC_RUN_KINDS: list[str] = _bridge_contracts.RC_RUN_KINDS
RUN_KINDS: tuple[str, ...] = tuple(_RC_RUN_KINDS)
...
_RC_INTERPOLATION_ALLOWLIST: list[str] = _bridge_contracts.RC_INTERPOLATION_ALLOWLIST
INTERPOLATION_ALLOWLIST: frozenset[str] = frozenset(_RC_INTERPOLATION_ALLOWLIST)
_INTERPOLATION_PATTERN: str = _bridge_contracts.RC_INTERPOLATION_PATTERN
_INTERPOLATION_RE = re.compile(_INTERPOLATION_PATTERN)
_RUN_ID_PATTERN: str = _bridge_contracts.RC_RUN_ID_PATTERN
_RUN_ID_RE = re.compile(_RUN_ID_PATTERN)
_CUDA_DEVICE_PATTERN: str = _bridge_contracts.RC_CUDA_DEVICE_PATTERN
_CUDA_DEVICE_RE = re.compile(_CUDA_DEVICE_PATTERN)
_TENHOU_SCOPE: str = _bridge_contracts.RC_TENHOU_SCOPE

_RC_OPTIMIZER_IDS: list[str] = _bridge_contracts.RC_OPTIMIZER_IDS
_OPTIMIZER_IDS: tuple[str, ...] = tuple(_RC_OPTIMIZER_IDS)
_RC_SCHEDULER_IDS: list[str] = _bridge_contracts.RC_SCHEDULER_IDS
_SCHEDULER_IDS: tuple[str, ...] = tuple(_RC_SCHEDULER_IDS)
_RC_ADAPTER_IDS: list[str] = _bridge_contracts.RC_ADAPTER_IDS
_ADAPTER_IDS: tuple[str, ...] = tuple(_RC_ADAPTER_IDS)
_RC_RUNTIME_PRECISIONS: list[str] = _bridge_contracts.RC_RUNTIME_PRECISIONS
_RUNTIME_PRECISIONS: tuple[str, ...] = tuple(_RC_RUNTIME_PRECISIONS)
_RC_COMPILE_MODES: list[str] = _bridge_contracts.RC_COMPILE_MODES
_COMPILE_MODES: tuple[str, ...] = tuple(_RC_COMPILE_MODES)
_RC_SELECTION_DESIGNS: list[str] = _bridge_contracts.RC_SELECTION_DESIGNS
_SELECTION_DESIGNS: tuple[str, ...] = tuple(_RC_SELECTION_DESIGNS)
...
class DataConfig:
    ...
    num_workers: int = 0
    world_size: int = 1
    #: Replay backend: ``"rust"`` (Rust per-game walk + tensor-native batch
    #: assembly over the identical stream: same order/shuffle/resume/sidecar,
    #: same rows, no per-row Python). Any other id fails closed. The value is
    #: part of the run digest, so a changed backend refuses resume fail-closed
...
class StreamCursor:
    ...
    def from_dict(cls, raw: Any) -> StreamCursor:
        ...
        try:
            unpacked: tuple[int, int, int, int, int] = _bridge_contracts.rc_unpack_cursor(raw)
            file_index, byte_offset, games_seen, seed, epoch = unpacked
