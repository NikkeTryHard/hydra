...
import yaml  # pyrefly: ignore[untyped-import] # pyyaml ships no stubs here; safe_dump bound to str at use
...
def run_config_digest(config: RunConfig) -> str:
    ...
    Thin hard-Rust delegate: :func:`run_config_to_dict` frames the resolved
    config as a plain mapping, then the bridge ``contracts`` pyfn
    (``run_config_digest_from_dict``: stage + JCS + hash, one detach)
    computes the digest returned directly. ``ImportError`` (extension not
    built) raises with a ``build-ext`` hint — NO oracle fallback, never
    silent. Layout/IO/effective-id logic stays Python (OS authority).
    """
    digest: str = _bridge_contracts.run_config_digest_from_dict(run_config_to_dict(config))
    return digest
