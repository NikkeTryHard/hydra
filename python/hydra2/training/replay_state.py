...
try:
    from hydra2._native import contracts as _replay_bridge  # pyrefly: ignore[missing-import]
except ImportError:  # pragma: no cover - import-time signal, same text as call-site
    _replay_bridge = None  # type: ignore[assignment]

#: Privileged replay fields (single source: ``hydra2._native.contracts``
#: ``REPLAY_FORBIDDEN_BATCH_KEYS``; the stale-.so fallback is the byte-identical
#: oracle literal, per the ``randomness.py`` discipline).
_BRIDGE_FORBIDDEN_KEYS: frozenset[str] = (
    _replay_bridge.REPLAY_FORBIDDEN_BATCH_KEYS if _replay_bridge is not None else frozenset()
)
FORBIDDEN_REPLAY_KEYS = (
    frozenset(_BRIDGE_FORBIDDEN_KEYS)
    if _replay_bridge is not None and hasattr(_replay_bridge, "REPLAY_FORBIDDEN_BATCH_KEYS")
    else frozenset(
        {
            "hidden_tiles",
            "wall",
...
#: Bridge leaf for the replay sha gate (single source: ``hydra2._native.contracts``
#: ``replay_require_sha256``; the stale-.so fallback is the byte-identical oracle).
_replay_require_sha256 = getattr(_replay_bridge, "replay_require_sha256", None)

#: Bridge leaves for the replay config gates (single source:
#: ``hydra2._native.contracts`` ``training_*`` / ``replay_precision_gate``).
_replay_minibatch_size = getattr(_replay_bridge, "training_minibatch_size", None)
_replay_sizes_gate = getattr(_replay_bridge, "training_sizes_gate", None)
_replay_precision_gate = getattr(_replay_bridge, "replay_precision_gate", None)


def _require_sha256(name: str, value: str) -> str:
    if _replay_require_sha256 is not None:
        try:
            bound: str = _replay_require_sha256(name, value)
            return bound
...
class ReplayConfig:
    ...
    def optimizer_minibatch_size(self) -> int:
        if _replay_minibatch_size is not None:
            try:
                size: int = _replay_minibatch_size(self.microbatch_size, self.accumulation_steps)
                return size
            except (ValueError, TypeError) as exc:
                raise ContractError(str(exc)) from exc
    ...
    def validate(self) -> None:
        if _replay_sizes_gate is not None:
            try:
                _replay_sizes_gate(
        ...
        if _replay_precision_gate is not None:
            try:
                _replay_precision_gate(self.precision)
            except (ValueError, TypeError) as exc:
                raise ContractError(str(exc)) from exc
...
def _model_forward(model: nn.Module, batch: Any) -> dict[str, Any]:
    ...
    if hasattr(model, "evaluate") and callable(model.evaluate):
        evaluate: Any = model.evaluate
        out: Any = evaluate(batch)
    else:
        applied: Any = model(batch)
        out: Any = applied
