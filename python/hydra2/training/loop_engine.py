...
class SupervisedLoopEngineMixin(SupervisedLoopCheckpointMixin):
    ...
    def __init__(
        ...
        *,
        ...
        snapshot: dict[str, Any] = self._sampler_state_snapshot()
        ...
        if self.device.type == "cuda":
            ...
            if not _loss_compiling and not torch.are_deterministic_algorithms_enabled():
                ...
                with contextlib.suppress(Exception):
                    ...
                    if callable(_compiled_loss):  # pyrefly: ignore[unknown-argument-type] # torch.compile result dynamic; callable gate is the check
    ...
    def _acquire_cpu_batch_with_state(
        ...
        try:
            snap: dict[str, Any] = self._sampler_state_snapshot()
        except Exception:
            snap = {}
