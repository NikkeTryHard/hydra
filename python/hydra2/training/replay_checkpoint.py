...
class ActorLearnerReplayCheckpointMixin:
    ...
    def replay_batches(self, *, batch_size: int, num_batches: int) -> list[list[str]]:
        ...
        for _ in range(num_batches):
            batch: dict[str, Any] | None = self.dataset.next_batch(batch_size)
            if batch is None:
                break
            dids: list[Any] = list(batch.get("_decision_ids", []))
            out.append([str(x) for x in dids])
    ...
    def verify_authorized(self) -> None:
        ...
        for row_any in getattr(self.dataset, "_rows", []):
            ...
            for bad in FORBIDDEN_REPLAY_KEYS:
                if bad in row:
                    decision_id: Any = row.get("decision_id")
                    raise ContractError(
                        f"privileged field {bad!r} in authorized row {decision_id!r}"
                    )
