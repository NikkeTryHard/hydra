"""Stream row expansion: games to slim plane rows plus buffer-window helpers.

Owns the Rust plane walk (single-game and batched expansion over raw framed
bytes), quarantine classification, the bounded spawn-pool expansion worker,
and the game-keyed window hash plus history-bucket grouping keys the
dataset buffer builds on. Row identity (decision_id format) is frozen, so
privileged joins and sidecar hashes agree.
"""
...
try:
    from hydra2._native import contracts as _expand_bridge  # pyrefly: ignore[missing-import]
except ImportError:  # pragma: no cover - import-time signal, same text as call-site
    _expand_bridge = None  # type: ignore[assignment]
...
#: Replay backend. ``"rust"`` is the only backend (Rust per-game walk over raw
#: framed bytes + tensor-native batch assembly, no per-row Python). The
#: retired ``"python"`` oracle shim fails closed in ``_require_replay_backend``.
_REPLAY_BACKENDS: tuple[str, ...] = ("rust",)


#: Bridge leaf for the replay-backend gate (single source:
#: ``hydra2._native.contracts`` ``require_replay_backend``; the stale-.so
#: fallback is the tuple above).
_require_replay_backend_bridge = getattr(_expand_bridge, "require_replay_backend", None)


def _require_replay_backend(backend: str) -> str:
    """Validate a replay backend id (fail closed on unknown or retired)."""
    if _require_replay_backend_bridge is not None:
        try:
            bound: str = _require_replay_backend_bridge(backend)
            return bound
...
def _require_bridge() -> Any:
    try:
        ...
        raise ContractError(
            "hydra2._native extension not importable; "
    ...
    for surface in ("expand_games", "replay_game_planes", "replay_game_planes_wall"):
        if not hasattr(_ext, surface):
            raise ContractError(
                f"hydra2._native.{surface} missing (stale .so); "
...
def _expand_game_planes(
    ...
    wall-less games (same wall predicate the counters use). Row dicts carry
    ``decision_id``, ``chosen_action_id``, ``action_kind`` plus the shared
    game planes blob (``_planes`` name→bytes, ``_row`` offset, ``_t_len``)
    for tensor assembly in ``next_batch`` — no ``DecisionRow`` objects, no
    observations. ``decision_id`` format (``<game_id>:d<seq:04d>``) is frozen,
    so privileged joins and sidecar hashes agree. Quarantines raise
    :class:`ContractError` (feed reason string) for the caller to
    quarantine-and-count exactly.
    ...
    try:
        if wall is None:
            walked: tuple[dict[str, bytes], int, int, bool, str, int] = bridge.replay_game_planes(
                text, 0
            )
        else:
            walked: tuple[dict[str, bytes], int, int, bool, str, int] = (
                bridge.replay_game_planes_wall(text, 0, wall)
            )
    except Exception as exc:
        raise ContractError(f"rust game walk failed: {exc}") from exc

    return (_row_dicts_from_walk(game.game_id, walked), sim_path)
...
def _slim_row_dicts(
    ...
    """
    chosen_buf: torch.Tensor = torch.frombuffer(planes["chosen_action_id"], dtype=torch.int64)
    chosen_flat: list[int] = chosen_buf.tolist()
    chosen_ids: list[int] = list(chosen_flat)
    if len(chosen_ids) != rows:
        raise ContractError(
            f"rust walk row drift on game {game_id!r}: {len(chosen_ids)} chosen vs {rows} rows"
        )
    # Per-row history lengths for bucket grouping (vectorized mask popcount;
    # slim rows carry no observations, so without this grouping silently
    # collapses every row to the max bucket).
    hist_lens: list[int] = []
    if rows > 0:
        mask: torch.Tensor = torch.frombuffer(planes["history_mask"], dtype=torch.bool).reshape(
            rows, t_len
        )
        mask_sums: list[int] = mask.sum(dim=1).tolist()
        hist_lens = list(mask_sums)
    if len(hist_lens) != rows:
        raise ContractError(
            f"rust walk row drift on game {game_id!r}: {len(hist_lens)} hist lens vs {rows} rows"
        )
    out: list[dict[str, Any]] = []
    for seq, (chosen_id, hist_len) in enumerate(zip(chosen_ids, hist_lens, strict=True)):
        out.append(
            {
...
def _action_kind_for_id(chosen_id: int) -> str:
    ...
    if cached is None:
        try:
            ...
            raw: dict[str, Any] = _json.loads(_table_path.read_bytes().decode("utf-8"))
            payload: dict[str, Any] = raw.get("payload", {})
            actions: list[Any] = payload.get("actions", [])
            cached = [
                str(a.get("kind", "unknown")) if isinstance(a, dict) else "unknown"  # pyrefly: ignore[unknown-argument-type] # JSON action entries dynamic; str owns the kind
                for a in actions
            ]
            if len(cached) == 0:
                return "unknown"
...
def _row_to_dict(row: Any) -> dict[str, Any]:
    ...
    observation: dict[str, Any] = row.actor_observation
    obs_hash: str = row.observation_hash
    return {
        "decision_id": decision_id,
        "chosen_action_id": chosen,
        "actor_observation": live if live is not None else dict(observation),
...
def _expand_streamed_chunk(
    ...
    if backend == "rust":
        ...
        for pos, (game, _split, _b, _n, _raw) in enumerate(payloads):
            try:
                tiles: Any = game.wall_tiles
                walls.append(_wall_override(tiles))
        ...
        idxs = [
            ...
            if len(raw) > 0 and pos not in pre_err
        ]
        try:
            outs: list[tuple[dict[str, bytes], int, int, bool, str, int]] = (
                bridge.expand_games([(payloads[i][4], 0, walls[i]) for i in idxs])
                if len(idxs) > 0
                else []
            )
        except Exception as exc:
        ...
        for pos, (game, split, _b, need_priv, _raw) in enumerate(payloads):
            try:
                ...
                if pos in by_pos:
                    staged: tuple[dict[str, bytes], int, int, bool, str, int] = by_pos[pos]
                    game_id: str = game.game_id
                    row_dicts = _row_dicts_from_walk(game_id, staged)
                    sim_path: bool = game.wall_tiles is None
                else:
                ...
                priv_game: Any = game
                priv_rows = expand_privileged_rows(priv_game, split=split) if need_priv else []
        ...
        return out
    raise ContractError(f"replay_backend must be 'rust', got {backend!r} (python oracle removed)")
...
def _history_bucket_of(row: dict[str, Any]) -> int:
    ...
    Prefers the slim ``_hist_len`` the Rust pull stamps per row (exact mask
    popcount); otherwise reads ``len(row["actor_observation"][
    "visible_history"])`` (live :class:`ActorObservation` or its
    validated-dict form) and returns the smallest
    ``HISTORY_BUCKET_LENGTHS`` entry covering it. Missing, malformed, or
    over-cap lengths map to the max bucket: grouping must never mask
    errors — over-cap rows still fail closed at encode time with the
    proper error.
    """
    from hydra2.models.schema import HISTORY_BUCKET_LENGTHS

    buckets = HISTORY_BUCKET_LENGTHS
    staged = row.get("_hist_len")
    if isinstance(staged, bool):
        return buckets[-1]
