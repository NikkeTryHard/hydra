...
class _StreamDatasetCore:
    ...
    def _pull_game(self) -> bool:
        ...
        Every game expands through the Rust per-game walk
        (:func:`_expand_game_planes`). Privileged rows expand only when the
        dataset was built with ``need_privileged`` (positive auxiliary
        weights); BC-only runs skip them so label-less games still train.
        :class:`ContractError` quarantines-and-counts the game (fail closed,
        never a silent drop); only fully expanded games buffer rows.
    ...
    def _pull_game_batch(self, limit: int) -> bool:
        ...
        while True:
            ...
            for pos, streamed in enumerate(batch):
                tiles: Any = streamed.game.wall_tiles
                if tiles is None:
                    walls.append(None)
                else:
                    wall: list[int] = [int(t) for t in tiles]
                    if len(wall) != 136:
            ...
            for pos, streamed in enumerate(batch):
                try:
                    ...
                    if pos in by_pos:
                        ...
                        row_dicts: list[dict[str, Any]] = _slim_row_dicts(
                            game_id, planes, rows, t_len
                        )
                        sim_path: bool = streamed.game.wall_tiles is None
                    else:
                        # Hand-built games (tests) carry no framed bytes:
                        # ``_expand_game_planes`` rebuilt path, same rows as the serial pull.
                        hand_game: Any = streamed.game
                        hand_split: str = streamed.split
                        row_dicts, sim_path = _expand_game_planes(hand_game, hand_split, b"")
                    if self._need_privileged:
                        game: Any = streamed.game
                        game_split: str = streamed.split
                        priv_rows = expand_privileged_rows(game, split=game_split)
                    else:
                        priv_rows = []
                except ContractError as exc:
                    self._count_quarantine(exc)
        ...
        while self._live_count() < need:
            ...
            for chunk, future in zip(chunks, futures, strict=True):
                ...
                if timed:
                    ...
                    _ev = sum(len(s.game.events) for s in chunk)  # pyrefly: ignore[unknown-argument-type] # streamed chunk dynamic; len owns the count
                    if _dt_chunk >= 0.15:
                        print(
    ...
    def _track_buffered(self, streamed: Any, row_count: int) -> None:
        ...
        try:
            ...
            path: str = streamed.path.as_posix()
