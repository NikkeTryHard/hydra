...
class ConfirmationCase:
    ...
    def __post_init__(self) -> None:
        ...
        validated: DigestText = _bridge_contracts.make_digest_text(self.observation_hash)
        object.__setattr__(self, "observation_hash", validated)
    ...
    def confirm(
        ...
        *,
        ...
        for case in cases:
            ...
            # Value fold via the bridge (byte-identical to the inline mix above);
            # the single RNG draw stays caller-side (live cursor).
            mixed: float = _bridge_contracts.confirmation_value_mix(val_raw, rng.random_float())
            value: float = mixed
            out.append(
                ConfirmationResult(
