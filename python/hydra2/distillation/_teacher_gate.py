...
TEACHER_CANDIDATES: tuple[str, ...] = (
    _distill_bridge.TEACHER_CANDIDATES
    if hasattr(_distill_bridge, "TEACHER_CANDIDATES")
    else (
        "candidate0",
        "candidate1",
        "candidate2",
...
_GATE_KINDS: tuple[str, ...] = (
    ...
    else ("contract", "exact", "search", "match", "analysis")
)
...
_ANALYSIS_ID_FOR_TEACHER: dict[str, str] = (
    _distill_bridge.ANALYSIS_ID_FOR_TEACHER
    if hasattr(_distill_bridge, "ANALYSIS_ID_FOR_TEACHER")
    else {
...
_REAL_FEATURE_DIM = (
    _distill_bridge.DISTILL_REAL_FEATURE_DIM
    if hasattr(_distill_bridge, "DISTILL_REAL_FEATURE_DIM")
    else 48
)

_DEFAULT_NUM_ACTIONS = (
    _distill_bridge.DISTILL_DEFAULT_NUM_ACTIONS
    if hasattr(_distill_bridge, "DISTILL_DEFAULT_NUM_ACTIONS")
    else 32
)  # import-time fallback only; real paths resolve via _action_table()
...
#: ``distill_require_sha256``; the stale-.so fallback is the byte-identical oracle).
_distill_require_sha256 = getattr(_distill_bridge, "distill_require_sha256", None)


def _require_sha256(name: str, value: str) -> str:
    """Guard sha256:<64-hex> shape; fail closed on mismatch."""
    if _distill_require_sha256 is not None:
        try:
            checked: str = _distill_require_sha256(name, value)
            return checked
...
#: Bridge leaf for the gate seal (single source: ``hydra2._native.contracts``
#: ``distill_gate_hash_for_kind``; the stale-.so fallback is the byte-identical oracle).
_distill_gate_hash_for_kind = getattr(_distill_bridge, "distill_gate_hash_for_kind", None)
...
def _gate_hash_for_kind(kind: str, *, candidate_spec_hash: str, gate: dict[str, Any]) -> str:
    ...
    """
    if _distill_gate_hash_for_kind is not None:
        # Missing-key behavior matches the oracle: the ``analysis`` arm reads
        # ``gate["analysis_spec_hash"]`` directly (KeyError when absent); the
        # other arms never touch it (the bridge ignores the passthrough).
        if kind == "analysis":
            analysis_spec_hash = str(gate["analysis_spec_hash"])
        else:
            analysis_spec_hash = str(gate.get("analysis_spec_hash"))
        try:
            gate_hex: str = _distill_gate_hash_for_kind(
                kind,
                candidate_spec_hash,
                str(gate["digest"]),
                str(gate["report_hash"]),
                analysis_spec_hash,
            )
            return gate_hex
        except (ValueError, TypeError) as exc:
