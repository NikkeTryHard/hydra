...
def _require_census_bridge() -> object:
    ...
    if not hasattr(bridge, "action_census") or not hasattr(bridge, "action_census_count"):
        raise ImportError(
            "hydra2._native.contracts census surface missing (stale .so); "
...
def _hash_to_uniform(key: bytes, index: int) -> float:
    ...
    # 32-bit uniform
    return int.from_bytes(b[:4], "big") / 0xFFFFFFFF
...
def _case_observation(
    *, case_id: str, teacher_id: str, actor: int, spec: CandidateSpec, seed_material: bytes
    ...
    try:
        rules_hash: DigestText = _bridge_contracts.make_digest_text(spec.rules_hash)
        action_table_hash: DigestText = _bridge_contracts.make_digest_text(spec.action_table_hash)
        packet_boundary_hash: DigestText = _bridge_contracts.make_digest_text(
            spec.packet_boundary_hash
        )
