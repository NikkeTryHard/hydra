...
def _note_forced_draw(
    ...
    _emit(
        ...
        actor=actor,
        tile=int(tiles.physical_of(pai)),  # pyrefly: ignore[unknown-argument-type] # mjai pai string dynamic
    )
    ...
    if head is None:
        ...
        while nxt < total:
            ...
            if not isinstance(following, dict):
                break
            following_type: str = str(following.get("type", ""))  # pyrefly: ignore[unknown-argument-type] # log event value dynamic
            if following_type in _TRANSPARENT_KINDS:
                nxt += 1
                continue
            break
...
def _do_forced_dahai(
    ...
    drawn_bound: TileId = _bridge_contracts.make_tile_id(pos.drawn)
...
def _do_dahai(
    ...
    dahai_seat: Seat = _bridge_contracts.make_seat(actor)
    dahai_tile: TileId = _bridge_contracts.make_tile_id(step.tile)
    expected = CanonicalAction(
        kind=cast("Any", kind),
        actor=dahai_seat,
        tile=dahai_tile,
