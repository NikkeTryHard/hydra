...
def _do_hora(state: _GameState, walk: _KyokuWalk, kyoku: int, event: dict[str, object]) -> None:
    ...
    try:
        deltas = [int(d) for d in cast("Any", deltas_raw)]
    ...
    if self_draw:
        ...
        drawn_str: str = tiles.mjai_string_of(step.drawn)
        offer: Any = None
        ...
        if offer is None:
            raise state.fail(kyoku, "hora", "tsumo win without a drawn winning step")
        tile = step.drawn
        if offer.tile is None or tiles.mjai_string_of(
            int(cast("Any", offer.tile))
        ) != tiles.mjai_string_of(tile):
        ...
        expected = CanonicalAction(
            kind=cast("Any", "tsumo"),
            actor=cast("Seat", _bridge_contracts.make_seat(winner)),
        ...
        try:
            walk.tw.do_tsumo_win(winner, cast("str", tiles.mjai_string_of(tile)))
        ...
        tile_str: str = tiles.mjai_string_of(tile)
        if not any(
            _safe_mjai_type(raw) == "hora"
            and raw.tile is not None
            and tiles.mjai_string_of(int(cast("Any", raw.tile))) == tile_str
