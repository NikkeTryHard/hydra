...
def _resolve_dora(state: _GameState, walk: _KyokuWalk, kyoku: int, marker: str) -> int:
    ...
    for tile in walk.last_oracle_dora:
        try:
            rendered: str = tiles.mjai_string_of(tile)
        except ValueError:
            continue
        if rendered != marker:
            continue
    ...
    try:
        ...
        folded_tile = (
            ...
            if tile_raw is None
                ...
                cast("Any", tiles.physical_of(cast("str", tiles.mjai_string_of(int(tile_raw)))))
            )
        )
        folded_consume = tuple(
            cast("Any", tiles.distinct_copies([int(t) for t in cast("Any", raw.consume_tiles)]))
        )
    except Exception:  # why-broad: folding never invents ids; use-site fails closed
        return raw
    return SimpleNamespace(
        action_type=cast("Any", raw.action_type),
        tile=folded_tile,
        consume_tiles=folded_consume,
        to_mjai=cast("Any", raw.to_mjai),
    )
...
def _live_step(
    ...
    *,
    ...
    if fold:
        hand = tuple(cast("Any", tiles.distinct_copies(list(pos.hand))))
        drawn: int | None = None
...
def _do_tsumo(
    ...
    _emit(
        ...
        actor=actor,
        tile=int(cast("Any", tiles.physical_of(pai))),
    )
