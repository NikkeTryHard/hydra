...
def _do_reach(
    ...
    expected = CanonicalAction(
        ...
        tile=cast("TileId", _bridge_contracts.make_tile_id(declaration_tile)),
        called_tile=None,
        consumed_tiles=(),
        source_seat=None,
...
def _match_stashed_claim(
    ...
    logged_strings = sorted(str(t) for t in cast("Any", consumed))
    offer: Any = None
    for raw in candidates:
        ...
        yielded_strings = sorted(
            tiles.mjai_string_of(int(t)) for t in cast("Any", raw.consume_tiles)
        )
    ...
    offer_consume = tuple(sorted(int(t) for t in cast("Any", offer.consume_tiles)))
...
def _do_claim(
    ...
    state.melds[actor].append(
        VisibleMeld(
            meld_id=None,
            kind=cast("Any", kind),
            owner=cast("Seat", _bridge_contracts.make_seat(actor)),
            source_seat=cast("Seat", _bridge_contracts.make_seat(discarder)),
            called_tile=cast("TileId", _bridge_contracts.make_tile_id(called)),
    ...
    for raw in step.raw_legals:
        ...
        if tile_raw is None or int(tile_raw) != called:
            continue
        variant_consumed = sorted({int(t) for t in cast("Any", raw.consume_tiles)} - {called})
