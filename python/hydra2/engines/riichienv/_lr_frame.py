...
def _drain_game_steps(staged_path: str, *, game_id: str) -> list[list[list[_SimStep]]]:
    ...
    for kyoku_index, kyoku in enumerate(kyokus):
        ...
        for seat in range(4):
            try:
                pairs = list(kyoku.steps(seat=seat))  # pyrefly: ignore[unknown-argument-type] # C++ replay iterator yields dynamic pairs
            except ContractError:
                raise
            except Exception as exc:
            ...
            for position, (obs, act) in enumerate(pairs):
                try:
                    queues[seat].append(_extract_step(seat, obs, act, game_id=game_id))  # pyrefly: ignore[unknown-argument-type] # C++ replay iterator yields dynamic pairs
                except ContractError:
                    raise
                except Exception as exc:
...
def _extract_step(seat: int, obs: Any, act: Any, *, game_id: str) -> _SimStep:
    ...
    consume_tiles: list[int] = list(act.consume_tiles)  # pyrefly: ignore[unknown-argument-type] # engine action consume tiles dynamic
    consume = tuple(sorted(consume_tiles))
    hand_tiles: list[int] = list(obs.hand)  # pyrefly: ignore[unknown-argument-type] # engine observation hand dynamic
    distinct_hand: list[int] = tiles.distinct_copies(hand_tiles)
    hand = tuple(distinct_hand)
    drawn_raw: Any = obs.drawn_tile
    drawn = None if drawn_raw is None else int(drawn_raw)
    dora_tiles: list[int] = list(obs.dora_indicators)  # pyrefly: ignore[unknown-argument-type] # engine observation dora dynamic
    dora = tuple(dora_tiles)
    rivers: list[Any] = list(obs.discards)  # pyrefly: ignore[unknown-argument-type] # engine observation discards dynamic
    discards = tuple(tuple(int(t) for t in river) for river in rivers)
    scores_raw: Any = obs.scores
    scores = tuple(int(s) for s in scores_raw)
    if len(scores) != 4:
        raise ContractError(f"{where}: scores must cover 4 seats")
    riichi_flags: list[bool] = [bool(v) for v in obs.riichi_declared]  # pyrefly: ignore[unknown-argument-type] # engine observation riichi dynamic
    riichi = tuple(riichi_flags)
    sticks_val: int = obs.riichi_sticks
    oya_val: int = obs.oya
    honba_val: int = obs.honba
    wind_val: int = obs.round_wind
    player_val: int = obs.player_id
    riichi_quad: tuple[bool, bool, bool, bool] = cast("tuple[bool, bool, bool, bool]", riichi)
