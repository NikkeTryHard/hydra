...
class AdapterStepMixin(AdapterEventsAMixin, AdapterEventsBMixin):
        ...
        *,
        ...
        self._builder = ObservationBuilder(
            ...
            event_schema_hash=cast(
                "DigestText", _bridge_contracts.make_digest_text(self._event_schema_hash)
            ),
            packet_boundary_hash=cast(
                "DigestText", _bridge_contracts.make_digest_text(self._packet_boundary_hash)
            ),
        )
        self._mode = None
    ...
    def _context_for(self, seat: int, *, phase_override: str | None = None):
        ...
        return ActionContext(
            actor=cast("Seat", _bridge_contracts.make_seat(seat)),
            action_table_hash=self._table.digest,
    ...
    def _view_for(self, seat: int) -> tuple[tuple[CanonicalAction, ...], tuple[bool, ...]]:
        ...
        engine_legals: list[riichienv.Action] = self._engine_observation(engine_pid).legal_actions()
        assert self._builder is not None
        for owner in range(4):
            melds.extend(self._builder_melds(owner))
    ...
    def _result(self, marker: int, *, next_actor: int | None) -> TransitionResult:
        return TransitionResult(
            ...
            state_digest=cast(
                "DigestText", _bridge_contracts.make_digest_text(self._state_digest())
            ),
        )
    ...
    def _detect_decision(self) -> bool:
        ...
        if self._env.phase == int(riichienv.Phase.WaitResponse):
            responders = [
                seat
                for seat in range(4)
                if len(cast("Any", self._engine_observation(int(self._perm[seat])).legal_actions()))
                > 0
            ]
            if len(responders) > 0:
        ...
        if (
            ...
            and current >= 0
            and len(cast("Any", self._engine_observation(current).legal_actions())) > 0
        ):
