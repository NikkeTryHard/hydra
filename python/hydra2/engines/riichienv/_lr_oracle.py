...
class _WindowOracle:
    ...
    def _legals(self, pid: int) -> list[Any]:
        try:
            return list(self._engine.get_observation(pid).legal_actions())  # pyrefly: ignore[unknown-argument-type] # untyped riichienv engine observation
        except Exception as exc:
        ...
        *,
        ...
        for raw in self._legals(pid):
            ...
            if tile_str is not None:
                ...
                if tile_int is None or tiles.mjai_string_of(tile_int) != tile_str:
                    continue
            if consumed_strs is not None:
                consume_raw: list[int] = list(raw.consume_tiles)  # pyrefly: ignore[unknown-argument-type] # engine action consume tiles dynamic
                got = sorted(tiles.mjai_string_of(t) for t in consume_raw)
    ...
    def note_tsumo(self, actor: int, pai: str) -> None:
        ...
        drawn_int: int | None = None if drawn_raw is None else int(cast("Any", drawn_raw))
        if drawn_int is not None and tiles.mjai_string_of(drawn_int) == pai:
        ...
        drawn_again: int | None = None if drawn_raw is None else int(cast("Any", drawn_raw))
        if drawn_again is None or tiles.mjai_string_of(drawn_again) != pai:
            raise self._fail(f"seat {actor} drew a different tile than logged")
        self._confirm_draw_holder(actor, pai, drawn_raw)  # pyrefly: ignore[unknown-argument-type] # engine drawn tile dynamic
    ...
    def _confirm_draw_holder(self, actor: int, pai: str, drawn_raw: Any) -> None:
        ...
        try:
            ...
            drawn_id: int = int(cast("Any", drawn_raw))
            owned = drawn_id in {int(cast("Any", t)) for t in hands[actor]}
        except Exception as exc:
    ...
    def window_open(self, discarder: int) -> bool:
        ...
        env = self._engine
        try:
            waiting = int(env.phase) == int(riichienv.Phase.WaitResponse)  # pyrefly: ignore[unknown-argument-type] # untyped riichienv engine phase
        except Exception as exc:
    ...
    def position(self, pid: int) -> _TablePosition:
        ...
        try:
            ...
            legals = list(self._legals(pid))  # pyrefly: ignore[unknown-argument-type] # untyped riichienv engine legals
            hand = tuple(int(cast("Any", t)) for t in hands_raw[pid])
            rivers = tuple(tuple(int(cast("Any", t)) for t in river) for river in rivers_raw)
            lens = tuple(len(cast("Any", item)) for item in hands_raw)
            dora = tuple(int(cast("Any", t)) for t in dora_raw)
            sticks = int(cast("Any", sticks_raw))
            declared = tuple(bool(cast("Any", v)) for v in declared_raw)
            drawn = None if drawn_raw is None else int(cast("Any", drawn_raw))
        except ContractError:
            raise
        except Exception as exc:
    ...
    def check_row(
        ...
        *,
        ...
        if hand_check:
            ...
            seat_hand: list[int] = [int(cast("Any", t)) for t in hands[seat]]
            engine_strings = sorted(tiles.mjai_string_of(t) for t in seat_hand)
            if hand_strings != engine_strings:
                raise self._fail(f"seat {seat} hand differs from the engine state")
            if step.drawn is not None:
                drawn_raw: Any = env.drawn_tile
                drawn_int: int | None = None if drawn_raw is None else int(cast("Any", drawn_raw))
                if drawn_int is None or tiles.mjai_string_of(drawn_int) != tiles.mjai_string_of(
                    step.drawn
                ):
                    raise self._fail(f"seat {seat} drawn tile differs from engine state")
        for other in range(4):
            other_river: list[int] = [int(cast("Any", t)) for t in rivers[other]]
            river_strings = [tiles.mjai_string_of(t) for t in other_river]
        ...
        for owner in range(4):
            owner_rows: list[object] = list(meld_rows[owner])  # pyrefly: ignore[unknown-argument-type] # engine meld rows dynamic
            if len(owner_rows) != len(melds[owner]):
