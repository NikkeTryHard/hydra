...
from hydra2.engines.riichienv._replay_end import _do_end_game as _do_end_game
from hydra2.engines.riichienv._replay_end import _do_end_kyoku as _do_end_kyoku
...
def _do_hora(state: _GameState, walk: _KyokuWalk, kyoku: int, event: dict[str, object]) -> None:
    ...
    try:
        deltas_raw_list: list[object] = list(deltas_raw)
        deltas = [int(cast("Any", d)) for d in deltas_raw_list]
    ...
    if self_draw:
        ...
        tile = step.drawn
        winner_seat: Seat = _bridge_contracts.make_seat(winner)
        win_tile: TileId = _bridge_contracts.make_tile_id(tile)
        ...
        win_str: str = tiles.mjai_string_of(tile)
        try:
            walk.tw.do_tsumo_win(winner, win_str)
        ...
        ron_seat: Seat = _bridge_contracts.make_seat(winner)
        ron_tile: TileId = _bridge_contracts.make_tile_id(tile)
        ron_source: Seat = _bridge_contracts.make_seat(discarder)
        expected = CanonicalAction(
            kind=cast("Any", "ron"),
            actor=ron_seat,
            tile=ron_tile,
...
def _do_ryukyoku(state: _GameState, walk: _KyokuWalk, kyoku: int, event: dict[str, object]) -> None:
    ...
    try:
        draw_raw: list[object] = list(deltas_raw)
        deltas = [int(cast("Any", d)) for d in draw_raw]
    except (TypeError, ValueError) as exc:
...
def replay_game(
    ...
    rules_bound: DigestText = _bridge_contracts.make_digest_text(rules_hash)
    schema_bound: DigestText = _bridge_contracts.make_digest_text(_event_schema_hash())
    packet_bound: DigestText = _bridge_contracts.make_digest_text(_packet_boundary_hash())
    builder = ObservationBuilder(
        game_id=sim_game_id,
        rules_id=manifest.rules_id,
        rules_hash=rules_bound,
        action_table_hash=table.digest,
        expected_legal_mask_length=len(table.actions),
        event_schema_hash=schema_bound,
        packet_boundary_hash=packet_bound,
    )
    state = _GameState(
        game=record,
        split=split,
    ...
    with tempfile.TemporaryDirectory(prefix="hydra2-simreplay-") as tmpdir:
        ...
        return _walk_game(state, drained)
