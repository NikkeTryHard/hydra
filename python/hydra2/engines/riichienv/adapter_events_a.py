...
class AdapterEventsAMixin:
    ...
    _accepted_claim_id: Callable[[int], int]
    _builder: ObservationBuilder | None
    _cursor: int
    _emit: Any
    _encode_fallback_discard: Callable[[int, int], int]
    _env: riichienv.RiichiEnv | None
    _event_schema_hash: str
    _exact_draw_int: Callable[[], int]
    _game_id: str
    _hand_index: int
    _inv: list[int]
    _ippatsu: list[bool]
    _last_discard: tuple[int | None, int | None]
    _last_discard_int: Callable[[int], int]
    _latest_meld_tiles: Callable[[int], list[int]]
    _next_discard_int: Callable[[int, str], int]
    ...
    _on_kakan: Any
    _on_ryukyoku: Any
    _peek_discard_int: Callable[[int, str], int]
    _refresh_public_snapshot: Any
    _rules_hash: str
    _stamp_id: int | None
    _stamp_if: Callable[[int, str], int | None]
    ...
    def _on_start_kyoku(self, event: Any) -> None:
        ...
        self._emit(
            make_envelope(
                ...
                actor=cast("int", _bridge_contracts.make_seat(dealer)),
    ...
    def _on_dahai(self, event: Any) -> None:
        actor = self._inv[int(cast("Any", event["actor"]))]
        tile: int = self._next_discard_int(
            int(cast("Any", event["actor"])), str(cast("Any", event["pai"]))
        )
        action_id: int | None = self._stamp_if(actor, "discard")
    ...
    def _on_reach(self, event: Any, declaration: Any) -> None:
        actor = self._inv[int(cast("Any", event["actor"]))]
        tile: int = self._peek_discard_int(
            int(cast("Any", declaration["actor"])), str(cast("Any", declaration["pai"]))
        )
        action_id: int | None = self._stamp_if(actor, "riichi_discard")
    ...
    def _on_call(self, event: Any) -> None:
        actor = self._inv[int(cast("Any", event["actor"]))]
        called: int = self._last_discard_int(int(cast("Any", event["target"])))
        meld_tiles: list[int] = self._latest_meld_tiles(int(cast("Any", event["actor"])))
        consumed = tuple(sorted(t for t in meld_tiles if t != called))
        action_id: int = self._accepted_claim_id(actor)
    ...
    def _on_ankan(self, event: Any) -> None:
        actor = self._inv[int(cast("Any", event["actor"]))]
        consumed: list[int] = self._latest_meld_tiles(int(cast("Any", event["actor"])))
        action_id: int | None = self._stamp_if(actor, "ankan")
        self._emit(
            make_envelope(
                game_id=self._game_id,
                sequence=self._next_seq(),
