...
def _require_driver_bridge() -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"hydra2._native.search missing ({exc}); rebuild the bridge with `pixi run build-ext`"
        ) from exc
    if not hasattr(mod, "local_resolving_batch"):
        raise ImportError(
            "hydra2._native.search.local_resolving_batch missing (stale .so); "
    ...
    def search(
        ...
        *,
        ...
        for p in particles:
            ...
            # Any (not Any | None): the name is reused as the loop variable
            # below, and the union pins None onto every later w.world_id read.
            # None-safety still holds via the `if w is None: raise` guard.
            w: Any = None
        ...
        for w in worlds:
            wid = str(w.world_id)  # pyrefly: ignore[unknown-argument-type] # Any belief world
        ...
        try:
            out: Any = bridge.local_resolving_batch(json.dumps(batch).encode())
        except ImportError:
            raise
        ...
        try:
            table_json: str = out.table_json
            avg_json: str = out.avg_json
            table_rows = json.loads(table_json)
            avg_rows = json.loads(avg_json)
        except Exception as exc:
        ...
        for row in table_rows:
            actor_r, info_r = _take_key(row)  # pyrefly: ignore[unknown-argument-type] # Any JSON row
            _dist_r: Any = row[2]
            _visits_r: Any = row[3]
        ...
        for row in avg_rows:
            ...
            _dist_r = row[2]
            _visits_r = row[3]
            avg_table.table[(actor_r, info_r)] = _take_dist(_dist_r)  # pyrefly: ignore[unknown-argument-type] # Any JSON row
            if _visits_r is not None:
