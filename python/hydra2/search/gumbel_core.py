...
def deterministic_gumbel(
    *, case_id: str, root_seat: int, candidate_id: str, action_id: int
    ...
    try:
        draw: float = _require_search_bridge().gumbel_for_action(
            case_id, root_seat, candidate_id, action_id
        )
        return draw
    except ImportError:
        raise
...
def deterministic_root_gumbels(
    *, case_id: str, root_seat: int, candidate_id: str, legal_action_ids: tuple[int, ...]
    ...
    try:
        pairs: list[tuple[int, float]] = _require_search_bridge().gumbel_roots(
...
def scalarize_vector(vector: tuple[float, ...], root_seat: int) -> float:
    ...
    try:
        out: float = _require_search_bridge().gumbel_scalarize(list(vector), root_seat)
        return out
    except (ImportError, AttributeError):
        return vector[root_seat]
    except Exception as exc:
        raise ContractError(f"gumbel bridge scalarize failed: {exc}") from exc
...
def model_vector_for_world(
    ...
    if wid.startswith("sha256:"):
        try:
            model_out: tuple[float, float, float, float] = (
                _require_search_bridge().ismcts_model_vector(wid, candidate_id)
            )
