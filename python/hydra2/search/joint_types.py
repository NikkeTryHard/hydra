...
records, and the validation checks that prove same-information equality. Frozen
type vocabularies and the joint-Gumbel draw execute in
``hydra2._native.search`` (``crates/bridge/src/joint_types.rs`` via
``hydra_search::joint``); this module keeps the dataclasses, ``ContractError``
shaping, and thin delegates. The
...
try:
    from hydra2._native import search as _joint_types_bridge  # pyrefly: ignore[missing-import]
except ImportError:  # pragma: no cover — bridge-less env keeps pure-Python oracles
    _joint_types_bridge = None  # type: ignore[assignment]


def _bridge_attr(name: str, default: Any) -> Any:
    """Read one ``hydra2._native.search`` joint leaf with a HEAD-literal fallback."""
    try:
        bridge: Any = _joint_types_bridge
        if bridge is None:
            return default
...
# ``joint_types::register`` (``JOINT_*``); HEAD literals are the
# stale-``.so``/bridge-less fallback (same values, verified against HEAD via
# ``git show HEAD:src/hydra2/search/joint_types.py``).
THETA_IDS: tuple[str, ...] = tuple(_bridge_attr("JOINT_THETA_IDS", ("tight", "loose")))
DIVERGENCE_DIRECTIONS: frozenset[str] = frozenset(
    _bridge_attr("JOINT_DIVERGENCE_DIRECTIONS", frozenset({"kl_q_nom", "kl_nom_q", "tv"}))
)
SUPPORT_CLASSES: frozenset[str] = frozenset(
    _bridge_attr("JOINT_SUPPORT_CLASSES", frozenset({"finite_categorical", "quantal"}))
)
RATIONALITY_RULES: frozenset[str] = frozenset(
    _bridge_attr("JOINT_RATIONALITY_RULES", frozenset({"quantal_softmax", "epsilon_greedy"}))
)

_MASTER_SEED = bytes(_bridge_attr("JOINT_MASTER_SEED", b"wp13_joint_type_world_v1"))
_JOINT_GUMBEL_DOMAIN = bytes(_bridge_attr("JOINT_GUMBEL_DOMAIN", b"joint_type_world_gumbel_v1"))
...
def deterministic_joint_gumbel(
    *, case_id: str, root_seat: int, candidate_id: str, action_id: int, theta: str
    ...
    # Plain non-negative int action ids ride the bridge
    # (``hydra2._native.search.joint_types_gumbel``, bit-exact owner
    # ``hydra_search::joint::deterministic_joint_gumbel``); exotic domains the
    # oracle accepts without validation (``bool`` -> ``"True"`` payload,
    # negative, non-``int``) keep the ``hashlib`` path below so no new
    # ``ContractError`` is introduced. Stale-``.so`` falls through to the same
    # values.
    if (
        isinstance(action_id, int)
        and not isinstance(action_id, bool)
        and 0 <= action_id <= 0xFFFFFFFF
    ):
        try:
            from hydra2._native import search as _gumbel_bridge  # pyrefly: ignore[missing-import]

            return float(
                _gumbel_bridge.joint_types_gumbel(  # type: ignore[attr-defined]  # reason: joint_types leaf lands with MAIN wiring; AttributeError fallback covers stale .so
                    case_id, root_seat, candidate_id, action_id, theta
                )
            )
        except (ImportError, AttributeError):
            pass  # stale .so: fall through to the oracle below (same values)
        except Exception as exc:
            raise ContractError(f"joint bridge draw failed: {exc}") from exc
...
def _plain_pairs(d: dict[int, float]) -> list[tuple[int, float]]:
    """Insertion-order (k, float(v)) pairs preserving the oracle `+=` order."""
