...
try:
    from hydra2._native import search as _candidate0_act_bridge  # pyrefly: ignore[missing-import]
except ImportError:
    _candidate0_act_bridge = None  # type: ignore[assignment]

# Thin delegates onto ``hydra2._native.search`` (MAIN wiring lands the
# ``candidate0_*`` leaves; a stale .so falls back to the HEAD oracle inline
# at each call site — never fail closed, never a second act impl).
_sorted_concealed_fn: Any = getattr(_candidate0_act_bridge, "candidate0_sorted_concealed", None)
_has_offer_fn: Any = getattr(_candidate0_act_bridge, "candidate0_has_offer", None)
_offered_by_fn: Any = getattr(_candidate0_act_bridge, "candidate0_offered_by_fallback", None)
_candidate0_history_limit: int = getattr(_candidate0_act_bridge, "CANDIDATE0_HISTORY_LIMIT", 16)
...
def _action_context_from_obs(observation: Any) -> Any:
    ...
    if own_drawn is not None:
        if _sorted_concealed_fn is not None:
            try:
                concealed = tuple(_sorted_concealed_fn(list(concealed_hand_raw), own_drawn))  # pyrefly: ignore[unknown-argument-type] # untyped bridge concealed fn
    ...
    if _has_offer_fn is not None:
        try:
            _has_offer = bool(_has_offer_fn(phase_raw))  # pyrefly: ignore[unknown-argument-type] # untyped bridge offer fn
        except (TypeError, ValueError, OverflowError):
            _has_offer = phase_raw in ("discard_response", "kan_response")
    else:
        _has_offer = phase_raw in ("discard_response", "kan_response")
    if _has_offer:
        ...
        if pending is not None:
            ...
            if _offered_by_fn is not None:
                try:
                    offered_by = _offered_by_fn(int(actor_raw))
                except (TypeError, ValueError, OverflowError):
...
class FrozenCandidate0:
    ...
    def observe(self, packet: Any) -> None:
        ...
        if len(self._history) > _candidate0_history_limit:
