    ...
    def act(self, request: SearchRequest) -> SearchResult:
        ...
        for _ in range(total_needed):
            ...
            if all(visits[aid] > 0 for aid in legal_ids) and self._config.tie_break == (
                ...
                try:
                    _puct_acts = list(legal_ids)
                    best_aid = _require_search_bridge().puct_select(  # pyrefly: ignore[unknown-variable-type] # bridge select int
                        _puct_acts,
                        [visits[a] for a in legal_ids],
                        [v for a in legal_ids for v in value_sum[a]],
                        _puct_acts,
                        root_seat,
                        self._config.puct_c,
                        self._config.tie_break,
                    )
                except ImportError:
                    raise
                except Exception as exc:
