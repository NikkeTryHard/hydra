...
from hydra2.search.local_abstraction import (
    _optional_search_bridge as _optional_search_bridge,
)
...
class StrategyTable:
    ...
    def get(self, actor: int, info_hash: str) -> tuple[float, ...] | None:
        ...
        _bridge_contracts.make_digest_text(info_hash)
...
def make_uniform_strategy(abstraction: LocalResolvingAbstraction) -> tuple[float, ...]:
    ...
    uniform = None
    if bridge is not None and n > 0:
        uniform = getattr(bridge, "local_graph_uniform_strategy", None)
    if uniform is not None:
        try:
            dist: tuple[float, ...] = uniform(n)
            return tuple(dist)
        except (ValueError, TypeError) as exc:
            raise ContractError(str(exc)) from exc
    # HEAD oracle verbatim (n == 0 yields ``()`` — the generator never divides).
...
def exhaustive_tiny_game_values(
    *,
    ...
    if bridge is not None:
        bridge_term = getattr(bridge, "local_graph_tiny_term", None)
    ...
    for path in itertools.product(ab_order, repeat=horizon):
        ...
        for w in worlds:
            ...
            # offset per path as in planner (per-step terms bridged when
            # available; the path sum stays ``builtin sum`` by design)
            if bridge_term is not None:
                off = sum(bridge_term(str(w.world_id), aid, d) for d, aid in enumerate(path))  # pyrefly: ignore[unknown-argument-type] # untyped bridge term fn
            else:
                off = sum(
                    (
                        int(hashlib.sha256(f"{w.world_id}:{aid}:{d}".encode()).hexdigest()[:4], 16)
                        % 100
                    )
                    / 1000.0
                    - 0.05
