...
def _require_driver_bridge() -> Any:
    ...
    if not hasattr(mod, "ismcts_descent"):
        raise ImportError(
            "hydra2._native.search.ismcts_descent missing (stale .so); "
            "rebuild the bridge with `pixi run build-ext`"
        )
    return mod
        ...
        *,
        ...
        try:
            ...
            for _cand_item in legal_actions:  # pyrefly: ignore[explicit-any]
                ...
                _v: Any = getattr(_cand_any, "action_id", None)  # pyrefly: ignore[explicit-any]
                if isinstance(_v, int) and not isinstance(_v, bool):
                    _cand_ids_list.append(_v)
                elif isinstance(_cand_any, int) and not isinstance(_cand_any, bool):
                    _cand_ids_list.append(_cand_any)
                else:
                    import hashlib as _ahl

                    _h = _ahl.sha256(str(_cand_any).encode()).hexdigest()
                    _cand_ids_list.append(int(_h[:8], 16) & 0xFFFF)
            if len(_cand_ids_list) != len(set(_cand_ids_list)):
                raise ContractError("legal_actions must hold distinct ids")
            if _cand_ids_list != sorted(_cand_ids_list):
                _paired = sorted(
                    zip(_cand_ids_list, legal_actions, strict=False), key=lambda x: x[0]
                )
                legal_actions = tuple(_p for _, _p in _paired)
                _cand_ids_list = sorted(_cand_ids_list)
        except ContractError:
            raise
        except (ValueError, TypeError, AttributeError) as exc:
            raise ContractError(f"ismcts: legal_actions ids unreadable: {exc}") from exc
        ...
        # the sample-then-floats interleave below is bit-pinned, so the stream
        # must be freezable upfront; indices and floats keep oracle positions.
        _frozen: tuple[bytes, int] = _pack_rng(rng)
        ...
        for _ in range(sims_to_run):
            ...
            try:
                ...
                _lat_raw: Any = getattr(cur_world, "latent_state", {})
                _lat: Any = _lat_raw if isinstance(_lat_raw, dict) else {}
        ...
        try:
            from hydra2._native import search as _step_bridge

            _tpl_json = json.dumps(_template_doc).encode()
            _steps: tuple[list[str], list[int]] = _step_bridge.ismcts_step_hashes(
            ...
            for row in range(len(skel_hand)):
                ...
                step = skel_step[row]
                if skel_want[row]:
                    keys_rows[sim][step] = _keys[row]
                else:
                    dirs_rows[sim][step] = _dirs[row]
        except ImportError:
            raise
        batch = {
            "worlds": worlds_json,
        ...
        try:
            out: Any = bridge.ismcts_descent(json.dumps(batch).encode())
        except ImportError:
            raise
        except Exception as exc:
        ...
        try:
            tree_json: str = out.tree_json
            dump: list[dict[str, Any]] = json.loads(tree_json)
        except Exception as exc:
            raise ContractError(f"ismcts: descent tree dump malformed: {exc}") from exc
        for entry in dump:
            try:
                key = str(entry["key"])  # pyrefly: ignore[unknown-argument-type] # Any JSON value
                node = InformationSetNode(
                    key=key,
                    visits=int(entry["visits"]),  # pyrefly: ignore[unknown-argument-type] # Any JSON value
                    legal_actions=tuple(sorted_legal),
                )
                for arm in entry["arms"]:
                    aid = int(arm["action"])  # pyrefly: ignore[unknown-argument-type] # Any JSON value
                    quad = tuple(float(v) for v in arm["sum"])  # pyrefly: ignore[unknown-argument-type] # Any JSON value
                    if len(quad) != 4:
                        raise ContractError(
                            f"ismcts: descent arm sum must hold 4 entries for {aid!r}"
                        )
                    node.action_stats[aid] = _ActionStats(
                        visits=int(arm["visits"]),  # pyrefly: ignore[unknown-argument-type] # Any JSON value
        ...
        if root_node is None or len(root_node.action_stats) == 0:
            ...
            completed: bool = self._simulations > 0
        else:
