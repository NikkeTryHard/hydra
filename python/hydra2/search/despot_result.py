...
class NaturalDespotPlannerSearchMixin:
    ...
    def _lower_value_for_action(
        ...
        *,
        ...
        if len(scenarios) == 0:
            return 0.0
        try:
            from hydra2._native import search as _search_bridge

            refs = [sc.world_ref for sc in scenarios]
            seeds = [sc.semantic_seed_bytes.hex() for sc in scenarios]
            weights: float = _search_bridge.despot_lower_values(
    ...
    def _priority_proxy_for(self, action: Any, lower_value: float, visits: int) -> float:
        ...
        Bridge is the single implementation (``search.despot_priority_proxy``);
        stale extension falls back to the oracle below (same value).
        """
        try:
            from hydra2._native import search as _despot_result_bridge

            _proxy_fn = _despot_result_bridge.despot_priority_proxy  # type: ignore[attr-defined]  # reason: despot_result leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        except (ImportError, AttributeError):
            pass  # stale .so: fall through to the oracle below (same value)
        else:
            try:
                return _proxy_fn(lower_value, visits, self._config.regularization)  # pyrefly: ignore[unknown-argument-type] # untyped bridge proxy fn
            except (TypeError, ValueError, OverflowError):
                pass  # exotic input: fall through to the oracle below (same value)
        bonus = 0.0
        if visits > 0:
    ...
    def _make_telemetry(
        ...
        *,
        ...
        try:
            ...
            joules = float(
                _despot_result_bridge.despot_joules(self._model_calls, self._transitions)  # type: ignore[attr-defined]  # reason: despot_result leaf lands with MAIN wiring; AttributeError fallback covers stale .so
            )
    ...
    def _make_result(
        ...
        *,
        ...
        try:
            ...
            _clamp_fn = _despot_result_bridge.despot_clamp_lower  # type: ignore[attr-defined]  # reason: despot_result leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        except (ImportError, AttributeError):
            _clamp_fn = None  # stale .so: per-item oracle below (same value)
        vecs: list[Any] = []
        for act in legal:
            ...
            # Clamp v to [-5,5] for safety (bridge single implementation; oracle fallback)
            if _clamp_fn is not None:
                try:
                    v_clamped: float = _clamp_fn(v)  # pyrefly: ignore[unknown-argument-type] # untyped bridge clamp fn
                except (TypeError, ValueError, OverflowError):
