...
from hydra2.search.joint_types import _plain_pairs as _plain_pairs
...
def exact_joint_posterior_oracle(
    *,
    ...
    legal_sorted = tuple(sorted(legal_action_ids))
    ...
    for particle in prior.particles:
        ...
        if cached_seed is None:
            cached_seed = policy_for_theta[particle.theta].seed_domain
            _seed_cache[particle.theta] = cached_seed
        seed_domains.append(cached_seed)
    try:
        from hydra2._native import search as _joint_bridge

        normed: list[float] = _joint_bridge.joint_posterior_weights(
            prior=prior,
            info_keys=info_keys,
            seed_domains=seed_domains,
            legal_ids=list(legal_sorted),
            observed_id=observed_action_id,
            physical_t=physical_transition_prob,
        )
    except ImportError:
        raise
    except Exception as exc:
        raise ContractError(f"joint bridge posterior failed: {exc}") from exc
    weights_in = [p.weight for p in prior.particles]
...
def coherent_trajectory(
    *,
    ...
    for aid in sorted(legal_action_ids):
        ...
        if r2 < cum2:
            return world, aid
    return world, max(legal_action_ids)
...
def _is_plain_dist(d: object) -> bool:
    """Bridge-eligible dist: dict with u32-range int keys + numeric values."""
    if not isinstance(d, dict):
        return False
    for k, v in d.items():
        if not isinstance(k, int) or isinstance(k, bool) or not 0 <= k <= 0xFFFFFFFF:
            return False
...
class UncertaintySet:
    ...
    def divergence(self, *, q: dict[int, float], nominal: dict[int, float]) -> float:
        ...
        # Bridge delegate: frozen math in `hydra2._native.search`
        # (`joint_uncertainty_divergence`, bit-parity 1e-15; TV set-order 1 ulp).
        # Plain u32 keys + numeric values ride the bridge; exotic domains keep
        # the oracle below so no new ContractError is introduced.
        if (
            self.divergence_direction in ("kl_q_nom", "kl_nom_q", "tv")
            and _is_plain_dist(q)
            and _is_plain_dist(nominal)
        ):
            try:
                from hydra2._native import search as _ju_bridge  # pyrefly: ignore[missing-import]

                return float(
                    _ju_bridge.joint_uncertainty_divergence(  # type: ignore[attr-defined]  # reason: joint_uncertainty leaf lands with MAIN wiring; AttributeError fallback covers stale .so
                        self.divergence_direction, _plain_pairs(q), _plain_pairs(nominal)
                    )
                )
                ...
                raise ContractError(f"joint bridge divergence failed: {exc}") from exc
        if self.divergence_direction == "kl_q_nom":
            ...
            for aid, pq in q.items():
                ...
                if pq > 0.0 and pn > 0.0:
                    ...
                    return math.inf
            return total
        if self.divergence_direction == "kl_nom_q":
            total = 0.0
            for aid, pn in nominal.items():
                ...
                if pn > 0.0 and pq > 0.0:
                    ...
                    return math.inf
            return total
        if self.divergence_direction == "tv":
    ...
    def is_feasible(self, *, q: dict[int, float], nominal: dict[int, float]) -> bool:
        ...
        # Bridge delegate: single-FFI feasibility incl. the 1e-9 window
        # (`joint_uncertainty_is_feasible`); same plain-domain rule as above.
        if (
            self.divergence_direction in ("kl_q_nom", "kl_nom_q", "tv")
            and _is_plain_dist(q)
            and _is_plain_dist(nominal)
            ...
            try:
                ...
                return bool(
                    _ju_bridge.joint_uncertainty_is_feasible(  # type: ignore[attr-defined]  # reason: joint_uncertainty leaf lands with MAIN wiring; AttributeError fallback covers stale .so
                        self.divergence_direction,
                        _plain_pairs(q),
                        _plain_pairs(nominal),
                        self.rho,
                    )
                )
            except (ImportError, AttributeError):
                ...
                raise ContractError(f"joint bridge feasibility failed: {exc}") from exc
    ...
    *,
    ...
    try:
        for name, val in [
            ...
            _bridge_contracts.make_digest_text(val)
    except Exception as exc:
