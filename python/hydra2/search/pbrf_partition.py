...
class ChildEntry:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "target_id", _bridge_contracts.make_digest_text(self.target_id))  # pyrefly: ignore[unknown-argument-type] # untyped bridge digest
        ...
        if self.tile is not None:
            ...
            try:
                object.__setattr__(self, "tile", _bridge_contracts.make_tile_id(self.tile))  # pyrefly: ignore[unknown-argument-type] # untyped bridge tile id
            except ValueError:
...
def _fixed_allocate_oracle(
    ...
    *,
    ...
    HEAD oracle for :func:`fixed_allocate` (bridge-first translator below the
    ``hydra2._native.search.pbrf_fixed_allocate`` leaf; also the stale-``.so``
    fallback). Deterministic: sorted keys get base batches, remainder
    distributed by hash order. The schedule is frozen before search;
    outcome-derived reallocation is prohibited.
    """
...
def fixed_allocate(
    ...
    *,
    ...
    Bridge-first translator over ``hydra2._native.search.pbrf_fixed_allocate``
    (``crates/bridge/src/pbrf_forest.rs``); stale-``.so`` or bridge reject falls
    back to the HEAD oracle above. Bit-exact (integer division + sha hex order).
    """
    if not isinstance(total_batches, int) or isinstance(total_batches, bool) or total_batches <= 0:
        raise ContractError("total_batches must be positive int")
    bridge_fn: Any = getattr(_pbrf_bridge, "pbrf_fixed_allocate", None)
    if bridge_fn is None:
        return _fixed_allocate_oracle(children, total_batches=total_batches)
    if len(children) == 0:
        return {}
    keys = sorted(children.keys(), key=lambda k: (k[0], k[1]))
    try:
        counts: list[int] = bridge_fn([k[0] for k in keys], [k[1] for k in keys], total_batches)
