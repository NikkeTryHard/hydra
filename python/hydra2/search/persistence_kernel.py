...
def _require_search_bridge() -> Any:
    ...
    """
    try:
        from hydra2._native import search as bridge  # pyrefly: ignore[missing-import]
    except ImportError as exc:
        raise ImportError(
    ...
    for fn in _BRIDGE_FNS:
        if not hasattr(bridge, fn):
            raise ImportError(
                f"hydra2._native.search.{fn} missing (stale .so); "
...
class FinitePacket:
    ...
    def __post_init__(self) -> None:
        _bridge_contracts.make_digest_text(self.packet_id)
...
def compute_packet_id(*, epoch_before: str, action_id: int, branch: int) -> DigestText:
    """Packet id — bridge-owned (``search.persistence_compute_packet_id``), oracle fallback."""
    if (
        isinstance(epoch_before, str)
        and isinstance(action_id, int)
        and not isinstance(action_id, bool)
        and 0 <= action_id < 1 << 64
        and isinstance(branch, int)
        and not isinstance(branch, bool)
        and 0 <= branch < 1 << 64
    ):
        try:
            from hydra2._native import search as _pk_bridge  # pyrefly: ignore[missing-import]

            _fn = _pk_bridge.persistence_compute_packet_id  # type: ignore[attr-defined]  # reason: persistence_kernel leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        except (ImportError, AttributeError):
            pass  # stale .so: fall through to the oracle below (same bytes)
        else:
            try:
                return DigestText(_fn(epoch_before, action_id, branch))  # pyrefly: ignore[unknown-argument-type] # untyped bridge packet fn
            except (ValueError, TypeError) as exc:
                raise ContractError(f"persistence packet id rejected: {exc}") from exc
...
class BeliefEpochLite:
    ...
    def __post_init__(self) -> None:
        _bridge_contracts.make_digest_text(self.observation_hash)
        _bridge_contracts.make_digest_text(self.target_id)
...
def enumerate_packets_for(
    *,
    ...
    try:
        ...
        raise
    try:
        bridge_rows: Any = bridge.persistence_packets_for(
            epoch, action_id, num_branches=num_branches
        )
        rows: list[tuple[Any, Any, Any, Any]] = bridge_rows
    except (ValueError, TypeError) as exc:
    ...
    return tuple(
        FinitePacket(
            packet_id=packet_id,  # pyrefly: ignore[unknown-argument-type] # bridge packet row
            action_id=action_id,  # pyrefly: ignore[unknown-argument-type] # bridge packet row
            epoch_before=epoch_id,  # pyrefly: ignore[unknown-argument-type] # bridge packet row
            epoch_after=epoch_after,  # pyrefly: ignore[unknown-argument-type] # bridge packet row
            probability=probability,  # pyrefly: ignore[unknown-argument-type] # bridge packet row
...
def fresh_rebuild_epoch(
    *,
    ...
    try:
        ...
        raise
    try:
        return bridge.persistence_rebuild_epoch(epoch_before, packet)  # pyrefly: ignore[unknown-argument-type] # untyped bridge epoch fn
    except (ValueError, TypeError) as exc:
...
def commit_equals_rebuild(
    *,
    ...
    try:
        ...
        raise
    try:
        return bridge.persistence_commit_equals_rebuild(epoch_before, packet)  # pyrefly: ignore[unknown-argument-type] # untyped bridge epoch fn
    except (ValueError, TypeError) as exc:
...
def _distribute_quota(sorted_pids: list[str], quota: int) -> dict[str, int]:
    ...
    try:
        ...
        raise
    try:
        return dict(bridge.persistence_distribute_quota(list(sorted_pids), quota))  # pyrefly: ignore[unknown-argument-type] # untyped bridge quota fn
    except (ValueError, TypeError) as exc:
