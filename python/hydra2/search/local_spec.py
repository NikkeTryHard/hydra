...
them into a frozen CandidateSpec. Pure config validation executes in
``hydra2._native.search`` (``crates/bridge/src/local_spec.rs``); this module keeps
the dataclass, ``ContractError`` shaping, and thin delegates. Abstraction lives in
...
from hydra2._native import search as _local_spec_bridge  # pyrefly: ignore[missing-import]
...
class LocalResolvingConfig:
    ...
    def __post_init__(self) -> None:
        """Thin bridge delegate: ``hydra2._native.search.local_spec_check_config`` decides."""
        try:
            _local_spec_bridge.local_spec_check_config(
                self.horizon,
                self.iterations,
                self.update_rule,
                self.averaging,
            ...
            pass  # stale .so: fall through to the oracle checks below (same text)
        if (
