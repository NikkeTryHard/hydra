...
def _bridge_sha256_file(path: Path) -> str:
    ...
    try:
        from hydra2._native import canon_rng as bridge  # pyrefly: ignore[missing-import]
    except ImportError as exc:
        raise ImportError(
            "hydra2 digest authority requires the hydra2._native bridge; "
    ...
    try:
        ...
        raise ImportError(
            "hydra2._native.canon_rng.sha256_file missing (stale .so); "
...
def _bridge_sha256_hex(data: bytes) -> str:
    ...
    try:
        ...
        raise ImportError(
            "hydra2._native.canon_rng.sha256_hex missing (stale .so); "
            "rebuild the bridge (`pixi run build-ext`)"
        ) from exc
    return text
