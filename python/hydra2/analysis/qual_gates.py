...
try:
    ...
    _gates_bridge = None  # type: ignore[assignment]  # reason: analysis_leaves2 leaf lands with MAIN wiring; None/AttributeError fallback covers stale .so

from hydra2.analysis._qual_gate_leaves import _gate_eligible as _gate_eligible
from hydra2.analysis._qual_gate_leaves import _summarize_gates as _summarize_gates
from hydra2.analysis._qual_gate_leaves import _truncate_reason as _truncate_reason
...
_ANALYSIS_REPORT_KIND_BRIDGED: Any = getattr(_gates_bridge, "QUAL_GATE_REPORT_KIND", None)
ANALYSIS_REPORT_KIND: str = (
    str(_ANALYSIS_REPORT_KIND_BRIDGED)
    if _ANALYSIS_REPORT_KIND_BRIDGED is not None
    else "hydra2.analysis_gate_report"
)
_ANALYSIS_REPORT_VERSION_BRIDGED: Any = getattr(
    _gates_bridge, "QUAL_GATE_REPORT_SCHEMA_VERSION", None
)
ANALYSIS_REPORT_SCHEMA_VERSION: str = (
    str(_ANALYSIS_REPORT_VERSION_BRIDGED)
    if _ANALYSIS_REPORT_VERSION_BRIDGED is not None
    else "1.0.0"
)
...
def _make_gameplay_spec_for(candidate_id: str) -> Any:
    ...
    try:
        if candidate_id == "candidate0":
            from hydra2.search.candidate0_frozen import make_candidate0_spec

            return make_candidate0_spec()
        # candidate1 has no spec factory (ismcts_natural never defined one);
        # it falls through to the generic file-backed fallback below.
    except Exception:
        pass
    try:
        if candidate_id == "candidate2":
            from hydra2.search.despot_core import make_despot_candidate_spec
    ...
    try:
        if candidate_id == "candidate6":
            from hydra2.search.gumbel_spec import make_gumbel_candidate_spec
    ...
    *,
    ...
    if observation is None or legal_actions is None:
        ...
        try:
            ...
            seat: Seat = _bridge_contracts.make_seat(0)
            tile_zero: TileId = _bridge_contracts.make_tile_id(0)
            obs = world_actor_observation(w, actor=seat)
            legal = (
                CanonicalAction(
                    kind="pass",
                    actor=seat,
    ...
    try:
        ...
        eligible = _gate_eligible(compute_only, deterministic_ok, privileged_leak)
