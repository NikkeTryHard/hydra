"""Thin digest judge over the ``hydra2._native.canon_rng`` boundary.
...
Resolution notes:
Staleness is owned by the ``build-ext`` pixi task (inputs/outputs
fingerprinting); there is no hand-rolled mtime gate and no legacy module
name — the single install target is ``hydra2._native``.
...
def _canon_rng() -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "hydra2._native.canon_rng submodule missing; rebuild the bridge"
        ) from exc
...
def of_canonical(doc: bytes | bytearray | memoryview) -> DigestText:
    ...
    hex_text: str = _canon_rng().of_canonical_json(bytes(doc))
...
class DigestJudge:
    ...
    def verify_batch(
        ...
        for doc in docs:
            if not isinstance(doc, _CANON_DOC_TYPES):
                raise TypeError(f"judge doc must be bytes-like, got {type(doc).__name__}")
        native_out: list[str] = _canon_rng().batch_of_canonical(payload)
        recomputed: list[DigestText] = [DigestText(t) for t in native_out]
...
class StreamHandle:
    ...
    def block(self, index: int) -> tuple[int, int, int, int]:
        ...
        spec: object = _canon_rng().open_stream(self.domain, self.seed)
    ...
    def below(self, word: int, n: int) -> int:
        ...
        draw: int = _canon_rng().bounded(word & 0xFFFF_FFFF, n)
        return draw


def open_stream(*, domain: bytes | bytearray, seed: bytes | bytearray) -> StreamHandle:
...
def default_chunk_bytes() -> int:
    ...
    chunk: int = _canon_rng().default_chunk_bytes()
    return chunk
