...
Bridge: the pure record-shape leaves live in
:mod:`hydra2.tracking.clearml_mirror` (bridge-first, ``TRACKING_*`` tables
on ``hydra2._native.contracts``); this module shares them through that
import, with byte-identical stale-``.so`` fallbacks there.
"""
...
_RUN_NAME_RAW: str | None = getattr(_bridge_contracts, "TRACKING_RUN_NAME_DEFAULT", None)
RUN_NAME_DEFAULT: str = _RUN_NAME_RAW if _RUN_NAME_RAW is not None else "hydra2-training"
...
class MlflowMirror:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raw_interval: float | None = system_metrics_interval
            interval: float | None = raw_interval
    ...
    def _post_scalars(self, metrics: Mapping[str, float], step: int) -> bool:
        ...
        try:
            ...
            posted: bool = bridge.post_scalars(self._base_url, self._run_id, step, body)
            return posted
        except Exception:
            return False
    ...
    def _post_checkpoint(self, name: str, manifest: Mapping[str, Any]) -> bool:
        ...
        try:
            ...
            saved: bool = bridge.post_checkpoint(self._base_url, self._run_id, name, body)
            return saved
        except Exception:
            return False
    ...
    def start_run(self, *, run_id: str | None = None) -> str | None:
        ...
        try:
            ...
            resume: str | None = run_id if run_id is not None and run_id != "" else None
            params = _flatten_params(self._loop_config)
            if resume is not None:
                self._run_id = resume
            else:
                self._run_id = (
                    f"offline-{abs(hash((self._experiment, self._run_name))) % 10**12:012d}"
                )
            self._run = {
