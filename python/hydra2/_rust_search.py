"""Thin isolated-act judge over the ``hydra2._native.search`` boundary.
...
def _search() -> Any:
    ...
    try:
        ...
        raise RuntimeError("hydra2._native.search submodule missing; rebuild the bridge") from exc
...
def act(
    *,
    ...
    outcome: tuple[int, bool, int, int, str] = _search().act_batch(
        spec, root, legal, worlds, max_sims, max_depth, deadline_ms
    )
    action_raw, completed_raw, sims_raw, nodes_raw, digest_hex = outcome
    action = _require_u32(action_raw, name="outcome action")
    done = completed_raw
    sims = sims_raw
    nodes = nodes_raw
    digest: DigestText = _bridge_contracts.make_digest_text(digest_hex)
    if sims < 0 or sims > max_sims:
