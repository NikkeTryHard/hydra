"""Thin columnar judge over the ``hydra2._native.columnar`` boundary.
...
def _columnar() -> Any:
    ...
    try:
        ...
        raise RuntimeError("hydra2._native.columnar submodule missing; rebuild the bridge") from exc


def batch_size() -> int:
    """Single-writer batch width (Rust ``columnar::BATCH`` owns the const)."""
    width: int = _columnar().batch_size()
    return width


def capsule_stats() -> tuple[int, int]:
    """Judge observability: ``(capsules_exported, batches_exported)``."""
    stats: tuple[int, int] = _columnar().capsule_stats()
    capsules, batches = stats
    return (capsules, batches)
...
def table_dataset_hash(table: pa.Table, *, id_column: str = _ID_COLUMN) -> str:
    ...
    for value in column:
        ...
        ids.append(value)
    # Verbatim ``training/shard_reader.py:dataset_hash_of`` formula (single LF,
    # no trailing newline, utf-8); inlined here so the judge never imports
    # training (torch) — the owner stays the single source of the formula.
...
class ColumnarJudge:
    ...
    def verify_ipc_roundtrip(self, *, recorded: str, table: pa.Table, path: Path | str) -> pa.Table:
        ...
        """
        _ = self.verify_table(recorded=recorded, table=table)
        round_tripped = read_ipc_file(persist_ipc_file(table, path))
        _ = self.verify_table(recorded=recorded, table=round_tripped)
...
def probe_a_capsule_release(
    *,
    ...
    for _ in range(iters):
        capsule: object = be.next_into_capsule(list(ptrs), list(byte_caps))
