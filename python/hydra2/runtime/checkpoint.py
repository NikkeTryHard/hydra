...
class StateTreeError(CorruptArtifactError):  # pyrefly: ignore[invalid-inheritance]  # reason: base is the Rust bridge exception class at runtime (statically Any); MRO/pickling follow the bridge, covered by contract tests
...
def _digest_text(value: str) -> DigestText:
    """Validate ``value`` as canon ``sha256:<hex>`` via the bridge (fail closed)."""
    digest: DigestText = _bridge_contracts.make_digest_text(value)
    return digest
...
def build_manifest(
    *,
    ...
    manifest = CheckpointManifest(
        ...
        run_spec_hash=_digest_text(run_spec_hash),
        model_spec_hash=_digest_text(model_spec_hash),
        model_state_hash=hash_state_tree(payload["model_state"]),
        optimizer_spec_hash=_digest_text(optimizer_spec_hash),
        optimizer_state_hash=hash_state_tree(payload["optimizer_state"]),
        scheduler_spec_hash=_digest_text(scheduler_spec_hash),
        ...
        environment_hash=_digest_text(environment_hash),
        rules_hash=_digest_text(rules_hash),
        utility_manifest_hash=_digest_text(utility_manifest_hash),
        action_schema_hash=_digest_text(action_schema_hash),
        observation_schema_hash=_digest_text(observation_schema_hash),
...
def manifest_from_json(raw: Mapping[str, Any]) -> CheckpointManifest:
    ...
    try:
        manifest = CheckpointManifest(
            ...
            run_spec_hash=_digest_text(cast("str", raw["run_spec_hash"])),
            model_spec_hash=_digest_text(cast("str", raw["model_spec_hash"])),
            model_state_hash=_digest_text(cast("str", raw["model_state_hash"])),
            optimizer_spec_hash=_digest_text(cast("str", raw["optimizer_spec_hash"])),
            optimizer_state_hash=_digest_text(cast("str", raw["optimizer_state_hash"])),
            scheduler_spec_hash=_digest_text(cast("str", raw["scheduler_spec_hash"])),
            scheduler_state_hash=_digest_text(cast("str", raw["scheduler_state_hash"])),
            training_state_hash=_digest_text(cast("str", raw["training_state_hash"])),
            sampler_state_hash=_digest_text(cast("str", raw["sampler_state_hash"])),
            rng_state_hash=_digest_text(cast("str", raw["rng_state_hash"])),
            environment_hash=_digest_text(cast("str", raw["environment_hash"])),
            rules_hash=_digest_text(cast("str", raw["rules_hash"])),
            utility_manifest_hash=_digest_text(cast("str", raw["utility_manifest_hash"])),
            action_schema_hash=_digest_text(cast("str", raw["action_schema_hash"])),
            observation_schema_hash=_digest_text(cast("str", raw["observation_schema_hash"])),
            dataset_manifest_hash=(
                None
                if raw["dataset_manifest_hash"] is None
                else _digest_text(cast("str", raw["dataset_manifest_hash"]))
            ),
            rollout_artifact_hash=(
                None
                if raw["rollout_artifact_hash"] is None
                else _digest_text(cast("str", raw["rollout_artifact_hash"]))
            ),
            parent_checkpoint_hash=(
                None
                if raw["parent_checkpoint_hash"] is None
                else _digest_text(cast("str", raw["parent_checkpoint_hash"]))
            ),
        )
