...
try:
    ...
    raise ImportError(
        "hydra2._native extension with eval not importable; "
...
class WallBlock:
    ...
    def __post_init__(self) -> None:
        try:
            leaf = _bridge_eval.eval_blocks_check_wall_block  # type: ignore[attr-defined]  # reason: eval_blocks leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        except AttributeError:
            pass
        else:
            try:
                leaf(self.wall_id, self.game_ids, self.contrasts)
...
try:
    EXCLUSION_REASONS: tuple[str, ...] = tuple(
        _bridge_eval.EXCLUSION_REASONS  # type: ignore[attr-defined]  # reason: eval_blocks leaf lands with MAIN wiring; AttributeError fallback covers stale .so
    )
except AttributeError:
    EXCLUSION_REASONS = (
        "missing_telemetry",
...
def aggregate_blocks(
    ...
    *,
    ...
    try:
        sort_leaf = _bridge_eval.eval_blocks_sort_wall_ids  # type: ignore[attr-defined]  # reason: eval_blocks leaf lands with MAIN wiring; AttributeError fallback covers stale .so
    except AttributeError:
        ordered = sorted(blocks, key=lambda item: item.wall_id)
    else:
        try:
            visit_order: list[str] = sort_leaf([block.wall_id for block in blocks])
        except (ValueError, TypeError) as exc:
            raise ContractError(str(exc)) from exc
        rank = {wall_id: index for index, wall_id in enumerate(visit_order)}

        def _visit_key(item: WallBlock) -> int:
            return rank[item.wall_id]

        ordered = sorted(blocks, key=_visit_key)
...
def _first_disqualification(
    ...
    try:
        empty_missing = _bridge_eval.eval_blocks_empty_or_missing  # type: ignore[attr-defined]  # reason: eval_blocks leaf lands with MAIN wiring; AttributeError fallback covers stale .so
        flag_leaf = _bridge_eval.eval_blocks_flag_exclusion  # type: ignore[attr-defined]  # reason: eval_blocks leaf lands with MAIN wiring; AttributeError fallback covers stale .so
    except AttributeError:
        pass
    else:
        try:
            hit: tuple[str, str] | None = empty_missing(
                list(block.game_ids), len(block.contrasts), list(telemetry_by_game)
            )
        except (ValueError, TypeError) as exc:
            raise ContractError(str(exc)) from exc
        if hit is not None:
            reason, detail = hit
            return ExcludedBlock(wall_id=block.wall_id, reason=reason, detail=detail)
        # Row-invalidity arm stays Python-side: each row decision must run
        # lazily in block order with early exit (the bridge telemetry leaf
        # decides per row). Eager projection would evaluate later rows the
        ...
        try:
            flag_hit: tuple[str, str] | None = flag_leaf(
                list(block.game_ids),
                [telemetry_by_game[game_id].fallback_used for game_id in block.game_ids],
                [telemetry_by_game[game_id].timeout for game_id in block.game_ids],
                [telemetry_by_game[game_id].illegal_action for game_id in block.game_ids],
                tolerance.allow_fallback_used,
                tolerance.allow_timeout,
                tolerance.allow_illegal_action,
            )
        except (ValueError, TypeError) as exc:
            raise ContractError(str(exc)) from exc
        if flag_hit is not None:
            reason, detail = flag_hit
            return ExcludedBlock(wall_id=block.wall_id, reason=reason, detail=detail)
        return None
