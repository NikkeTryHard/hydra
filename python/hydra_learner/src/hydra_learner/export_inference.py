...
from hydra_learner.rl_experiments.drda import DRDA_RESIDUAL_MODE, DRDA_RESIDUAL_OBJECTIVE
...
class ExportInitState(Protocol):
    @property
    ...
    @property
    def weight_source(self) -> WeightSource: ...
...
def load_export_policy(
    config: ExportConfig,
) -> tuple[PolicyOnly | PolicyValueOnly, torch.Tensor, InitOnlyState, ModelConfig, dict[str, object]]:
...
def export_loaded_policy(
    ...
    *,
    ...
    checkpoint: dict[str, object] = {"torch_version": torch_version}
...
def _reject_drda_checkpoint_export(checkpoint: Mapping[str, object]) -> None:
...
def _model_config_from_checkpoint(raw: dict[str, object]) -> ModelConfig:
    return ModelConfig(
        ...
        encoder_shape=_encoder_shape(raw.get("encoder_shape", (OBS_CHANNELS, TILE_WIDTH))),
    )


def _config_int(raw: dict[str, object], key: str, default: int | None = None) -> int:
...
def _encoder_shape(raw: object) -> tuple[int, int]:
    if not isinstance(raw, (list, tuple)):
        raise ValueError("model_config.encoder_shape must be a two-item sequence")
    if len(raw) != 2 or not all(isinstance(item, int) for item in raw):
        raise ValueError("model_config.encoder_shape must be a two-item int sequence")
    return (raw[0], raw[1])
