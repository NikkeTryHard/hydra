    ...
    for epoch in range(config.epochs):
        ...
        for start in range(0, batch_rows, microbatch_size):
            ...
            if batch_on_model_device:
                ...
                bc_logits = batch.bc_logits[row_slice] if config.bc_kl_reverse_coef != 0.0 else None
            else:
                ...
                bc_logits = (
                    batch.bc_logits[row_slice].to(model_device, non_blocking=True)
                    if config.bc_kl_reverse_coef != 0.0
                    else None
                )
    ...
    *,
    ...
    if config.bc_kl_reverse_coef != 0.0:
        if bc_logits is None:
            raise ValueError("bc_logits are required when bc_kl_reverse_coef is nonzero")
