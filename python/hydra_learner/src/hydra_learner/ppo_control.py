...
from hydra_learner.checkpointing.core import ModelConfig, load_checkpoint, load_checkpoint_init_only
from hydra_learner.export_inference import ExportConfig, export_loaded_policy
from hydra_learner.mahjax.ppo_rollout import collect_mahjax_ppo_rollout
from hydra_learner.model import HydraPolicyNet
from hydra_learner.model.losses import LossWeights
from hydra_learner.model.optim import build_optimizer, set_optimizer_lr
from hydra_learner.ppo.checkpoint import (
    _model,
    _model_config,
    _optimizer_config,
    _runtime_config,
...
from hydra_learner.ppo.step import (
    PpoBatch,
    PpoTrainStepConfig,
    PpoTrainStepResult,
...
from hydra_learner.telemetry.logging import JsonlLogger, ScalarEventWriter, add_scalars
from hydra_learner.telemetry.phase import PhaseTelemetry
...
from hydra_learner.telemetry.system import resource_delta_metrics, sample_resources, snapshot_metrics
...
def _collect_rollout_batch_from_snapshot_artifact(
    *,
    ...
    extension = (
        None
        if config.rollout_inference == "mahjax-gpu"
        else _load_extension(config.extension_path or default_arena_pyo3_library_path())
    )
...
def _collect_rollout_batch(
    *,
    ...
    if config.rollout_inference in {"rust-ort", "torch-callback"} and extension is None:
        raise ValueError(f"{config.rollout_inference} rollout requires native arena extension")
    native_extension = cast("object", extension)
    ...
    if config.rollout_inference == "rust-ort":
        ...
        stage_started = time.perf_counter()
        payload = _collect_native_rollout(native_extension, config, policy_dir, rollout_seed, snapshot)
        outcome_metrics: Mapping[str, float] = {}
        native_rollout_ms = (time.perf_counter() - stage_started) * 1000.0
        stage_started = time.perf_counter()
        batch = _batch_from_native_payload_fast(payload, model)
    elif config.rollout_inference == "torch-callback":
        onnx_export_ms = 0.0
        stage_started = time.perf_counter()
        payload = _collect_callback_rollout(native_extension, config, model, rollout_seed, snapshot)
        native_rollout_ms = (time.perf_counter() - stage_started) * 1000.0
        outcome_metrics = {}
        stage_started = time.perf_counter()
        batch = _batch_from_native_payload_fast(payload, model)
    elif config.rollout_inference == "mahjax-gpu":
        onnx_export_ms = 0.0
        stage_started = time.perf_counter()
        mahjax_rollout = collect_mahjax_ppo_rollout(
            config=config, model=model, seed=rollout_seed, snapshot_metadata=snapshot
        )
        native_rollout_ms = (time.perf_counter() - stage_started) * 1000.0
        payload = {"snapshot_metadata": snapshot.to_payload(), "timing": mahjax_rollout.timing}
        outcome_metrics = mahjax_rollout.metrics
        stage_started = time.perf_counter()
        batch = mahjax_rollout.batch
    else:
        raise ValueError(f"unsupported PPO rollout inference backend {config.rollout_inference!r}")
    batch_build_ms = (time.perf_counter() - stage_started) * 1000.0
    native_timing = payload.get("timing")
    if isinstance(native_timing, dict):
        cast("dict[str, object]", native_timing).update(_legal_count_probe_metrics(batch))
...
def run_ppo_control(config: PpoControlConfig) -> dict[str, object]:
    ...
    process_pipeline = config.ppo_pipeline_depth == 1 and config.rollout_inference != "mahjax-gpu"
        ...
        while config.steps is None or global_step < config.steps:
            ...
            if pipeline_future is None:
                ...
                rollout = _RolloutResult(
                    ...
                    outcome_metrics=rollout.outcome_metrics,
            ...
            if process_pipeline and (config.steps is None or global_step < config.steps):
            ...
            metrics: dict[str, object] = {
                ...
                **rollout.outcome_metrics,
                ...
                "pipeline_enabled": process_pipeline,
                "pipeline_mode": _pipeline_mode(config, process_pipeline),
...
def _pipeline_mode(config: PpoControlConfig, process_pipeline: bool) -> str:
    if process_pipeline:
        return "process_short_lived"
    if config.ppo_pipeline_depth == 1 and config.rollout_inference == "mahjax-gpu":
        return "mahjax_serial_gpu"
    return "serial"
