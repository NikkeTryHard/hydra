...
impl GameState {
    ...
    #[inline]
    fn _execute_step(&mut self, actions: &HashMap<u8, Action>) {
        let np = NP;
            ...
            if let Some(act) = actions.get(&pid) {
                match act.action_type {
                    ...
                    ActionType::Tsumo => {
                        ...
                        let calc =
                            crate::hand_evaluator::HandEvaluator::new(hand, melds);
    ...
    /// Execute game logic for one step using array-indexed actions.
    ///
    /// Same logic as `_execute_step` but takes `&[Option<Action>; 4]` instead of
    /// `&HashMap<u8, Action>`, eliminating hashing overhead in the hot path.
    #[inline]
    ...
    pub fn _trigger_ryukyoku(&mut self, reason: &str) {
        ...
        if reason == "exhaustive_draw" {
            for (i, p) in self.players.iter().enumerate() {
                let calc =
                    crate::hand_evaluator::HandEvaluator::new(&p.hand, &p.melds);
                if calc.is_tenpai() {
                    tenpai[i] = true;
                }
            }
