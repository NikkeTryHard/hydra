...
mod unit_tests {
    ...
    fn test_is_tenpai() {
        ...
        let calc = HandEvaluator::new(&hand, &[]);
    ...
    fn test_kuikae_deadlock_repro() {
        ...
        actions.insert(3, Action::new(ActionType::Discard, Some(8), &[], None));
    ...
    fn test_match_84_agari_check() {
        ...
        let calc = HandEvaluator::new(&tiles, &[]);
    ...
    fn test_reach_to_mjai_includes_actor() {
        ...
        let action = Action::new(ActionType::Riichi, None, &[], Some(2));
        ...
        let action_no_actor = Action::new(ActionType::Riichi, None, &[], None);
    ...
    fn test_reach_accepted_mjai_includes_actor() {
        ...
        let mut actions = HashMap::new();
        actions.insert(pid, Action::new(ActionType::Riichi, None, &[], None));
        state.step(&actions);
    ...
    fn test_sanma_no_chi() {
        ...
        actions.insert(
            0,
            Action::new(ActionType::Discard, Some(discard_tile), &[], None),
        );
        state.step(&actions);
    ...
    fn test_action_encode_4p_discard() {
        ...
        let a = Action::new(ActionType::Discard, Some(0), &[], None);
        assert_eq!(a.encode().unwrap(), 0);

        // tile 4 (2m, type 1) → ID 1
        let a = Action::new(ActionType::Discard, Some(4), &[], None);
        assert_eq!(a.encode().unwrap(), 1);

        // tile 132 (C/7z, type 33) → ID 33
        let a = Action::new(ActionType::Discard, Some(132), &[], None);
    ...
    fn test_action_encode_4p_special() {
        ...
        assert_eq!(
            Action::new(ActionType::Pon, None, &[], None)
                .encode()
                .unwrap(),
            41
        );
        ...
        assert_eq!(
            Action::new(ActionType::Daiminkan, Some(0), &[], None)
                .encode()
                .unwrap(),
            42
        );
        assert_eq!(
            Action::new(ActionType::Ron, None, &[], None)
                .encode()
                .unwrap(),
            79
        );
        assert_eq!(
            Action::new(ActionType::KyushuKyuhai, None, &[], None)
                .encode()
                .unwrap(),
            80
        );
        ...
        assert!(Action::new(ActionType::Kita, None, &[], None)
            .encode()
            .is_err());
    }
    ...
    fn test_action_encode_3p_discard() {
        ...
        let a = Action::new(ActionType::Discard, Some(32), &[], None);
        assert_eq!(enc.encode(&a).unwrap(), 1);

        // 1p (type 9, tile 36) → compact 2
        let a = Action::new(ActionType::Discard, Some(36), &[], None);
        assert_eq!(enc.encode(&a).unwrap(), 2);

        // 9p (type 17, tile 68) → compact 10
        let a = Action::new(ActionType::Discard, Some(68), &[], None);
        assert_eq!(enc.encode(&a).unwrap(), 10);

        // 1s (type 18, tile 72) → compact 11
        let a = Action::new(ActionType::Discard, Some(72), &[], None);
    ...
    fn test_action_encode_3p_special() {
        ...
        assert_eq!(
            enc.encode(&Action::new(ActionType::Riichi, None, &[], None))
                .unwrap(),
            27
        );
        assert_eq!(
            enc.encode(&Action::new(ActionType::Pon, None, &[], None))
                .unwrap(),
            28
        );
        // Kan 1m (type 0) → 29 + 0 = 29
        assert_eq!(
            enc.encode(&Action::new(ActionType::Daiminkan, Some(0), &[], None))
                .unwrap(),
            29
        );
        // Kan 9m (type 8, tile 32) → 29 + 1 = 30
        assert_eq!(
            enc.encode(&Action::new(ActionType::Daiminkan, Some(32), &[], None))
                .unwrap(),
            30
        );
        // Kan C (type 33, tile 132) → 29 + 26 = 55
        assert_eq!(
            enc.encode(&Action::new(ActionType::Ankan, None, &[132], None))
                .unwrap(),
            55
        );
        assert_eq!(
            enc.encode(&Action::new(ActionType::Ron, None, &[], None))
                .unwrap(),
            56
        );
        assert_eq!(
            enc.encode(&Action::new(ActionType::KyushuKyuhai, None, &[], None))
                .unwrap(),
            57
        );
        assert_eq!(
            enc.encode(&Action::new(ActionType::Pass, None, &[], None))
                .unwrap(),
            58
        );
        assert_eq!(
            enc.encode(&Action::new(ActionType::Kita, None, &[], None))
                .unwrap(),
            59
        );
    }
    ...
    fn test_action_encode_3p_invalid_manzu() {
        ...
        let a = Action::new(ActionType::Discard, Some(16), &[], None);
        assert!(enc.encode(&a).is_err());

        // 8m (type 7, tile 28) should be invalid in 3P
        let a = Action::new(ActionType::Discard, Some(28), &[], None);
        assert!(enc.encode(&a).is_err());
    }
    ...
    fn test_action_encode_3p_chi_not_allowed() {
        ...
        let a = Action::new(ActionType::Chi, Some(36), &[40, 44], None);
        assert!(enc.encode(&a).is_err());
    }
    ...
    fn test_3p_encode_all_discard_ids_contiguous() {
        ...
        for &tile_type in &valid_types {
            ...
            let a = Action::new(ActionType::Discard, Some(tile_136), &[], None);
    ...
    fn test_kazoe_yakuman_hand_evaluator_cap() {
        ...
        let calc = HandEvaluator::new(&tiles_136, &[]);
        ...
        let res = calc.calc(18, &dora_indicators, &[], Some(cond));
