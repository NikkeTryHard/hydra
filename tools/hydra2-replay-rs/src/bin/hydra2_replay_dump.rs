...
use clap::Parser;
use hydra2_replay_rs::{ActionTable, Quarantine};
...
fn main() -> Result<(), String> {
    ...
    for path in &files {
        ...
        let version = match args.engine_version.as_str() {
            "v2" => hydra2_replay_rs::EngineVersion::V2,
            "v1" => hydra2_replay_rs::EngineVersion::V1,
            other => return Err(format!("unknown --engine-version {other:?} (want v1|v2)")),
        };
        match hydra2_replay_rs::replay_game_text_with_version(&text, &object_id, &table, version) {
            Ok(rows) => {
                n_ok += 1;
                for row in &rows {
    ...
    let quarantine_json: BTreeMap<&String, &Quarantine> =
        quarantines.iter().map(|(k, v)| (k, v)).collect();
    fs::write(
        &quarantine_path,
