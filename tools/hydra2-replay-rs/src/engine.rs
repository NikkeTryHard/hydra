...
impl EngineVersion {
    ...
    pub fn derivation_mark(&self) -> &'static str {
        match self {
            EngineVersion::V1 => crate::hydra2_row::SIM_DERIVATION_MARK,
...
impl<'a> SeatView<'a> {
    pub fn new(
        ...
        hand: Vec<u8>,
        drawn: Option<u8>,
        drawn_true: Option<u8>,
        melds: &'a [TrackedMeld],
        riichi: bool,
        score: i32,
        wall: u32,
    ) -> Result<Self, EngineDesync> {
        if seat > 3 {
...
pub fn draw_offer(view: &SeatView<'_>) -> Result<DrawOffer, EngineDesync> {
    ...
    for tile in &view.hand {
        ...
        if seen.insert(pai.clone()) {
            if let Some(first) = copies_of_string(&pai).ok().and_then(|p| p.first().copied()) {
                discards.push(first);
            }
        }
    }
...
pub fn kuikae_withheld(chi_takes: &[u8], discards: &[u8]) -> Vec<u8> {
    ...
    discards
        ...
        .filter(|d| {
            ...
            let suit = ty / 9;
            types.iter().any(|(ct, cs)| {
                *cs == suit && (*ct as i8 - ty as i8).abs() <= 1
            })
        })
        .copied()
        .collect()
...
fn kakan_adds(melds: &[TrackedMeld], hand: &[u8]) -> Vec<u8> {
    ...
    for meld in melds {
        ...
        if pai.len() == 2 && pai.as_bytes()[0] == b'5' {
            if let Ok(step) = physical_of(&pai) {
                let base = (step / 4) * 4;
