...
//!
//! Single-cdylib tree: this module registers as the `columnar` submodule
//! (`hydra2_replay_rs.columnar` today, `hydra_bridge._native.columnar` once
//! the Phase-6 maturin `module-name` cutover lands) via `register`, mirroring
//! `replay::register` / `canon_rng::register`. The legacy
...
//! applied; DO NOT guess-fix locally): `hydra-shard = { path = "../hydra-shard" }` +
...
use hydra_shard::replay::expand::{
    actor_schema, assemble_owned_games, expand_batch_owned,
};
...
fn negotiate_schema(
    ...
    let ptr = unsafe {
        pyo3::ffi::PyCapsule_GetPointer(capsule.as_ptr(), c"arrow_schema".as_ptr())
    };
    if ptr.is_null() {
    ...
    let requested = Schema::try_from(ffi_schema)
        .map_err(|err| PyValueError::new_err(format!("hydra2 requested_schema import failed: {err}")))?;
...
fn check_descriptors(ptrs: &[u64], byte_caps: &[usize]) -> Result<(), String> {
    if ptrs.is_empty() {
        return Err(String::from("hydra2 columnar descriptors must not be empty"));
    }
...
fn copy_descriptors_attached(ptrs: &[u64], byte_caps: &[usize]) -> Vec<Vec<u8>> {
    ptrs.iter()
        .zip(byte_caps.iter())
        .map(|(ptr, cap)| unsafe {
            std::slice::from_raw_parts(*ptr as *const u8, *cap).to_vec()
        })
        .collect()
}
...
fn next_into_capsule(
    ...
    let (schema, batches) =
        build_reader(schema, vec![batch]).map_err(PyValueError::new_err)?;
...
mod columnar_tests {
    ...
    fn vec_reader_serves_schema_and_rows() -> Result<(), ArrowError> {
        ...
        assert_eq!(reader.next().map(|r| r.map(|b| b.num_rows()).unwrap()), Some(1));
