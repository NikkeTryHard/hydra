...
//! Future single-cdylib tree: this module registers as the `packet`
//! submodule (`hydra2_replay_rs.packet` today,
//! `hydra_bridge._native.packet` once the Phase-6 maturin `module-name`
//! cutover lands) via `register`, mirroring `canon_rng::register`. The
//! legacy `hydra2_replay_rs` entry point is untouched.
//!
...
mod packet_tests {
    ...
    fn verified_decode_round_trip_and_guards() {
        ...
        Python::attach(|py| {
            ...
            assert!(
                decode_zstd_verified(py, raw.clone(), Some("sha256:00".to_string()), Some(
                    raw.len() as u64
                ))
                .is_err()
            );
            assert!(decode_zstd_verified(py, raw.clone(), Some(sha.clone()), Some(1)).is_err());
            assert!(decode_zstd_verified(py, b"\x28\xB5\x2F\xFD corrupt".to_vec(), None, None).is_err());
        });
    }
    ...
    fn stem_group_wall_assign_parity() {
        ...
        Python::attach(|py| {
            ...
            assert_eq!(
                group_key_for_path(py, "lobby".to_string(), "2024010112.mjai.json.zst".to_string()),
            ...
            let bytes =
                hydra_feed::canon::canonical_bytes(&list, "kat").unwrap();
            assert_eq!(
                got,
                hydra_feed::partition::wall_hash_of_canon_bytes(&bytes)
            );
