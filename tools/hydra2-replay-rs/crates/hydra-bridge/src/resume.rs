...
//! for the stats mutex, frozen pyclasses only (`&self` methods). Future
//! single-cdylib tree: this module registers as the `resume` submodule
//! (`hydra2_replay_rs.resume` today, `hydra_bridge._native.resume` once the
//! Phase-6 maturin `module-name` cutover lands — Wave5 M5, one cdylib, legacy
//! name survives only as a deleting shim) via `register`, mirroring
...
fn check_buffer_entries(entries: Vec<Bound<'_, PyDict>>) -> PyResult<usize> {
    ...
    for (idx, entry) in entries.iter().enumerate() {
        ...
        for key in key_list.iter() {
            let name: String =
                key.extract()
                    .map_err(|_| PyValueError::new_err(format!("buffer entry {idx}: key must be str")))?;
        ...
        let non_empty_str = |field: &str| -> PyResult<String> {
            let value = entry.get_item(field).map_err(|e| {
                PyValueError::new_err(format!("buffer entry {idx} field {field:?} unreadable: {e}"))
            })?;
...
fn encode_reservoir(
    ...
    let (bytes, index) = py
        .detach(|| -> Result<(Vec<u8>, hydra_feed::manifest::ReservoirIndex), String> {
    ...
    bump_blobs(py, 1);
    Ok((out, index.count, index.uncompressed_bytes, index.blob_sha256))
}
...
fn verify_prefix(
    ...
    let text =
        std::str::from_utf8(data).map_err(|_| "prefix blob must be UTF-8".to_string())?;
...
fn draw_snapshot_words(
    ...
    if n == 0 || n > MAX_SNAPSHOT_WORDS {
        return Err(format!("resume snapshot draws must be 1..={MAX_SNAPSHOT_WORDS}"));
    }
    let mut padded = [0u8; 14];
    padded[..domain.len()].copy_from_slice(domain);
    let key = hydra_feed::rng::StreamKey::new(padded, domain.len() as u32, *seed);
...
fn snapshot_words(py: Python<'_>, snap: &RngSnapshot, n: usize) -> PyResult<Vec<u32>> {
    ...
    let words =
        py.detach(|| draw_snapshot_words(&domain, &seed, block, pos, n))
            .map_err(PyValueError::new_err)?;
    bump_restores(py, 1);
    Ok(words)
}
...
fn pack_cursor(
    ...
    for (key, value) in cursor.to_dict() {
        dict.set_item(key, value).map_err(|e| {
            PyOSError::new_err(format!("resume pack_cursor failed: {e}"))
        })?;
    }
    Ok(dict.unbind())
}
...
fn unpack_cursor(raw: Bound<'_, PyDict>) -> PyResult<(u64, u64, u64, u64, u64, u64)> {
    ...
    for key in [
        ...
        let value = raw.get_item(key).map_err(|e| {
            PyValueError::new_err(format!("cursor field {key:?} unreadable: {e}"))
        })?;
        let Some(value) = value else {
            return Err(PyValueError::new_err(format!(
    ...
    let bits = py
        .detach(|| -> Result<Vec<bool>, String> {
            ...
            for (idx, (old, new)) in olds.iter().zip(news.iter()).enumerate() {
                let old_digest =
                    hash_canonical_section(old, "bridge:state_tree_sha_diff:old")
                        .map_err(|e| format!("section {idx} old: {e}"))?;
                let new_digest =
                    hash_canonical_section(new, "bridge:state_tree_sha_diff:new")
...
mod resume_tests {
    ...
    fn cursor_dict_round_trip_parity() {
        ...
        let back =
            hydra_feed::manifest::StreamCursor::from_dict(&dict).expect("round-trips");
    ...
    fn state_tree_diff_shapes_and_single_bit() {
        ...
        assert_eq!(hydra_feed::digest::sha256_hex(b"hello world"), HELLO_WORLD_SHA);
    }
