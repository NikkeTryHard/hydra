...
//! false` default on PyO3 >= 0.28 covers this submodule too). Future
//! single-cdylib tree: this module registers as the `packet_decode`
//! submodule (`hydra2_replay_rs.packet_decode` today,
//! `hydra_bridge._native.packet_decode` once the Phase-6 maturin
//! `module-name` cutover lands) via `register`, mirroring
//! `canon_rng::register` / `packet::register`. The legacy
...
fn decode_frames_batch(
    ...
    let outcomes: Vec<Result<hydra_feed::decode::GameRecord, hydra_feed::decode::DecodeErr>> =
        py.detach(|| {
            frames
                .iter()
                .zip(stems.iter())
...
fn validate_batch(
    ...
    let owned: Vec<hydra_feed::decode::GameRecord> = records
        .iter()
        .map(|r| r.borrow().inner.clone())
        .collect();
...
fn dict_required_str(
    ...
    let value = dict.get_item(field).map_err(|e| {
        PyValueError::new_err(format!("packet_decode {what} {idx} field {field:?} unreadable: {e}"))
    })?;
...
fn dict_optional_str(dict: &Bound<'_, PyDict>, field: &str, idx: usize, what: &str) -> PyResult<Option<String>> {
    let value = dict.get_item(field).map_err(|e| {
        PyValueError::new_err(format!("packet_decode {what} {idx} field {field:?} unreadable: {e}"))
    })?;
    let Some(value) = value else {
...
fn assign_partitions(
    ...
    let mut owned_games: Vec<hydra_feed::partition::GameIdentity> =
    ...
    for (idx, game) in games.iter().enumerate() {
        ...
        let source_id = match game.get_item("source_id").map_err(|e| {
            PyValueError::new_err(format!("packet_decode game {idx} field \"source_id\" unreadable: {e}"))
        })? {
    ...
    let ratios_value = spec.get_item("ratios").map_err(|e| {
        PyValueError::new_err(format!("packet_decode spec field \"ratios\" unreadable: {e}"))
    })?;
    ...
    for (key, value) in ratios_dict.iter() {
        let name: String = key.extract().map_err(|_| {
            PyValueError::new_err("packet_decode spec ratios keys must be str")
        })?;
        if value.is_instance_of::<PyBool>() {
            return Err(PyValueError::new_err(format!(
...
fn manifest_digest(py: Python<'_>, files: Vec<(String, u64)>, root: &str) -> PyResult<String> {
    ...
    let digest = py
        .detach(move || {
            let manifest =
                hydra_feed::manifest::build_manifest_from_paths(&owned_root, files);
...
mod packet_decode_tests {
    ...
    fn identity_dict<'py>(
        ...
        d.set_item("player_ids", vec![format!("p-{game_id}")]).unwrap();
        d.set_item("timestamp", format!("20240101{game_id}")).unwrap();
    ...
    fn spec_dict<'py>(py: Python<'py>) -> Bound<'py, PyDict> {
        ...
        spec.set_item("grouping_keys", Vec::<String>::new()).unwrap();
    ...
    fn batch_order_and_identity_parity() {
        ...
        Python::attach(|py| {
            ...
            for (idx, dict) in out.iter().enumerate() {
                ...
                assert_eq!(bound.get_item("ok").unwrap().unwrap().extract::<bool>().unwrap(), true);
                assert_eq!(
                    bound.get_item("index").unwrap().unwrap().extract::<usize>().unwrap(),
                    idx
                );
                let want_id = format!("g{idx}");
                assert_eq!(
                    bound.get_item("game_id").unwrap().unwrap().extract::<String>().unwrap(),
                    want_id
                );
                assert_eq!(
                    bound
    ...
    fn stems_are_parallel_and_never_synthesized() {
        ...
        Python::attach(|py| {
            let frames = vec![game_bytes("explicit"), b"{\"type\":\"start_game\"}\n{\"type\":\"end_game\"}\n".to_vec()];
            ...
            let other = hydra_feed::decode::decode_game_object(
                "other-stem",
                "other-stem",
                &frames[1],
            )
    ...
    fn taxonomy_is_closed() {
        ...
        Python::attach(|py| {
            ...
            let cases: Vec<(Vec<u8>, &str)> = vec![
                (b"\xff\xfe\n".to_vec(), "utf8"),
                (b"{\"type\":\"start_game\"}\n{\"type\":\"end_game\"}".to_vec(), "trailing_newline"),
                (b"{\"type\":\"start_game\"}\n\n{\"type\":\"end_game\"}\n".to_vec(), "blank_line"),
                (
                    b"{\"type\":\"start_game\"}\nnot json\n{\"type\":\"end_game\"}\n".to_vec(),
                    "json",
            ...
            assert_eq!(
                out[0].bind(py).get_item("ok").unwrap().unwrap().extract::<bool>().unwrap(),
                true
            );
            for (i, (_, want_class)) in cases.iter().enumerate() {
                let bound = out[i + 1].bind(py);
                assert_eq!(
                    bound.get_item("ok").unwrap().unwrap().extract::<bool>().unwrap(),
                    false
                );
    ...
    fn validate_batch_marks_and_hashes() {
        ...
        Python::attach(|py| {
            ...
            assert_eq!(ok0.get_item("valid").unwrap().unwrap().extract::<bool>().unwrap(), true);
            ...
            assert_eq!(bad1.get_item("valid").unwrap().unwrap().extract::<bool>().unwrap(), false);
            let class: Option<String> = bad1.get_item("error_class").unwrap().unwrap().extract().unwrap();
    ...
    fn dora_4_shim_fails_with_shape_gate() {
        ...
        Python::attach(|py| {
            ...
            assert_eq!(first.get_item("valid").unwrap().unwrap().extract::<bool>().unwrap(), false);
            let class: Option<String> =
                first.get_item("error_class").unwrap().unwrap().extract().unwrap();
    ...
    fn validation_hash_of_is_thin_and_skipped_probe_labelled() {
        ...
        Python::attach(|py| {
            ...
            let probe_bad =
                hydra_feed::validate::AdapterProbe::failed(ADAPTER_ERR_NOT_WIRED);
            let expect_bad =
                hydra_feed::validate::validation_hash_of(&solo_owned, &probe_bad);
    ...
    fn assign_partitions_happy_digest_and_gates() {
        ...
        Python::attach(|py| {
            ...
            let digest: String =
                bound.get_item("digest").unwrap().unwrap().extract().unwrap();
            assert!(hydra_feed::partition::is_digest_text(&digest));
            let assignments: Bound<'_, PyDict> =
                bound.get_item("assignments").unwrap().unwrap().extract().unwrap();
            ...
            let hashes: Bound<'_, PyDict> =
                bound.get_item("input_hashes").unwrap().unwrap().extract().unwrap();
            ...
            let feed_games = vec![
                hydra_feed::partition::GameIdentity::new(
                    "pa", "obj-pa", "lobby", vec!["p-pa".to_string()],
                    Some("20240101pa".to_string()), None, &sha_a,
                ),
                hydra_feed::partition::GameIdentity::new(
                    "pb", "obj-pb", "lobby", vec!["p-pb".to_string()],
                    Some("20240101pb".to_string()), None, &sha_b,
                ),
            ];
            ...
            bad_spec.set_item("grouping_keys", Vec::<String>::new()).unwrap();
    ...
    fn near_duplicates_fail_closed() {
        ...
        Python::attach(|py| {
            ...
            let g0 = identity_dict(py, "na", &format!("sha256:{}", "1".repeat(64)), Some(&wall_hex));
            let g1 = identity_dict(py, "nb", &format!("sha256:{}", "2".repeat(64)), Some(&wall_hex));
            let spec = spec_dict(py);
