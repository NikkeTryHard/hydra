    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            StreamError::NullPointer { plane } => {
                write!(f, "hydra2 replay next_into got a null buffer pointer for plane {plane}")
            }
...
fn object_id_for(file_name: &str) -> String {
    for suffix in [".jsonl.zst", ".jsonl.gz", ".mjai.json.zst", ".mjai.json.gz", ".jsonl", ".mjai.json"] {
...
impl FeedStream {
    ...
    pub fn open(cfg: FeedOpen) -> Result<Self, StreamError> {
        ...
        if cfg.source_hash.is_empty() {
            return Err(StreamError::Open("source_hash must not be empty".to_string()));
        }
        if cfg.rules_hash.is_empty() {
            return Err(StreamError::Open("rules_hash must not be empty".to_string()));
        }
        ...
        for dir in &cfg.data_dirs {
            ...
            let mut names: Vec<String> = fs::read_dir(dir)
                ...
                .map(|entry| {
                    entry
                        .map(|e| e.path())
                        .map_err(|e| StreamError::Open(format!("inputs entry {}: {e}", dir.display())))
                })
    ...
    fn stage_at(&mut self, index: usize, game_idx: u32) -> Result<u32, StreamError> {
        ...
        let bytes = fs::read(path)
            .map_err(|e| StreamError::Io(format!("input read {file_name}: {e}")))?;
        ...
        let object_id = u32::try_from(index).map_err(|_| {
            StreamError::Envelope(format!("file index overflow {index}"))
        })?;
    ...
    pub fn next_into(
        ...
        for (plane, ptr) in ptrs.iter().enumerate() {
            if *ptr == 0 {
                return Err(StreamError::NullPointer {
                    plane: plane as u8,
                });
            }
        }
        ...
        let games_quarantined = (self.quarantines.len() - snap_quars) as u32;
        ...
        match self.scratch.fill_pinned(&ptrs, &byte_caps).map_err(fill_py_err) {
            Ok(out) => {
            ...
            Err(err) => {
                ...
                self.scratch.truncate_rows(u32::try_from(snap_rows).map_err(|_| {
