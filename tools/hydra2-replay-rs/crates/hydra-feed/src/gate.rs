...
const fn saturate_idx(idx: u32) -> u16 {
    if idx > u16::MAX as u32 { u16::MAX } else { idx as u16 }
}
...
pub fn gate_game(game_idx: u32, ev: &[StackedEvent<'_>]) -> Result<SampleDecision, GateReject> {
    ...
    if last.kind != KIND_END {
        return Err(GateReject::bad_boundary(game_idx, ev.len() as u32 - 1));
    }
    ...
    for (idx, e) in ev.iter().enumerate() {
        let kind = e.kind;
        if kind == KIND_OTHER {
            return Err(GateReject::new(game_idx, idx as u32, REASON_UNKNOWN_KIND));
        }
        if kind == KIND_START {
            ...
            if !span_is_bare_end_alias(e.span) {
                ends += 1;
            }
        }
...
mod tests {
    ...
    /// `tools/hydra2-replay-rs/tests/fixtures/s4/*.jsonl` kind sequences;
