...
pub fn build_manifest_from_paths(
    ...
    paths.sort_by(|a, b| order_key(&a.0).cmp(&order_key(&b.0)));
    StreamManifest {
        files: paths
            .into_iter()
...
impl StreamCursor {
    ...
    pub fn to_dict(&self) -> BTreeMap<String, i64> {
        BTreeMap::from([
            ("byte_offset".to_string(), self.byte_offset as i64),
            ("epoch".to_string(), self.epoch as i64),
            ("file_index".to_string(), self.file_index as i64),
            ("games_seen".to_string(), self.games_seen as i64),
            ("seed".to_string(), self.seed as i64),
            ("shuffle_pos".to_string(), self.shuffle_pos as i64),
        ])
    }
...
mod tests {
    ...
    fn sha_sort_orders_by_path_hash_not_lexically() {
        ...
        want.sort_by(|a, b| order_key(a).cmp(&order_key(b)));
    ...
    fn reservoir_round_trip_v2_and_v1_drain() {
        ...
        v1.extend_from_slice(&(frame.len() as u32).to_le_bytes());
