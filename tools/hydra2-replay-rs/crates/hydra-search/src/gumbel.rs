...
impl GumbelParams {
    ...
    pub fn validate(&self) -> Result<(), SearchError> {
        if self.halving_rounds == 0 || self.halving_rounds > 5 {
            return Err(SearchError::InvalidArg { detail: "halving_rounds must be 1..5" });
        }
        ...
        for visits in &self.visits_per_round {
            if *visits == 0 || *visits > 64 {
                return Err(SearchError::InvalidArg { detail: "visits_per_round must be 1..64" });
            }
        }
...
pub fn deterministic_gumbel(
    ...
    let mut u = (int_val as f64 + 0.5) / U64_DENOM;
    if u <= 0.0 {
        u = GUMBEL_CLIP;
    }
    if u >= 1.0 {
...
pub fn deterministic_root_gumbels(
    ...
    for action in sorted {
        out.push((action, deterministic_gumbel(case_id, root_seat, candidate_id, action)?));
    }
    Ok(out)
}
...
pub fn halving_cut(
    ...
    let score = |action: u32| -> Result<f64, SearchError> {
        let q = means
            ...
            .map(|(_, q)| *q)
            .ok_or(SearchError::InvalidArg { detail: "halving means missing action" })?;
        let g = gumbels
            .iter()
            .find(|(aid, _)| *aid == action)
            .map(|(_, g)| *g)
            .ok_or(SearchError::InvalidArg { detail: "halving gumbels missing action" })?;
        if !q.is_finite() || !g.is_finite() {
            return Err(SearchError::NonFinite { context: "halving score" });
        }
        Ok(q + g)
    };
    ...
    while i < ranked.len() {
        ...
        while j < ranked.len() {
            ...
            if cur > top + HALVING_EPS {
                best = j;
            } else if (cur - top).abs() <= HALVING_EPS && tie_wins(ranked[j], ranked[best], tie) {
                best = j;
            }
            j += 1;
        }
...
pub fn puct_select(
    ...
    if !puct_c.is_finite() || puct_c <= 0.0 {
        return Err(SearchError::InvalidArg { detail: "puct_c must be finite >0" });
    }
    // Unvisited arms: PUCT still scores them (q=0, n=0) — no early return.
    let prior = 1.0 / sorted.len() as f64;
    ...
    for action in &sorted {
        let q = match node.scalar_mean(*action, root_seat)? {
            Some(q) => q,
            None => 0.0,
        };
        let n = node.stats(*action).map(|stats| stats.visits).unwrap_or(0);
        let u = puct_c * prior * (total as f64).sqrt() / (1.0 + n as f64);
        let score = q + u;
        if !score.is_finite() {
            return Err(SearchError::NonFinite { context: "puct score" });
        }
        match best {
            None => {
                best = Some(*action);
            ...
            Some(current) => {
                if score > best_score + HALVING_EPS {
                    best = Some(*action);
                    best_score = score;
                } else if (score - best_score).abs() <= HALVING_EPS
...
mod tests {
    ...
    fn gumbel_bytes_verbatim_shape() {
        ...
        let mut raw = [0u8; 8];
        raw.copy_from_slice(digest.as_slice()[..8].as_ref());
        let u = (u64::from_be_bytes(raw) as f64 + 0.5) / U64_DENOM;
    ...
    fn gumbel_golden_digests_pinned() {
        ...
        assert_eq!(a, b);
        assert!(a.is_finite() && a >= -20.0 && a <= 20.0);
    }
    ...
    fn sha_per_node_parity_t9_n64() {
        ...
        assert_eq!(deterministic_gumbel("t9_case", 1, "t9_cand", 0).expect("g"), first[0]);
    ...
    fn puct_select_matches_oracle_shape() {
        ...
        let node = IsNode::new();
        let got =
            puct_select(&node, &[8, 3], 0, PUCT_C, TieBreak::LowestActionId).expect("puct");
        assert_eq!(got, 3);
