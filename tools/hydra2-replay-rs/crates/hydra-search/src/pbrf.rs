...
pub fn conditional_carry_logps(entries: &[ChildEntry]) -> Result<Vec<f64>, SearchError> {
    ...
    for entry in entries {
        if !entry.raw_weight.is_finite() {
            return Err(SearchError::NonFinite { context: "pbrf carry raw" });
        }
...
pub fn ess_gate(ess: f64, parents: u32, threshold: f64) -> Result<bool, SearchError> {
    if !ess.is_finite() || ess < 0.0 {
        return Err(SearchError::NonFinite { context: "pbrf ess" });
    }
    if !threshold.is_finite() || threshold < 0.0 {
        return Err(SearchError::InvalidArg { detail: "ess threshold must be finite >=0" });
    }
...
mod tests {
    ...
    fn carry_is_normalized_log_not_uniform() {
        ...
        let entries =
            [honest_entry(8, 1.0), honest_entry(9, 3.0)];
        ...
        assert!((logps[0] - (-2f64.ln())).abs() > 1e-9, "must not be uniform");
    }
    ...
    fn delta_verify_legacy_window_probes() {
        ...
        assert!(!verify_delta(parent, &succ, "delta:deadbeefdeadbeef", 2, None));
    }
    ...
    fn ess_gate_threshold() {
        assert_eq!(ess_gate(4.0, 16, 0.5).expect("ess"), true);
        assert_eq!(ess_gate(12.0, 16, 0.5).expect("ess"), false);
