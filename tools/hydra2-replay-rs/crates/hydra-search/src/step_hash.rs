...
pub fn step_hashes(
    ...
    if live_lens.len() != rows || actors.len() != rows || want_keys.len() != rows {
        return Err(SearchError::InvalidArg { detail: "ismcts step lane length mismatch" });
    }
    let template: serde_json::Value = serde_json::from_slice(template_json)
        .map_err(|err| SearchError::Canon { detail: err.to_string() })?;
    if !template.is_object() {
        return Err(SearchError::InvalidArg { detail: "ismcts template must be a JSON object" });
    }
    ...
    while row < rows {
        ...
        obj.insert("live_wall_tiles_remaining".to_string(), serde_json::json!(live));
        obj.insert("actor".to_string(), serde_json::json!(actor));
        obj.insert("turn_actor".to_string(), serde_json::json!(actor));
        let dec = format!(
            "dec_hand_{}_{actor}",
            hand.iter().map(|t| t.to_string()).collect::<Vec<_>>().join("_")
        );
        obj.insert("decision_id".to_string(), serde_json::json!(dec));
        if want_keys[row] {
            obj.remove("legal_mask");
            let bytes = hydra_feed::canon::canonical_bytes_value(&doc, "search:step_key")
                .map_err(|err| SearchError::Canon { detail: err.to_string() })?;
            keys.push(hydra_feed::digest::sha256_hex(&bytes));
            dirs.push(0);
        } else {
            let bytes = hydra_feed::canon::canonical_bytes_value(&doc, "search:step_obs")
                .map_err(|err| SearchError::Canon { detail: err.to_string() })?;
...
mod tests {
    ...
    fn key_lane_drops_mask_and_dir_lane_keeps_it() {
        let tpl = template();
        let (keys, dirs) =
            step_hashes(&tpl, &[vec![1, 0], vec![3, 2]], &[31, 30], &[0, 1], &[true, false])
                .expect("hashes");
    ...
    fn lane_mismatch_and_bad_template_fail_closed() {
        let tpl = template();
        assert!(
            step_hashes(&tpl, &[vec![0]], &[1, 2], &[0], &[true]).is_err()
        );
