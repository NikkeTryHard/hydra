...
pub use crate::persistence_kernel::{
    ArmId, BeliefEpochLite, FinitePacket, ForestState, PersistenceArm, action_key,
    arm_def, check_arm, commit_equals_rebuild, compute_packet_id, enumerate_packets_for,
    fresh_rebuild_epoch, make_persistence_arm, ACTION_KIND_ORDINALS, DEPLOYABLE_DEADLINE_MS,
};
pub use crate::persistence_planner::{ActOutcome, CommitEntry, PersistencePlanner};
pub use crate::persistence_report::{
    ArmStrata, FactorialContrasts, FactorialReport, ResourceSample, CONTRAST_PAIRS,
    block_diffs, factorial_contrasts, generate_factorial_report,
    persistence_contrast_seed, stratify_surprise_miss_recovery,
};
pub use crate::persistence_spec::{
    ArmBudget, PersistenceCandidateSpec, arm_budget, deterministic_gumbel_for_arm,
    effective_deadline_ms, make_persistence_candidate_spec, spec_parameters_digest,
    validate_deadline_and_fallback, GUMBEL_SEED_DEFAULT, GUMBEL_SEED_PLANNER,
};
...
pub fn arm_defs() -> [(ArmId, (bool, bool, u64, u64, bool)); 5] {
    [
        ...
        (ArmId::C, (false, false, DEPLOYABLE_DEADLINE_MS, 2000, false)),
    ]
}
...
mod tests {
    ...
    fn facade_surface_resolves() {
        ...
        let spec =
            make_persistence_candidate_spec(ArmId::B, None, None, None, None).unwrap();
        assert_eq!(spec.candidate_id, "persistence-B");
        assert!(spec_parameters_digest(&spec).unwrap().starts_with("sha256:"));
        let packets = enumerate_packets_for("epoch:facade", 1, 2).unwrap();
        assert!(commit_equals_rebuild("epoch:facade", &packets[0]).unwrap());
        assert_eq!(compute_packet_id("epoch:facade", 1, 0).unwrap().len(), 7 + 64);
        assert_eq!(fresh_rebuild_epoch("epoch:facade", &packets[1]).unwrap().len(), 6 + 16);
