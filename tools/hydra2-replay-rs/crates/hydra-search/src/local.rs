...
impl UpdateRule {
    ...
    pub fn parse(name: &str) -> Result<UpdateRule, SearchError> {
        match name {
            ...
            _ => Err(SearchError::InvalidArg { detail: "unknown update_rule" }),
        }
    }
}
...
impl AveragingRule {
    ...
    pub fn parse(name: &str) -> Result<AveragingRule, SearchError> {
        match name {
            ...
            _ => Err(SearchError::InvalidArg { detail: "unknown averaging rule" }),
        }
    }
...
impl LocalConfig {
    ...
    pub fn validate(&self) -> Result<(), SearchError> {
        if self.horizon == 0 || self.horizon > LOCAL_MAX_HORIZON {
            return Err(SearchError::InvalidArg { detail: "horizon must be 1..16" });
        }
        if self.iterations == 0 || self.iterations > LOCAL_MAX_ITERS {
            return Err(SearchError::InvalidArg { detail: "iterations must be 1..1024" });
        }
        Ok(())
    }
}
...
pub fn check_distribution(dist: &[f64]) -> Result<(), SearchError> {
    if dist.is_empty() {
        return Err(SearchError::InvalidArg { detail: "distribution must be non-empty" });
    }
    let mut sum = 0.0;
    for prob in dist {
        if !prob.is_finite() || *prob < 0.0 || *prob > 1.0 {
            return Err(SearchError::InvalidArg { detail: "distribution entry must be in [0,1]" });
        }
        sum += *prob;
    }
    if (sum - 1.0).abs() > DIST_TOL {
        return Err(SearchError::InvalidArg { detail: "distribution must sum to 1" });
    }
    Ok(())
}
...
pub fn uniform_strategy(actions: usize) -> Result<Vec<f64>, SearchError> {
    if actions == 0 {
        return Err(SearchError::InvalidArg { detail: "action count must be >= 1" });
    }
    Ok(vec![1.0 / actions as f64; actions])
}

/// Regret-matching update (`_regret_matching_update`): positive regrets
/// normalized; `sum <= 1e-12` -> uniform.
pub fn regret_matching_update(
    current: &[f64],
    regrets: &[f64],
) -> Result<Vec<f64>, SearchError> {
    if current.is_empty() || current.len() != regrets.len() {
        return Err(SearchError::InvalidArg { detail: "regret shapes must match non-empty" });
    }
    let mut sum = 0.0;
    for regret in regrets {
        if !regret.is_finite() {
            return Err(SearchError::NonFinite { context: "local regret" });
        }
        sum += regret.max(0.0);
    }
    if sum > REGRET_EPS {
...
pub fn hedge_update(q_values: &[f64], eta: f64) -> Result<Vec<f64>, SearchError> {
    if q_values.is_empty() {
        return Err(SearchError::InvalidArg { detail: "q_values must be non-empty" });
    }
    if !eta.is_finite() {
        return Err(SearchError::NonFinite { context: "local hedge eta" });
    }
    let mut max = f64::NEG_INFINITY;
    for q in q_values {
        if !q.is_finite() {
            return Err(SearchError::NonFinite { context: "local hedge q" });
        }
        max = max.max(*q);
    }
    ...
    for q in q_values {
        ...
        if !exp.is_finite() {
            return Err(SearchError::NonFinite { context: "local hedge exp" });
        }
        exps.push(exp);
        sum += exp;
    }
...
pub fn fictitious_play_update(
    ...
    if current.is_empty() {
        return Err(SearchError::InvalidArg { detail: "strategy must be non-empty" });
    }
    if best_response_idx >= current.len() {
        return Err(SearchError::InvalidArg { detail: "best_response_idx out of range" });
    }
    if !new_weight.is_finite() || new_weight <= 0.0 {
        return Err(SearchError::InvalidArg { detail: "new_weight must be finite >0" });
    }
    let total = count as f64 + new_weight;
    let mut out = Vec::with_capacity(current.len());
    let mut i = 0;
    while i < current.len() {
        let prev = if count > 0 { current[i] * count as f64 / total } else { 0.0 };
        let add = if i == best_response_idx { new_weight / total } else { 0.0 };
        out.push(prev + add);
        i += 1;
    }
...
pub fn apply_update(
    ...
    match rule {
        UpdateRule::RegretMatching => match regrets {
            ...
            None => Err(SearchError::InvalidArg { detail: "regret_matching requires regrets" }),
        },
        UpdateRule::Hedge => match q_values {
            Some(q_values) => hedge_update(q_values, 1.0),
            None => Err(SearchError::InvalidArg { detail: "hedge requires q_values" }),
        },
        UpdateRule::FictitiousPlay => match best_response_idx {
            Some(idx) => fictitious_play_update(current, idx, visit_count, 1.0),
            None => {
                Err(SearchError::InvalidArg { detail: "fictitious_play requires best_response_idx" })
            }
        },
    }
}
...
pub fn greedy_root_pick(avg: &[f64]) -> Result<usize, SearchError> {
    if avg.is_empty() {
        return Err(SearchError::InvalidArg { detail: "avg table must be non-empty" });
    }
    let mut max = f64::NEG_INFINITY;
    for prob in avg {
        if !prob.is_finite() {
            return Err(SearchError::NonFinite { context: "local avg" });
        }
        max = max.max(*prob);
    }
    let mut idx = 0;
...
pub fn rotate_world(worlds: &[u64], iteration: u32) -> Result<u64, SearchError> {
    if worlds.is_empty() {
        return Err(SearchError::InvalidArg { detail: "worlds must be non-empty" });
    }
...
mod tests {
    ...
    fn regret_positive_normalizes() {
        let got =
            regret_matching_update(&[0.5, 0.5], &[1.0, 3.0]).expect("regret");
    ...
    fn regret_zero_falls_back_uniform() {
        ...
        let got =
            regret_matching_update(&[0.2, 0.8], &[-1.0, 0.0]).expect("regret");
    ...
    fn fictitious_play_counts() {
        ...
        let got =
            fictitious_play_update(&[0.5, 0.5], 0, 1, 1.0).expect("fp");
