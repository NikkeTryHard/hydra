...
impl ArmId {
    ...
    pub fn parse(text: &str) -> Result<Self, SearchError> {
        match text {
            ...
            "P" => Ok(ArmId::P),
            "C" => Ok(ArmId::C),
            _ => Err(SearchError::InvalidArg { detail: "unknown arm" }),
        }
    }
...
pub fn check_arm(arm: &PersistenceArm) -> Result<(), SearchError> {
    ...
    {
        return Err(SearchError::InvalidArg { detail: "arm invariant violated" });
    }
    if arm.own_deadline_ms == 0 {
        return Err(SearchError::InvalidArg { detail: "own_deadline_ms must be positive" });
    }
    match arm.id {
        ...
        ArmId::C => {
            ...
            if arm.deployable {
                return Err(SearchError::InvalidArg { detail: "C must not be deployable" });
            }
        }
    }
    Ok(())
...
impl FinitePacket {
    ...
    pub fn check(&self) -> Result<(), SearchError> {
        if !crate::eval::is_digest_text(&self.packet_id) {
            return Err(SearchError::InvalidArg { detail: "packet_id must be sha256:<hex>" });
        }
...
impl BeliefEpochLite {
    ...
    pub fn check(&self) -> Result<(), SearchError> {
        ...
        {
            return Err(SearchError::InvalidArg { detail: "epoch digest malformed" });
        }
        Ok(())
    }
}
...
pub fn enumerate_packets_for(
    ...
    if num_branches == 0 {
        return Err(SearchError::InvalidArg { detail: "num_branches must be positive" });
    }
    ...
    for (branch, probability) in probs.into_iter().enumerate() {
        ...
        let packet = FinitePacket {
            ...
            epoch_after,
            probability,
            delta: vec![action_id as i64, branch as i64],
        };
        packet.check()?;
        packets.push(packet);
    }
...
pub fn fresh_rebuild_epoch(epoch_before: &str, packet: &FinitePacket) -> Result<String, SearchError> {
    if packet.epoch_before != epoch_before {
        return Err(SearchError::InvalidArg { detail: "packet epoch_before != epoch" });
    }
    let payload = serde_json::json!({
        "epoch_before": epoch_before,
...
mod tests {
    ...
    fn arm_invariants() {
        ...
        assert_eq!(make_persistence_arm(ArmId::B).own_deadline_ms, DEPLOYABLE_DEADLINE_MS);
    ...
    fn forest_clear() {
        let mut forest = ForestState {
            ...
            action_id: 3,
            children: [("p".to_string(), enumerate_packets_for("epoch:x", 3, 1).unwrap().remove(0))]
                .into_iter()
                .collect(),
    ...
    fn degenerate_mask_migration_golden() {
        let p = crate::persistence_planner::PersistencePlanner::new(
            ArmId::B, None, None, None, None, None,
        )
        .unwrap();
        assert_eq!(p.pick_action(&[], None), Err(crate::SearchError::EmptyLegal));
    }
}
