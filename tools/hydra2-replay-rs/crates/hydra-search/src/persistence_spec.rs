...
pub fn deterministic_gumbel_for_arm(
    arm: ArmId,
    case_id: &str,
    action_id: u64,
    seed: &[u8],
) -> f64 {
    let mut preimage = Vec::with_capacity(seed.len() + 1 + arm.name().len() + 1 + case_id.len() + 1 + 20);
...
pub fn effective_deadline_ms(arm: &PersistenceArm, deadline_ms: u64) -> Result<u64, SearchError> {
    if deadline_ms == 0 {
        return Err(SearchError::InvalidArg { detail: "deadline_ms must be positive" });
    }
    match arm.id {
...
pub fn validate_deadline_and_fallback(
    ...
    if arm.id == ArmId::C && !arm.deployable && arm.extra_wait_allowance_ms == 0 {
        return Err(SearchError::InvalidArg { detail: "C extra allowance must be positive" });
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn arm_budgets() {
        assert_eq!(arm_budget(ArmId::B, None, None), ArmBudget { max_model_calls: 1, max_transitions: 0 });
        assert_eq!(
            arm_budget(ArmId::F, None, None),
            ArmBudget { max_model_calls: 32, max_transitions: 128 }
        );
        assert_eq!(
            arm_budget(ArmId::C, None, None),
            ArmBudget { max_model_calls: 64, max_transitions: 256 }
        );
        assert_eq!(
            arm_budget(ArmId::P, Some(7), Some(9)),
            ArmBudget { max_model_calls: 7, max_transitions: 9 }
        );
    }
