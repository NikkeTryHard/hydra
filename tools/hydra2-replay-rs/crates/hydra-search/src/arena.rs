...
impl SegmentArena {
    ...
    pub fn insert(&mut self, digest: &[u8], legal: &[u32]) -> Result<usize, SearchError> {
        if self.position(digest).is_some() {
            return Err(SearchError::InvalidArg { detail: "arena node already exists" });
        }
    ...
    pub fn backup(
        ...
        if pos >= self.nodes.len() {
            return Err(SearchError::InvalidArg { detail: "arena node out of range" });
        }
        for value in vector {
            if !value.is_finite() {
                return Err(SearchError::NonFinite { context: "arena backup vector" });
            }
        }
    ...
    pub fn merge_from(&mut self, other: SegmentArena) -> Result<(), SearchError> {
        ...
        for node in other.nodes {
            let kid = match other.node_keys.get(idx) {
                Some(kid) => *kid,
                None => {
                    return Err(SearchError::InvalidArg { detail: "segment keys/nodes skew" });
                }
            };
            let digest: Vec<u8> = match other.keys.resolve(kid) {
                Some(bytes) => bytes.to_vec(),
                None => {
                    return Err(SearchError::InvalidArg { detail: "segment key missing" });
                }
            };
...
fn add(left: u64, right: u64) -> Result<u64, SearchError> {
    left.checked_add(right).ok_or(SearchError::NonFinite { context: "arena counter overflow" })
}
...
fn check_budget(budget: &Budget) -> Result<(), SearchError> {
    ...
    if budget.deadline_ms == 0 || budget.deadline_ms > MAX_DEADLINE_MS {
        return Err(SearchError::InvalidArg { detail: "deadline_ms must be 1..60000" });
    }
    Ok(())
}
...
fn sim_vector(spec: &[u8], root: &[u8], sim: u64, action: u32, world: u64) -> [f64; 4] {
    ...
    let f = raw as f64 / 18_446_744_073_709_551_616.0;
    [f, 1.0 - f, 0.0, 0.0]
}
...
fn decision_digest(
    ...
    doc.insert("nodes_visited".to_string(), serde_json::json!(nodes_visited));
...
pub fn act_batch(
    ...
    while sim < budget.max_sims {
        ...
        let world = if worlds.is_empty() { 0 } else { worlds[sim as usize % worlds.len()] };
...
mod tests {
    ...
    fn budget(sims: u64) -> Budget {
        Budget { max_sims: sims, max_depth: 6, deadline_ms: 5000 }
    }
    ...
    fn frontier_carries_counters_only() {
        ...
        let bad = Budget { max_sims: 0, max_depth: 6, deadline_ms: 5000 };
    ...
    fn merge_is_deterministic_zero_to_t() {
        ...
        let rpos = right.get_or_insert(b"sha256:root", &[1, 2]).expect("insert");
        ...
        let lpos2 = left2.get_or_insert(b"sha256:root", &[1, 2]).expect("insert");
        left2.backup(lpos2, 1, [1.0, 0.0, 0.0, 0.0]).expect("backup");
        left2.backup(lpos2, 2, [0.0, 1.0, 0.0, 0.0]).expect("backup");
        let mut right2 = SegmentArena::open(2, 4, 4);
        let rpos2 = right2.get_or_insert(b"sha256:root", &[1, 2]).expect("insert");
        right2.backup(rpos2, 2, [0.0, 2.0, 0.0, 0.0]).expect("backup");
        right2.backup(rpos2, 1, [2.0, 0.0, 0.0, 0.0]).expect("backup");
    ...
    fn golden_decision_digest_stable() {
        ...
        let digest = |spec: &[u8]| {
            act_batch(spec, b"golden-root", &[1, 2, 4, 8], &worlds4(), &budget(32))
                .decision_digest
        };
