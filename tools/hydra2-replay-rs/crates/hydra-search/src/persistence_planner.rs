...
use crate::persistence_kernel::{
    ArmId, FinitePacket, ForestState, PersistenceArm, commit_equals_rebuild,
    distribute_quota, enumerate_packets_for, fresh_rebuild_epoch, make_persistence_arm,
};
use crate::persistence_spec::{
    GUMBEL_SEED_PLANNER, arm_budget, deterministic_gumbel_for_arm,
...
impl PersistencePlanner {
    ...
    fn bump(counts: &mut HashMap<String, usize>, name: &str) -> Result<(), SearchError> {
        match counts.get_mut(name) {
            ...
            None => Err(SearchError::InvalidArg { detail: "unknown surprise outcome" }),
        }
    }
    ...
    pub fn pick_action(
        ...
        for action in legal_actions {
            let score = (deterministic_gumbel_for_arm(
                self.arm.id,
                resolved,
                *action,
                &self.seed,
                ...
                % 1.0;
            if score > best_score
                || (score - best_score).abs() < 1e-9 && *action < best
            {
                best_score = score;
                best = *action;
            }
    ...
    pub fn act(
        ...
        if let Some(remaining) = remaining_ms {
            if remaining < self.fallback_margin_ms as f64 {
                ...
                self.total_joules += self.max_model_calls as f64 * 0.04
                    + self.max_transitions as f64 * 0.005;
        ...
        if matches!(self.arm.id, ArmId::R | ArmId::P) {
            ...
            self.forest = Some(ForestState {
                ...
                action_id: selected,
                children: children.into_iter().map(|p| (p.packet_id.clone(), p)).collect(),
                child_stats: stats,
        ...
        self.total_joules += calls as f64 * 0.04 + trans as f64 * 0.005;
        Ok(ActOutcome {
            selected,
            candidate_actions: legal_actions.to_vec(),
            value_vectors: vec![[0.0; 4]],
    ...
    pub fn observe(&mut self, packet_id: &str, epoch_after: &str) -> Result<(), SearchError> {
        if packet_id.is_empty() {
            return Err(SearchError::InvalidArg { detail: "packet must carry packet_id" });
        }
        match self.arm.id {
            ArmId::B | ArmId::F | ArmId::C => {
                self.forest = None;
    ...
    pub fn ponder(
        ...
        let total: usize = match ponder_quota_total {
            ...
            None => {
                ...
                let mut applied = 0;
                for stat in forest.child_stats.values_mut() {
        ...
        self.total_model_calls += total as u64;
        self.total_transitions += total as u64 / 2;
        self.ponder_budget_used += total;
        self.total_joules += total as f64 * 0.04;
        Ok(())
    }
...
mod tests {
    ...
    fn observe_commit_paths() {
        ...
        let mut p = PersistencePlanner::new(ArmId::P, None, None, None, None, None).unwrap();
        p.act("epoch:a", &[3, 5], None, None).unwrap();
        let packet_id = p.forest.as_ref().unwrap().children.keys().next().unwrap().clone();
        ...
        let packet_id = p3.forest.as_ref().unwrap().children.keys().next().unwrap().clone();
    ...
    fn epoch_and_pick_determinism() {
        ...
        assert_eq!(p.pick_action(&legal, Some("c")), p.pick_action(&legal, Some("c")));
