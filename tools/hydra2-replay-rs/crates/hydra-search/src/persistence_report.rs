...
pub fn block_diffs(a: &[f64], b: &[f64]) -> Result<Vec<f64>, SearchError> {
    if a.len() != b.len() {
        return Err(SearchError::InvalidArg { detail: "block count mismatch" });
    }
...
pub fn factorial_contrasts(
    ...
    for (name, a, b) in CONTRAST_PAIRS {
        let a_vals = placements_by_arm.get(a.name()).ok_or(SearchError::InvalidArg {
            detail: "missing arm placements",
        })?;
        let b_vals = placements_by_arm.get(b.name()).ok_or(SearchError::InvalidArg {
            detail: "missing arm placements",
        })?;
        ...
        out.insert(
            name.to_string(),
            FactorialContrasts {
                estimate: neumaier_sum(&diffs) / diffs.len() as f64,
                ci_low: low,
                ci_high: high,
    ...
    for (arm, logs) in commit_logs_by_arm {
        ...
        strata.insert(
            ...
            ArmStrata {
                ...
                miss_rate: miss as f64 / total as f64,
                total_packets: total,
            },
        );
    }
...
pub fn generate_factorial_report(
    ...
    if placements_by_arm.is_empty() {
        return Err(SearchError::InvalidArg { detail: "placements_by_arm must not be empty" });
    }
    ...
    let n = lengths[0];
    if n == 0 {
        return Err(SearchError::InvalidArg { detail: "must have at least one block" });
    }
    let ids = match block_ids {
        Some(ids) => {
            if ids.len() != n {
                return Err(SearchError::InvalidArg { detail: "block_ids length mismatch" });
            }
            ids
        }
    ...
    for (arm, vals) in placements_by_arm {
        per_arm_mean.insert(arm.clone(), neumaier_sum(vals) / vals.len() as f64);
    }
...
mod tests {
    ...
    fn factorial_block_hash_golden() {
        let samples: HashMap<String, Vec<ResourceSample>> = [
            (
                "P".to_string(),
                vec![
                    ResourceSample { model_calls: 36, exact_transitions: 144, energy_joules: 1.5 },
                    ResourceSample { model_calls: 37, exact_transitions: 148, energy_joules: 1.6 },
                ],
            ),
            (
                "F".to_string(),
                vec![
                    ResourceSample { model_calls: 32, exact_transitions: 128, energy_joules: 1.3 },
                    ResourceSample { model_calls: 33, exact_transitions: 132, energy_joules: 1.4 },
                ],
            ),
        ]
        .into_iter()
        ...
        assert!(generate_factorial_report(
            &placements(),
            Some(vec!["wb-0".to_string(), "wb-1".to_string()]),
            &same,
            &HashMap::new(),
    ...
    fn strata_rates() {
        let logs: HashMap<String, Vec<CommitEntry>> = [(
            "P".to_string(),
            vec![
                CommitEntry { arm: ArmId::P, packet_id: "p1".to_string(), outcome: "hit".to_string(), ponder_calls: 4 },
                CommitEntry { arm: ArmId::P, packet_id: "p2".to_string(), outcome: "miss_recovery".to_string(), ponder_calls: 2 },
                CommitEntry { arm: ArmId::P, packet_id: "p3".to_string(), outcome: "rebuild_no_forest".to_string(), ponder_calls: 0 },
            ],
        )]
        .into_iter()
        .collect();
