...
pub fn held_count(n: usize, ratio: f64) -> Result<usize, SearchError> {
    if n == 0 {
        return Err(SearchError::InvalidArg { detail: "blocks must be nonempty" });
    }
    if !(0.0 < ratio && ratio < 1.0) || !ratio.is_finite() {
        return Err(SearchError::InvalidArg { detail: "held_out_ratio must be in (0,1)" });
    }
    let scaled = n as f64 * ratio;
...
fn require_split_input(blocks: &[WallBlock], ratio: f64, seed: i64) -> Result<(), SearchError> {
    if blocks.is_empty() {
        return Err(SearchError::InvalidArg { detail: "blocks must be nonempty tuple" });
    }
    let mut ids = HashSet::new();
    for block in blocks {
        if block.wall_id.is_empty() || !ids.insert(block.wall_id.as_str()) {
            return Err(SearchError::InvalidArg { detail: "blocks must carry unique wall_ids" });
        }
    }
    if !(0.0 < ratio && ratio < 1.0) || !ratio.is_finite() {
...
pub fn split_blocks_held_out(
    ...
    let n = blocks.len();
    if torch_perm.len() != n {
        return Err(SearchError::InvalidArg { detail: "torch_perm must index every block" });
    }
    let mut seen = vec![false; n];
    for p in torch_perm {
        if *p >= n || seen[*p] {
            return Err(SearchError::InvalidArg { detail: "torch_perm must be a permutation" });
        }
        seen[*p] = true;
    }
    let held_n = held_count(n, held_out_ratio)?;
    let mut held: Vec<WallBlock> =
        torch_perm[..held_n].iter().map(|i| blocks[*i].clone()).collect();
    let mut train: Vec<WallBlock> =
        torch_perm[held_n..].iter().map(|i| blocks[*i].clone()).collect();
    ...
    Ok(BlockSplit { train_blocks: train, held_out_blocks: held, seed, held_out_ratio, digest })
}
...
pub fn build_wall_blocks(
    ...
    for (game_id, value) in contrasts_by_game {
        if game_id.is_empty() {
            return Err(SearchError::InvalidArg { detail: "game_id must be nonempty str" });
        }
        if !value.is_finite() {
            return Err(SearchError::InvalidArg { detail: "contrast must be finite" });
        }
    }
    ...
    for wall_id in &schedule.wall_ids {
        let game_ids: Vec<String> = match game_ids_by_wall {
            ...
            None => (0..TOTAL_GAMES_PER_WALL).map(|slot| format!("{wall_id}:g{slot}")).collect(),
        };
...
pub fn make_block_manifest(
    ...
    for block in &ordered {
        if !order.contains_key(block.wall_id.as_str()) {
            return Err(SearchError::InvalidArg { detail: "block wall not in schedule" });
        }
    }
...
pub fn balance_audit(schedule: &MatchSchedule) -> Result<BTreeMap<String, Vec<Vec<i64>>>, SearchError> {
    super::schedule::seat_pair_placements_exact(schedule)?;
...
pub fn make_eval_case(
    ...
    if case_id.is_empty() {
        return Err(SearchError::InvalidArg { detail: "case_id must be a nonempty str" });
    }
    if arms.0.is_empty() || arms.1.is_empty() || arms.0 == arms.1 {
        return Err(SearchError::InvalidArg { detail: "arms must be two distinct nonempty labels" });
    }
    if !UNCERTAINTY_UNITS.contains(&uncertainty_unit) {
        return Err(SearchError::InvalidArg { detail: "uncertainty_unit not in vocabulary" });
    }
    ...
    if !super::is_digest_text(rules_hash) {
        return Err(SearchError::InvalidArg { detail: "rules_hash must be sha256:<hex>" });
    }
    Ok(EvalCase {
...
mod tests {
    ...
    fn torch_randperm_oracle_and_split_golden() {
        ...
        assert_eq!(split.digest, "sha256:7d23d892e4a438732a0fd0d1f75f6d4310337a0d0fc638e1adc42785b2cef6d0");
        let held: Vec<&str> =
            split.held_out_blocks.iter().map(|b| b.wall_id.as_str()).collect();
    ...
    fn block_manifest_golden() {
        ...
        for (wi, wid) in ["w-001", "w-002"].iter().enumerate() {
            ...
            for (slot, gid) in ids.iter().enumerate() {
                contrasts.insert(gid.clone(), ((wi * 10 + slot) % 5) as f64 - 2.0);
            }
        ...
        assert_eq!(manifest.digest, "sha256:1ac27d655519bc786c66cc4032e2dddfa522f0ac074ac044a84774e7c61e0ac4");
        assert_eq!(manifest.schedule_hash, "sha256:7e4bd2c5a1e5b8d054e1471e6df6a07945ffd08cbcc6b551848958243250783d");
        ...
        assert_eq!(sliced.wall_ids, vec!["w-001".to_string(), "w-002".to_string()]);
    ...
    fn game_cluster_rejection_and_case_hash() {
        ...
        let c1 = make_eval_case("case-001", ("alpha", "beta"), &rules, "wall_block", false).unwrap();
        let c2 = make_eval_case("case-002", ("gamma", "delta"), &rules, "wall_block", true).unwrap();
