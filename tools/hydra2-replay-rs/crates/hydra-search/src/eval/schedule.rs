...
pub fn focal_rotation_allocations(focal: &str, others: &[String; 3]) -> Vec<[String; 4]> {
    ...
    for seat in 0..4 {
        ...
        rows.push([row[0].clone(), row[1].clone(), row[2].clone(), row[3].clone()]);
    }
    rows
}
...
impl MatchSchedule {
    ...
    pub fn check(&self) -> Result<(), SearchError> {
        if self.wall_ids.is_empty() {
            return Err(SearchError::InvalidArg { detail: "schedule needs at least one wall" });
        }
        let mut seen = std::collections::HashSet::new();
        for wall_id in &self.wall_ids {
            if wall_id.is_empty() || !seen.insert(wall_id.as_str()) {
                return Err(SearchError::InvalidArg { detail: "wall_ids must be unique" });
            }
        }
        ...
        for digest in [
            ...
            if !super::is_digest_text(digest) {
                return Err(SearchError::InvalidArg { detail: "schedule digest malformed" });
            }
        }
...
fn require_labels(labels: &[String]) -> Result<[String; 4], SearchError> {
    if labels.len() != 4 || labels.iter().any(String::is_empty) {
        return Err(SearchError::InvalidArg { detail: "labels must be four nonempty strings" });
    }
    let uniq: std::collections::HashSet<&str> =
        labels.iter().map(String::as_str).collect();
    if uniq.len() != 4 {
        return Err(SearchError::InvalidArg { detail: "labels must be distinct" });
    }
    Ok([labels[0].clone(), labels[1].clone(), labels[2].clone(), labels[3].clone()])
}
...
pub fn latency_draw(master_seed: &[u8], key: &serde_json::Value) -> Result<usize, SearchError> {
    ...
    Ok((n % LATENCY_CLASSES.len() as u64) as usize)
}
...
pub fn build_match_schedule(
    ...
    if wall_ids.is_empty() || wall_ids.iter().any(String::is_empty) {
        return Err(SearchError::InvalidArg { detail: "wall_ids must be nonempty strings" });
    }
    let uniq: std::collections::HashSet<&str> =
        wall_ids.iter().map(String::as_str).collect();
...
pub fn seat_pair_placements_exact(schedule: &MatchSchedule) -> Result<(), SearchError> {
    ...
    for (wall_index, wall_id) in schedule.wall_ids.iter().enumerate() {
        ...
        let rotations =
            &schedule.seat_allocations[base + SYMMETRIC_ALLOCATIONS_PER_WALL..base + TOTAL_GAMES_PER_WALL];
        ...
        if hits != [3, 3, 3, 3] {
            return Err(SearchError::InvalidArg { detail: "seat coverage != [3,3,3,3]" });
        }
        let mut rotation_seats: Vec<usize> = rotations
            .iter()
            .flat_map(|row| {
        ...
        if rotation_seats != vec![0, 1, 2, 3] {
            return Err(SearchError::InvalidArg { detail: "focal rotation != all seats" });
        }
        let _ = wall_id;
    }
    Ok(())
...
mod tests {
    ...
    fn latency_percent3_kat() {
        ...
        for (slot, class_index) in want.iter().enumerate() {
            ...
            assert_eq!(latency_draw(&master(), &key).unwrap(), *class_index, "slot {slot}");
        }
    }
    ...
    fn allocation_geometry() {
        ...
        assert_eq!(
            focal_rotation_allocations("A", &["B".to_string(), "C".to_string(), "D".to_string()]).len(),
            4
        );
    ...
    fn schedule_constructor_guards() {
        ...
        assert!(build_match_schedule(&empty, &labels(), &format!("sha256:{}", "a".repeat(64)), &master(), "e", "s").is_err());
        let dup = vec!["w".to_string(), "w".to_string()];
        assert!(build_match_schedule(&dup, &labels(), &format!("sha256:{}", "a".repeat(64)), &master(), "e", "s").is_err());
        let walls = vec!["w".to_string()];
        let bad_labels = vec!["A".to_string(), "A".to_string(), "C".to_string(), "D".to_string()];
        assert!(build_match_schedule(&walls, &bad_labels, &format!("sha256:{}", "a".repeat(64)), &master(), "e", "s").is_err());
        assert!(build_match_schedule(&walls, &labels(), "not-a-digest", &master(), "e", "s").is_err());
    }
}
