        ...
        if let Some(obj) = value.as_object() {
            for key in ["wall", "wall_tiles", "tiles"] {
                if let Some(list) = obj.get(key).and_then(|v| v.as_array()) {
                    ...
                    for entry in list {
                        let tile = entry.as_u64().or_else(|| entry.as_i64().map(|v| v as u64));
                        match tile {
                            Some(v) if v <= 135 => tiles.push(v as u8),
                            _ => {
...
impl KyokuTrack {
    ...
    pub fn install(
        ...
        for seat in 0..4 {
            if tehais[seat].len() != 13 {
                return Err(fail(
                    "tile-conservation",
                    &format!(
                        "seat {seat} tehais hold {} tiles, not 13",
                        tehais[seat].len()
                    ),
                ));
            }
            track.tehais[seat] = tehais[seat].clone();
            for pai in &tehais[seat] {
    ...
    pub fn remove_one(
        ...
        if let Some(d) = drawn {
            if hand.contains(&d) {
                if let Ok(rendered) = mjai_string_of(d) {
                    if rendered == pai {
...
mod tests {
    ...
    fn walled_games_parse_with_wall_tiles() {
        ...
        assert_eq!(game.wall_tiles, Some((0..136u16).map(|v| v as u8).collect::<Vec<u8>>()));
