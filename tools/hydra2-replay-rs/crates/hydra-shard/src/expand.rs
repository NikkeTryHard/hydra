...
impl ExpandedBatch {
    ...
    /// for the order-independent `dataset_hash`.
    pub fn decision_ids(&self) -> &StringArray {
        self.batch
            .column(DECISION_ID_COL)
            .as_any()
            .downcast_ref::<StringArray>()
            .expect("actor schema: decision_id column is utf8")
    }
}
...
fn blob_field_u32(
    ...
    match doc.get(key) {
        ...
        Some(other) => Err(blob_err(index, format!("{key} must be an integer, got {}", json_kind(other)))),
        None => Err(blob_err(index, format!("missing {key}"))),
    }
}
    ...
    let chosen_action_id = match doc.get("chosen_action_id") {
        ...
        Some(serde_json::Value::Number(number)) => Some(
            number
                ...
                .map(|value| value as u32)
                .ok_or_else(|| {
                    blob_err(index, format!("decisions[{position}].chosen_action_id must fit u32"))
                })?,
        ),
        Some(other) => {
            return Err(blob_err(
...
fn capture_row(
    ...
    if !obs.is_object() {
        return Err(ExpandError::ObservationNotObject {
            decision_id,
        });
    }
    firewall_check_obs(&decision_id, &obs)?;
    ...
    let obs_text =
        String::from_utf8(obs_canonical.clone()).expect("canonical bytes are ascii JSON");
...
/// `(5,)` exactly.
fn firewall_check_obs(decision_id: &str, obs: &serde_json::Value) -> Result<(), ExpandError> {
    let map = obs.as_object().expect("caller checked is_object");
...
mod tests {
    ...
    fn assembler_round_trip_matches_struct_call() {
        ...
        assert_eq!(expanded.decision_ids().value(0), "ga:d0000");
