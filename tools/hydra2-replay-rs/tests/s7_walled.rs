...
fn wall_digest_matches_python_authority() {
    let tiles: Vec<u8> = (0..136u16).map(|v| v as u8).collect();
...
fn vocabulary_fork_is_unified() {
    use hydra2_replay_rs::mjai_event::{
...
fn check_walled_identity(name: &str, object_id: &str, expect_rows: usize) {
    ...
    let prov = provenance();
    let json_a: Vec<serde_json::Value> =
        rows_a.iter().map(|r| r.to_decision_json(&prov)).collect();
    let json_b: Vec<serde_json::Value> =
        rows_b.iter().map(|r| r.to_decision_json(&prov)).collect();
    ...
    let priv_rows = hydra2_replay_rs::expand_privileged_rows(&game, "train", Some(&digest))
    ...
    for (p, r) in priv_rows.iter().zip(rows_a.iter()) {
        ...
        hydra2_replay_rs::check_privileged_label(&label).expect("label gate");
    }
}
...
fn mode_machine_matches_expander_contract() {
    use hydra2_replay_rs::WalkerMode;
...
fn walled_ankan_binds_consecutive_block() {
    ...
    let ankan = &rows[0];
    match &ankan.chosen {
        hydra2_replay_rs::ChosenAction::Ankan { consumed } => {
...
fn walled_kakan_resolves_single_added_copy() {
    ...
    let pon = rows
        .iter()
        .find(|r| matches!(r.chosen, hydra2_replay_rs::ChosenAction::Pon { .. }))
        .expect("pon row");
    match &pon.chosen {
        hydra2_replay_rs::ChosenAction::Pon { called, consumed, .. } => {
    ...
    let kakan = rows
        .iter()
        .find(|r| matches!(r.chosen, hydra2_replay_rs::ChosenAction::Kakan { .. }))
        .expect("kakan row");
    match &kakan.chosen {
        hydra2_replay_rs::ChosenAction::Kakan { tile } => {
            // The added fourth copy is the one pool copy absent from the pon
            // triple (deterministic, contract-valid on legal logs).
            if let hydra2_replay_rs::ChosenAction::Pon { consumed, .. } = &pon.chosen {
...
fn sim_mark_rides_wall_less_only() {
    ...
    let prov = provenance();
    for row in &rows {
        let value = row.to_decision_json(&prov);
        let doc = actor_doc(&value);
        assert_eq!(
            doc.get("projection").and_then(|v| v.as_str()),
