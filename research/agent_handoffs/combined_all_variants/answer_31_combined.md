<combined_run_record run_id="answer_31" variant_id="runtime-authority-promotion" schema_version="1">
  <metadata>
    <notes>Self-contained combined record for Agent 31. It preserves the compact prompt shell and artifact manifest generated from the authoritative prompt config, plus the preserved answer text.</notes>
...
You are evaluating a narrow runtime/search challenger for Hydra, not the whole architecture.
</role>
...
Work toward the strongest exact blueprint for selective runtime search authority promotion.

We want a concrete answer that makes clear:
- what live runtime authority seam already exists today
- why current search outputs remain learner-only
- what exact producer, trust, and freshness conditions would be required before runtime-authoritative reuse is safe
- what evaluation harness would be needed to prove this lane should outrank the current DeltaQ-centered path
- whether this lane is truly next-build ready or still a later challenger

Do not assume code presence equals runtime authority. Use the artifacts below to derive your conclusions.
</task>
...
Why it matters: Current runtime decision seam: authoritative ponder reuse is gated off, otherwise inference uses actor logits plus SaF fast path.
...
Why it matters: Concrete runtime tests showing authoritative cache reuse, learner-only exclusion, and cache invalidation behavior.
...
Why it matters: Cache insertion, generation invalidation, trusted lookups, and manager helpers for the runtime authority lane.
...
Why it matters: Concrete test surface for cache hits, predictive child caching, ponder priority, and from_tree semantics.
...
Why it matters: Additional dense runtime/cache/trust test surface including trust ordering, generation invalidation, and authoritative filtering.
...
Why it matters: Short reference snapshot reinforcing that runtime/eval/inference surfaces already exist in the live tree.
...
Why it matters: Additional small reference slice on live runtime/search surfaces to clear the prompt floor without adding filler.
...
Why it matters: Shows the gap between current endgame reality and the stronger target direction.
...
Why it matters: Current endgame implementation reality for comparison with promoted doctrine and later challenger lanes.
...
Why it matters: Current Hand-EV reality for distinguishing shipped baseline realism from later refinement lanes.
...
Why it matters: Useful reserve-shelf/runtime challenger surface for context on why broader runtime/search work remains later.
...
High confidence: Hydra already has a real **runtime search authority seam**, but it is **dormant by design**. Search also already has a **live runtime advisory seam**. The authority seam is the `Authoritative` ponder-cache short-circuit in inference; the advisory seam is the active SaF logit-residual path fed by search-derived context. What is *not* present in the evidence is a producer that deserves `Authoritative`, or a promotion harness strong enough to move this lane ahead of the now-closed DeltaQ-centered path. So this is **not next-build ready as the new mainline**. It is **next-build ready only as a narrow proving experiment**: exact observed-root, discard-only, calibrated authority promotion with a hard falsification harness. (Artifacts 02–11, 13–18, 25–30)
...
1. **Advisory runtime seam (already live):** search-derived per-action context is encoded into the observation, decoded into SaF features, and used by a small MLP to shift actor logits before action selection. That is real runtime use of search outputs today, just not full action authority. (Artifacts 06, 07, 08)

2. **Authority seam (implemented but inert):** `infer_with_budget()` first tries `lookup_ponder_trusted(info_state_hash, TrustLevel::Authoritative)`. If such a hit exists, it returns the cached `exit_policy` as the acting policy; otherwise it falls back to actor logits plus SaF. Tests prove that a manually inserted `Authoritative` cache entry changes runtime action selection, while a `LearnerOnly` entry does not, and cache invalidation rejects stale generations. (Artifacts 08, 10, 11, 13)

There is also already a **selective routing surface** for which states are worth pondering: the priority score is a function of small top-2 gap, high risk, and low particle ESS. That is already the right kind of “hard-state” selector for a selective authority challenger. (Artifact 09)

So the seam is not hypothetical. The precise statement is:

* **search is already runtime-advisory**
* **cache-based runtime override already exists**
* **what is missing is trustworthy promotion into `Authoritative`**

### 2) Why current search outputs remain learner-only

**Direct artifact support, high confidence**

There are four local reasons.

First, the code says so explicitly: inference comments say only `Authoritative` cache hits may drive runtime action selection, and that currently **nothing qualifies**, which keeps ponder outputs learner-only. The archive roadmap says the same thing. (Artifacts 08, 04)

Second, the default producer semantics say so: `PonderResult::from_tree()` records provenance, but defaults `trust_level` to `LearnerOnly`. The cache layer enforces trust thresholds and generation freshness, but the artifact set does not show any real producer that promotes a result past `LearnerOnly`. (Artifacts 09, 11, 13)

Third, the only concrete shared search producer shown in the evidence is **teacher-side**, not deployment-side: the live self-play producer builds a root with legal discard children, evaluates each child with the model value head, runs **root-only AFBS**, then emits visit-based ExIt labels and DeltaQ labels from that tree. That is a narrow label-amplification object, not a validated runtime solver. (Artifact 05)

Fourth, promoted doctrine is still **supervision-first and selective-search-later**. The reconciliation doc ranks truthful target generation and evaluator realism ahead of AFBS runtime promotion, describes `afbs.rs` as a shell rather than a fully integrated runtime, and explicitly rejects broad search-first or full public-belief search as the immediate move. (Artifact 03)

**External evidence, medium-high confidence**

That local reading matches the literature. In systems where search is truly authoritative, search is not merely a labeler: AlphaZero acts by MCTS and then trains the network back toward the search policy; ExIt explicitly decomposes planning from generalization in the same way. Reanalyse and targeted search-control work strengthen the *teacher/data-selection* side of the loop by generating improved targets or revisiting better states. That lines up far more closely with Hydra’s shipped DeltaQ/ExIt/sidecar regime than with unvalidated direct cache override. ([arXiv][1])

The caution is also literature-consistent. Gumbel planning gives a policy-improvement guarantee only when action-values are correctly evaluated. Recent analysis of MuZero finds that learned models struggle to evaluate unseen policies accurately, that the error grows as the evaluated policy moves away from the data-collection policy, and that policy priors help precisely because they regularize search toward regions where the model is more accurate. That is exactly the pattern under which a same-net approximate planner is useful as a teacher or advisory bias, but not automatically safe as blanket runtime authority. ([OpenReview][2])

Hydra’s domain makes that caution stronger, not weaker. The cleanest search-with-guarantees result in imperfect information is ReBeL, and that is for **two-player zero-sum** public-belief search. Multiplayer imperfect-information systems such as Pluribus can work very well, but even there the lack of strong general guarantees outside the two-player zero-sum setting is part of the story, and recent multi-agent planning papers still frame planning/search integration as intrinsically difficult. Hydra is a four-player imperfect-information game, so “code exists” is nowhere near enough to justify runtime authority. ([arXiv][3])
...
The artifact set already implies at least three producer classes:

* **`LiveExitRootOnly`**: the current label producer in `live_exit.rs`. Root-only, discard-only, same-net child evaluation. This should remain **LearnerOnly**. (Artifact 05)
* **`ObservedRoot` ponder result**: a cache entry for the exact current observed state. This is the only class that is a plausible future candidate for `Authoritative`. (Artifacts 09, 11)
* **`SpeculativeChildHint`**: predicted-child cache entries already have their own namespace. These should not be first-wave authoritative results. (Artifact 11)

**Recommendation**

Start with **exact observed-root, discard-only** authority promotion. Do **not** start with speculative-child authority, and do **not** start with full action-space authority. That narrowing is justified because the strongest live search-derived surfaces in the evidence are discard-compatible: the live producer is built around legal discards, the surviving honest DeltaQ object is discard-compatible, and SaF’s current per-action delta-q path is tile/discard oriented. (Artifacts 04, 05, 06)
...
Use a separate **promotion gate**:

* raw producer writes a cache result plus quality/provenance metadata
* promoter classifies it into `LearnerOnly`, `Advisory`, `WarmStart`, or `Authoritative`
* inference only obeys `Authoritative`

That separation matters because the current evidence proves a dangerous fact: the runtime seam is strong enough that *any* cache entry marked `Authoritative` can steer action selection, while the current cache semantics shown in evidence only enforce generation freshness and minimum trust. That is why promotion cannot just be “set the enum.”

A minimal acceptance rule should look like this:
...
That is the cleanest statement of the missing proof objects.

A key blocked surface: `infer_with_budget()` hashes only `obs` and passes `legal` separately. The artifact set does **not** prove that legality is part of the cache identity. Until that proof exists, authoritativeness should require an explicit legal-mask hash. (Artifact 08)

Another blocked surface: `PonderCache::get_trusted()` filters by generation and trust only. The artifact set does **not** show a runtime lookup layer that additionally verifies `source_net_hash`, `source_version`, or `cache_namespace`. So either that check must be added, or the proof that it already happens elsewhere is missing. (Artifact 11)

#### 3.3 Freshness

**High confidence on current reality**

Current freshness is basically **generation-based**. That is good, but too coarse for authority. (Artifacts 09, 11, 13)
...
* **timestamp only as audit/TTL**, never as the primary authority criterion

This is also how practical tree-reuse systems behave: KataGo reuses search structure, but it explicitly discusses stale nodes and update handling rather than treating reuse as inherently trustworthy. ([GitHub][4])

### 4) The evaluation harness needed to prove this lane should outrank the DeltaQ-centered path

**Direct artifact support, high confidence**

Hydra’s current eval surface is not enough. `eval.rs` gives coarse game metrics and latency/throughput gates; validation gives loss and policy agreement; unit tests prove cache semantics. None of that is a decision-quality authority-promotion harness. (Artifacts 27, 28, 29, 30)

So the missing harness has to do four jobs.

#### A. State-level decision-quality bench

Use Hydra’s own validation style as the template: a **200K stratified-state** bench is already the doctrine for testing positive decision improvement. Reuse that scale and make the comparator explicit. (Artifact 20)
...
Against a stronger reference policy/value object (Q_{\text{ref}}(s,a)), compute:
...
The lane should not outrank DeltaQ unless:

* lower 95% confidence bound of (\Delta_{\text{Auth}}) is (>0)
* negative-share stays below Hydra’s own G0-style tolerance
* the win is present on the slices Hydra actually routes to pondering: low top-2 gap, high risk, low ESS, late wall

This is the right comparison because search is **already** helping runtime through SaF, so the challenger must beat the shipped advisory path, not a straw-man actor-only baseline. (Artifacts 06–08, 20)
...
If that oracle stack does not exist yet, say so. That is a real blocker, not a detail.
...
Re-run those, then add the tests that actually matter for promotion:
...
under the same deployment envelope Hydra already names:
...
* conditional gain given a hit
* net gain (=) hit rate × conditional gain

A useful paired-sample sizing rule is:

[
n \approx \left(\frac{1.96 + 0.84}{\delta} \sigma_d \right)^2
]

where (\delta) is the mean-placement effect you want to detect and (\sigma_d) is the paired SD. If (\sigma_d \in [1.0, 1.2]) and you care about (\delta=0.02), you are in the ballpark of **20k–28k paired games**.

#### D. Calibration bench

Authority needs a calibrated acceptance model, not hand-wavy thresholds.
...
Then fit a simple calibrator for “does authority beat current SaF on this bucket?” and set thresholds from held-out lower-confidence-bound performance, not from intuition.
...
* any claim that the current root-only label producer is already a runtime-authoritative solver
* any reprioritization above the DeltaQ-centered path before the new harness wins
...
* **High confidence:** the authority seam exists.
* **High confidence:** current outputs are intentionally learner-only for runtime override.
* **Medium-high confidence:** the current evidence supports keeping DeltaQ/SaF ahead of this lane.
* **Medium confidence:** the best surviving next step in this lane is a narrow exact-root authority experiment, not broad runtime search promotion.

So the strongest exact blueprint is:

1. keep DeltaQ/SaF as the incumbent,
2. add a real promotion gate,
3. restrict first-wave authority to exact observed-root discard states,
4. add missing provenance/freshness fields,
5. run the 200K-state + paired-arena harness,
6. only then ask whether this lane deserves to outrank the current path.

Until that proof exists, this is a **later challenger**, not the next Hydra mainline.
