<combined_run_record run_id="answer_1" variant_id="agent_answers_mixed_transcript" schema_version="1">
  <metadata>
  ...
  <prompt_section>
  <prompt_text status="preserved" source_path="agent_answers/ANSWER_1.md" extracted_from="mixed_transcript">
  ...
  **Important:** use `research/BUILD_AGENT_PROMPT.md` as a cross-cutting overlay on the rest of the docs, but do so in a **design/advisory** sense rather than a code-execution sense. In other words, use it to understand ordering, rigor, and what must not be hand-waved.
  ...
  <answer_section>
  <answer_text status="preserved" source_path="agent_answers/ANSWER_1.md" extracted_from="mixed_transcript">
  <![CDATA[I treated the docs zip as the primary source, then cross-checked the key claims against the raw markdown and the cited papers.
