...
Anything that assumes new main model heads is rejected by scope and by `HYDRA_RECONCILIATION.md`. Anything that assumes current per-mode utility labels already exist in mainline is blocked by missing label/provenance surfaces. ([GitHub][1])
...
where `g(s)=top2_policy_gap`, `r(s)=risk_score`, and `e(s)=particle_ess`, exactly as encoded by `compute_ponder_priority(top2_gap, risk_score, particle_ess)` in `afbs.rs`. The current queue input surface is therefore a hash plus three scalar diagnostics. ([GitHub][3])
...
The docs define hard-state compute triggers and an on-turn latency envelope for AFBS, but `infer_with_budget` does not currently call AFBS. ([GitHub][5])
