...
<metadata>
<notes>Self-contained combined record for Agent 27. Keeps compact prompt shell, artifact manifest from authoritative prompt config, plus preserved answer text.</notes>
...
<![CDATA[# Hydra prompt — stronger public-teacher belief semantics: repo-grounded impl blueprint

<role>
You are Hydra research agent producing repo-grounded impl blueprint.
Do not give memo. Answer itself must be blueprint.
</role>

<task>
Task only = repo-grounded impl + validation blueprint for stronger public-teacher belief-semantics lane.
Assume semantic-object question handled by separate research agent. Do not spend answer re-deriving whole belief theory unless needed for concrete file-level choice.
...
- step-by-step impl-ready blueprint minimizing guesswork, avoiding tranche widen
Prefer smallest buildable path. Reuse existing carrier/mask/loss plumbing when possible. Do not invent new heads or broad train/infer parity project unless artifacts force it.
</task>
...
- use search/browse aggressively when it can strengthen answer: find original paper, adjacent papers, official docs, repos, other primary sources; use abstracts/summaries mainly for discovery, not final evidence base
- use bash tool to run Python for lightweight research support when helpful: calculations, math checks, parsers, data inspection, sanity checks, quick experiments, validation
...
- inspect own draft before finishing: if confident claim not objectively justified by visible evidence, downgrade to inference, proposal, or blocked
- do not finish early; keep looping through discovery, thinking, testing, validation until info saturated, falsified, or truly blocked, do not stop because first pass looked plausible
</rules>
...
- no vague answer
- include reasoning
- when you sound confident, show justification for confidence level
...
Type: `literal`
Why it matters: Explains why prompt family split into two narrow research agents instead of one blended packet.
...
Why it matters: Canonical archive SSOT says current Stage-A belief and mixture teachers are projection-grade, should stay off until repaired.
...
Why it matters: Promoted architecture doctrine says advanced activation remains staged because public-teacher belief semantics not equally closed, and gives intended Mixture-SIB plus CT-SMC belief story.
...
Why it matters: Confirms fixed 192x34 compatibility surface and current runtime-reality authority note.
...
Why it matters: Shows current Stage-A teacher already invoked in replay loader and exact place stronger teacher can replace it.
...
Why it matters: Shows batch carrier, masks, exact tensor contract already exist for belief_fields and mixture_weight.
...
Why it matters: Shows current per-sample loss objects and helps separate teacher-object questions from existing loss plumbing.
...
Why it matters: Confirms belief_fields and mixture_weight intentionally blocked in `train.rs` today.
...
Why it matters: Tests prove current keep-off posture intentional and enforced, not accidental missing work.
...
Why it matters: Shows current public CT-SMC summary API available at bridge seam.
...
Why it matters: Reminder agent should treat artifacts as evidence, distinguish direct support from inference, produce impl-ready blueprint not memo.
...
Why it matters: High-signal archive excerpt reconstructing semantically correct public belief object, why Stage weak, why current belief/mix stay off.
...
Why it matters: Archive excerpt summarizing how optional advanced targets consumed in losses and batches, useful for spotting belief-object versus loss-shape mismatches.
...
Why it matters: Shows exactly how search/belief planes and presence masks written into live encoder surface.
...
Why it matters: Shows CT-SMC context already flows into runtime feature production in tested way, useful for grounding later stronger teacher ideas.
...
Why it matters: Defines live advanced output heads and model config surface that belief-related supervision must plug into without widening heads.
...
Why it matters: Shows how belief, mixture, delta_q, exit, safety residual targets/masks carried through collation into HydraTargets.
...
Type: `file_full`
Source: `crates/hydra-train/src/bin/train/loss_policy.rs`
Why it matters: Full small file showing BC/RL policy gates blocking belief, mixture, opponent-hand-type activation in `train.rs` today.
...
Why it matters: Large end-to-end integration test surface proving what training/runtime pipeline already closes and how advanced targets exercised today.
...
<![CDATA[Smallest repo-grounded path:

`belief_fields` can move before `mixture_weight`, but only if you stop treating 16×34 surface as true per-component mixture teacher and instead use it as carrier for one **public-posterior belief object** duplicated across four existing component blocks. `mixture_weight` should stay fully off in v1, at producer and `train.rs` policy-gate layers.

rec grounded by four direct repo facts.

First, current loader seam already supports belief present while mixture absent. `build_stage_a_belief_targets` returns belief and mixture independently as `(Option<[f32;16*34]>, Option<[f32;4]>, bool, bool)`, so carrier for “belief on, mixture off” already exists with no batch-schema change (Artifact 10 LOADER L0308-L0335).

Second, batch and model contracts already right size. `belief_fields_target` is `[B,16,34]`, `mixture_weight_target` is `[B,4]`, with sample-level masks `[B]`; model outputs same shapes; no new head needed (Artifacts 11, 25, 33, 35, 36).

Third, current Stage-A mixture weaker than it looks. `build_stage_a_teacher` calls `MixtureSib::new(...)` then never calls `bayesian_update`, `posterior_step`, `reproject`, split, or merge before reading back beliefs and weights (Artifact 09 BELIEF L0096-L0153; Artifact 30 SINK L0196-L0210, L0347-L0361). `MixtureSib::new` initializes **all components with same projected belief** and equal log-weights (Artifact 30 SINK L0202-L0207). So Stage-A already emits identical component beliefs with uniform weights. I also checked current trust formula numerically: with 4 uniform weights, entropy is `ln 4 ≈ 1.386294` and trust about `0.700001`, above default `0.55` threshold, while mixture emission blocked by `1.15` entropy threshold. So current code can emit “belief” broadly while “mixture” is uninformative and suppressed. Existing trust/entropy heuristics wrong thing to preserve.

Fourth, `mixture_weight` has explicit identity problem in current runtime code. `build_search_features` sorts components by current weight before writing them into fixed planes (Artifact 27 BRIDGE L0316-L0335). So component slot 0 = “top-ranked this sample,” not canonical cross-sample identity. Even before doctrine, repo already treats component order as local/ranked, not stable.

So split work into substrate phase and activation phase.

## 1. Substrate phase: land stronger lane with train gates unchanged

Do this first, before touching `train.rs` policy gates.

### 1.1 `crates/hydra-core/src/ct_smc.rs`

**Direct support:** current API exposes `weighted_mean_tile_count(tile,col)` and `mean_allocation()`; latter unweighted, and archive explicitly warns future public belief teacher should use weighted path, not simple particle averaging (Artifact 17 CTSMC L0303-L0323; Artifact 32 CTSMC L0341-L0359; Artifact 23 A15 L0331-L0334).
...
**Why justified:** smallest change closing weighted-posterior seam without touching heads, loaders, or runtime encoder layout.
...
Validation simple: create two particles with different allocations and log weights `0.0` and `-10.0`; `weighted_mean_allocation()` should sit near first particle, while `mean_allocation()` sits near arithmetic average.

### 1.2 `crates/hydra-train/src/teacher/belief.rs`

Main file to change first.
...
**What should be removed from active path:**

* `build_uniform_kernel()`
* `project_hidden_count_to_col_sums(hidden_tiles)`
* `build_stage_a_teacher(...)` as loader’s default path

Keep only as legacy helpers if you want archived regression tests, but remove from active loader path.

**What to add:**
Narrow, semantics-agnostic encoder from stronger public teacher object into existing 16×34 carrier. Teacher object from Agent can vary; encoding path need not.

I would standardize train-time object as rowwise public posteriors:

[
P_t(z \mid k) = \frac{\bar B_t(k,z)}{\sum_{z'} \bar B_t(k,z')}
]

for rows with positive remaining mass, zero rows otherwise (Artifact 23 A15 L0264-L0272).

Then encode into existing 16×34 slot by **repeating same 4-zone row distribution across all four component blocks**:
...
**Why recommend duplication across component blocks:**
Least invasive way to reuse `[16,34]` without inventing new head or pretending component identity exists. Also not semantic regression relative to current Stage-A path, because Stage-A already initializes identical component beliefs and never differentiates them before emission.
...
Replace current `trust_threshold` / `mixture_entropy_threshold` gating with hard validity checks:

* finite
* nonnegative
* if counts available before normalization: row sums match public remaining counts within tolerance
* if counts available before normalization: column sums match public zone sizes within tolerance
* after normalization: each valid row sums to `1 ± eps`; invalid rows exactly zero

Better gate than current entropy/trust pair, which not tied to teacher correctness.
...
* replace current “finite and nonnegative” belief test with:
  ...
  * if counts path available: `public_belief_target_preserves_row_and_col_marginals_before_normalization`

## 2. Loader phase: swap teacher, not carriers

### 2.1 `crates/hydra-train/src/data/mjai_loader.rs`

Exact seam = `build_stage_a_belief_targets(...)` (Artifact 10 LOADER L0308-L0335).

**Change:**
Rename to something like `build_belief_targets(...)` and stop computing single `hidden_tiles` total as active teacher input.

Loader already has public row marginals:

* `remaining = extract_public_remaining_counts(...)`

It also already has public zone-size pieces:

* each opponent concealed size from `state.players[*].hand_len`
* wall remainder from `state.wall.remaining()`

So loader should compute exact zone sizes in canonical zone order and pass those to stronger teacher provider instead of collapsing them to one total.

Do **not** restate zone order from memory. Pin by test. Artifact packet does not show full table, and this exactly kind of thing that should be frozen by asymmetric unit test, not verbal recall.
...
**Critical v1 rule:** if stronger teacher not available, return `(None, None, false, false)`.
Do not silently fall back to Stage-A. Matches doctrine to leave unavailable targets absent rather than fabricating weak labels (Artifact 19 RECON L0445-L0455).

**Exact v1 return wanted when belief available but mixture stays off:**
...
That asymmetry already supported by tuple and downstream collation.

## 3. Loss phase: repair loss/object pairing before any activation

### `crates/hydra-train/src/training/losses.rs`

Second must-change file.

**Direct repo fact:** current belief loss is BCE-with-logits over `[B,16,34]` (Artifact 12 LOSS L0276-L0289; Artifact 13 LOSS L0594-L0600). Archive already flags this as mismatch for transport-style mass tables or row-normalized posterior conditionals (Artifact 23 A15 L0331-L0332).

**Recommended v1 loss:**
Use rowwise soft cross-entropy / KL over 4 zones for each tile, not BCE.

Why smallest viable repair:

* head already outputs raw logits
* rowwise posterior targets are probabilities over 4 zones
* no new output activation needed
* no new batch mask tensor needed

Implement by reshaping `[B,16,34] -> [B,4,4,34]`, then for each component/tile row:

* compute `row_mass = sum_z target`
* if `row_mass == 0`, ignore row
* else normalize target row and apply soft cross-entropy against 4 logits
...
where `V_b` = set of valid tile rows for sample `b`.

This keeps existing sample-level `belief_fields_mask: [B]` exact; per-row validity mask derived from target inside loss.

**Why no new mask carrier needed:**
Only rows needing masking are `r_t(k)=0` rows, and those visible directly from normalized target row sum. So handle inside `belief_fields_*_per_sample()`.
...
Small but important detail: mean over valid rows and components so duplication across four component blocks does **not** multiply sample loss by 4.

## 4. Sample/collation phase: keep contracts, add assertions

### `crates/hydra-train/src/data/sample.rs`

Avoid contract changes here. Current structures already enough (Artifacts 35 and 36).
...
**One narrow safety improvement to add before activation:**
assert target/presence consistency for belief and mixture, same way exit and delta-Q already panic on target/mask mismatch.
...
Reason: today bad producer could copy belief target into batch while leaving sample mask at zero, and loss would silently skip it. Doctrine explicitly says target presence should govern loss existence and plumbing should not silently disappear (Artifact 19 RECON L0463-L0469).
...
`crates/hydra-train/src/bin/train/loss_policy.rs` should stay unchanged until new teacher encoding, loss, and tests land. Current rejection of `advanced_loss.belief_fields` and `advanced_loss.mixture_weight` deliberate and tested (Artifacts 14, 15, 37).

This preserves today’s train-bin safety.

### Phase 1: unlock belief only, still default-off

After substrate tests pass, do one narrow policy move:

* in `build_loss_config(...)`, allow `belief_fields`
* continue rejecting `mixture_weight`
* continue rejecting `opponent_hand_type`
* leave RL-side `build_rl_loss_config(...)` unchanged in v1 to avoid tranche widen

That makes first activation BC/train-only and opt-in.

I would not make belief default-nonzero in same PR. Keep default at `None` or `0.0`; require explicit config for pilot runs.
...
Only after Phase 1, run narrow BC pilot with:

* belief target present
* mixture absent
* small auxiliary weight

Only numeric prior I see in repo docs is older opponent-modeling suggestion of `0.02` for Sinkhorn-style belief supervision (Artifact 20 OPPMODEL L0676-L0679). I would treat `0.02` as reasonable **starting** belief weight for pilot runs, not doctrinal constant.

## 6. Why `belief_fields` can move before `mixture_weight`

Yes, can, and repo already supports asymmetry.
...
1. No canonical cross-sample component identity closed in current repo doctrine or code (Artifacts 6, 22, 23).
2. Runtime bridge code explicitly re-ranks components by weight per sample before encoding (Artifact 27).
3. Stage-A components identical anyway, so component axis not informative teacher surface today (Artifacts 9 and 30).

So v1 gate for mixture should be two-layered:

* producer gate: emit `mixture_weights = None`, `mixture_weights_present = false`
* train gate: keep `reject_blocked_advanced_loss_presence("mixture_weight", ...)` unchanged

That narrowest safe gate.

External literature also supports split. Sinkhorn-style OT is about regularized transport/projection object computed by matrix scaling, not about magically giving canonical latent component identity; if later you want stable component alignment, that becomes separate assignment/permutation problem, exactly kind of problem adjacent Sinkhorn work treats explicitly rather than implicitly. ([NeurIPS Proceedings][1])
...
* no Stage-A fallback when stronger labels absent
...
* any belief-side search/ponder integration beyond existing CT-SMC/search seams

If later you need canonical mixture identity, treat as separate lane. Adjacent literature frames that as latent alignment/permutation problem, not something to smuggle into weak label surface. ([OpenReview][2])

## 8. What is safe to implement now even before Agent A finishes

These changes robust no matter which stronger teacher object Agent chooses:
...
4. add new belief-only integration test in `integration_pipeline.rs`
5. stop planning any `mixture_weight` activation in this tranche

Only part depending on Agent = actual provider of `34×4` public posterior counts or row distributions:

* if reconstructible locally, wire in `teacher/belief.rs` and call from `mjai_loader.rs`
* if it needs replay/search provenance, materialize as sidecar and still terminate in same existing `[16*34]` carrier

That stays aligned with repo’s sidecar-first pattern for other advanced labels and avoids batch-surface widen (README status note; Artifact 19 RECON).

## Confidence

High confidence:

* no new heads or carrier shapes needed
...
* best smallest path = repeated aggregate posterior across four component blocks
* rowwise soft CE/KL = right v1 loss repair

Blocked on Agent A’s output:

* whether stronger public teacher locally reconstructible in loader code or should come from search/CT-SMC sidecar

SMC side also on solid ground: ESS-based gating/resampling with particle-count threshold standard, with `N/2` common heuristic, so Hydra’s existing `0.4 * P` CT-SMC threshold already in normal range and needs no new belief-specific retune in this tranche. ([Oxford Statistics][3])
