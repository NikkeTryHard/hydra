<combined_run_record run_id="prompt-5" variant_id="prompt_pack" schema_version="1">
  <metadata>
    <notes>Prompt-pack file preserved as a single combined context record.</notes>
    <layout>single_markdown_file_prompt_then_answer</layout>
  </metadata>

  <prompt_section>
  <prompt_text status="preserved" source_path="prompt-5.md" kind="multi_variant_prompt_pack">
  <![CDATA[# Hydra long-think prompt pack 5 — cross-field follow-up and implementation expedition
  ...
  <answer_section>
  <answer_text status="hand_written_context_note" source_path="assistant_generated_context_note">
  <![CDATA[This source file is a multi-variant prompt pack, not a single answer-bearing run artifact. It is included here in combined form only because you asked for one-folder instant-context LLM files.
  ]]>
  </answer_text>
  </answer_section>
</combined_run_record>
