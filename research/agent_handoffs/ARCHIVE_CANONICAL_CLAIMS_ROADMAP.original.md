...
- Completed (supporting BC runtime contract): learner BC now keeps fresh runs config-derived, preserves strict partial-epoch runtime identity, and may reuse matching preflight-selected runtime for the selected-runtime tuple at epoch boundaries only; loader-runtime remains config-derived.
...
|---|---|---|---|---|---|
| 1 | Narrow `safety_residual` / BC activation sublane | Active-path doctrine | Completed | Reconciliation says the immediate need is supervision-loop closure, not broader search expansion. The completed portion is the replay-derived `safety_residual` BC lane plus hardware-agnostic BC microbatch accumulation, and current BC runtime authority now keeps fresh runs config-derived while allowing epoch-boundary-only reuse of matching preflight-selected runtime for the selected-runtime tuple. Broader Recommendation 1 closure remains open. | `HYDRA_RECONCILIATION.md` Recommendation 1; canonical rows 24, 34, 35, 55 |
