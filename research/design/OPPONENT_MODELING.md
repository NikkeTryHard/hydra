...
> |------|---------|---------|-------|
> | **Formula A** (RCPO training) | `A_combined = (A^R(s,a) - λ × A^C(s,a)) / (1 + λ)` | Older training-plan context | Historical Hydra planning |
...
> The PID-Lagrangian controller details referenced in older Hydra planning are reserve-only context. The active doctrine should come from `HYDRA_RECONCILIATION.md`, not from the missing historical training-plan doc.
...
Wait-Set predicts the opponent's waiting tiles regardless of whether the player discards them — it models the opponent's hand structure. Danger predicts the consequence of a specific discard action. In older Hydra planning, dense danger labels shared infrastructure with wait-set labels; keep that as rationale context rather than current hidden authority.
...
> The older teacher-student architecture notes referenced here are reserve/historical context. For the current active path, defer to `HYDRA_RECONCILIATION.md` and the live `hydra-train` code.

The teacher sees everything (opponent hands, wall composition, ura-dora) and is expected to converge significantly faster than a blind agent (Suphx reports "much faster" convergence qualitatively but provides no quantitative ratio; arXiv:2003.13590, Section 3.3). Older Hydra planning paired this with a KL-style student objective; keep that as reserve-stage rationale, not as a statement of the current active tranche.
...
> Historical feature-dropout schedules belong to reserve-stage planning only. Do not treat missing old training-plan links as live authority.
...
**Computational cost:** 10-30 iterations of matrix-vector multiply on a [34 x 4] matrix. Microseconds per forward pass, negligible relative to Hydra's backbone inference budget.
...
> Older milestone and transition-gate details belong to reserve planning. Use `HYDRA_RECONCILIATION.md` for the active path.
