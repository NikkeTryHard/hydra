...
- completed recently as the next shipped DeltaQ follow-up tranche: the lane is now promotion-gated through an implemented arena-confirmation path rather than only structurally validated offline. Hydra has a dedicated `--delta-q-promotion` mode, persisted promotion artifacts, teacher-regret / policy-transfer reports, explicit stage-aware recommendation semantics, a paired arena executor, and arena helper/config/report objects under matched seeds / fixed seat rotation / matched temperature / frozen baseline opponents.
- current DeltaQ status: the arena-confirmation path now populates the promotion artifact with an explicit `arena_decision` plus `arena_report`, while the pre-arena `recommendation` field remains the truthful offline/policy-transfer next-step signal. The lane is still not default-on because no later doctrine change has yet promoted it to the live default path.
