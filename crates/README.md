# hydra-bridge — wall-less replay driver (slice 4)
...
```sh
`cargo run -p hydra-bridge --bin hydra2_replay_dump -- \
...
Same run: `cargo nextest run --manifest-path crates/Cargo.toml`
...
`crates/bridge/src/stream.rs` exposes `PyHydra2ReplayStream{inner:
Mutex<Option<ReplayStream>>}` (`import hydra2._native`; PyO3 0.28.3,
...
buffer-too-small, open-args, artifact-root gates) and the bridge
integration tests (`crates/bridge/tests/fill_bridge.rs`, `s6_sink.rs`).
