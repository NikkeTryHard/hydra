...
use hydra_feed::fill::{
    FillOut, N_PLANES, Scratch, StageJob, inject_wall, pick_t_len, row_bytes, stage_one,
};
use hydra_feed::gate::reason_name as gate_reason_name;
use hydra_feed::ledger::{PLANE_NAMES, walk_reason_name};
use pyo3::prelude::*;
use pyo3::types::{PyByteArray, PyDict};

/// One staged game result: `(dict, rows, t_len, quarantined, reason, event_idx)`.
type StagedGameOut = (Py<PyDict>, u32, u32, bool, String, u32);
...
fn run_planes(py: Python<'_>, events: Vec<u8>, game_idx: u32) -> PyResult<StagedGameOut> {
...
/// Boxed plane buffers keep the enum small (`Rows` would otherwise dwarf
/// `Quarantined` by 600+ bytes).
enum Staged {
    Rows {
        bufs: Box<[Vec<u8>; N_PLANES]>,
        rows: u32,
        t_len: u32,
    },
    Quarantined {
...
fn stage_game(events: Vec<u8>, game_idx: u32) -> Result<Staged, String> {
    ...
    let t_len: u32 =
        u32::try_from(out.t_len).map_err(|_| format!("replay t_len {} outside u32", out.t_len))?;
    Ok(Staged::Rows {
        bufs: Box::new(bufs),
        rows: out.rows,
        t_len,
...
fn staged_to_py(py: Python<'_>, staged: Result<Staged, String>) -> PyResult<StagedGameOut> {
    match staged {
...
mod replay_tests {
    ...
    fn f1_text() -> Vec<u8> {
        ...
        for (k, pai) in ["5pr", "5p", "5p", "5p", "6p"].iter().enumerate() {
            // proof: `k` in 0..5 (test loop), so `k%4` in 0..4, fits `u8`.
            #[allow(clippy::cast_possible_truncation)]
            let actor: u8 = (k % 4) as u8;
    ...
    fn stage_one_golden_ok() {
        ...
        let chosen: Vec<i64> = (0..5)
            .map(|r| {
                // proof: test `r` in 0..5, fits `usize`.
                #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
                let r_u: usize = r as usize;
                let o = r_u * 8;
