...
use hydra_feed::stream_driver::{
    COMPACT_ROWS, DriverEntry, DriverError, DriverSnapshot, DriverStats, MAX_BATCH_GAMES,
    MAX_TAKE_ROWS, MicrobatchOut, PushOut, StreamDriver,
};
...
impl PyStreamDriver {
    ...
    fn open(
        ...
        let driver = py
            .detach(|| StreamDriver::open(seed, threads, compact))
    ...
    fn push_games(&self, py: Python<'_>, games: Vec<DriverGame>) -> PyResult<PyPushOut> {
        ...
        py.detach(|| {
            let jobs = games
                .into_iter()
                .map(
                    |(bytes, game_id, key, wall)| hydra_feed::stream_driver::DriverJob {
                        bytes,
                        game_id,
                        key,
                        wall,
