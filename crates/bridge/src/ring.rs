...
//!
//! Single-cdylib tree: this module registers as the `hydra2._native.ring`
//! submodule via `register`, mirroring `replay::register` /
//! `canon_rng::register` / `columnar::register` / `search::register`.
//!
...
fn quantile_sorted(sorted: &[f64], q: f64) -> Option<f64> {
    ...
    // proof: window len <= `TIMING_WINDOW` (small, < 2^53), exact; quantile tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let len_m1_f: f64 = (sorted.len() - 1) as f64;
    let pos = q * len_m1_f;
    // proof: `pos` in [0, len-1] (`q` in [0,1], checked above), fits `usize`.
    ...
    // proof: `lo` <= len-1 (< 2^53), exact.
    #[allow(clippy::cast_precision_loss)]
    let lo_f: f64 = lo as f64;
    let frac = pos - lo_f;
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FillError::NullPointer { plane } => {
                write!(
                    f,
                    "hydra2 ring fill got a null buffer pointer for plane {plane}"
                )
            }
...
unsafe fn copy_planes_raw(dst_ptrs: &[u64], src_ptrs: &[u64], byte_lens: &[usize]) {
...
/// Ring stats: `(depth, cursor, acquires, h2d_last, sync_last)`.
type RingStats = (usize, usize, u64, Option<f64>, Option<f64>);
...
impl PyRing {
    fn lock<'a>(&'a self, py: Python<'a>) -> Result<std::sync::MutexGuard<'a, RingInner>, PyErr> {
        self.inner
            .lock_py_attached(py)
...
impl PyRing {
    ...
    fn record_h2d(&self, py: Python<'_>, ms: f64) -> PyResult<()> {
        if !ms.is_finite() || ms < 0.0 {
            return Err(PyValueError::new_err(format!(
                "ring h2d_ms {ms} not finite/nonneg"
            )));
        }
        let mut guard = self.lock(py)?;
    ...
    fn record_sync(&self, py: Python<'_>, ms: f64) -> PyResult<()> {
        if !ms.is_finite() || ms < 0.0 {
            return Err(PyValueError::new_err(format!(
                "ring sync_ms {ms} not finite/nonneg"
            )));
        }
        let mut guard = self.lock(py)?;
    ...
    fn stats(&self, py: Python<'_>) -> PyResult<RingStats> {
        let guard = self.lock(py)?;
        Ok((
            guard.depth,
            guard.cursor,
...
fn ring_fill_batch(
    ...
    .map_err(FillError::Geometry)
    .map_err(fill_py_err)?;
    let total = check_descriptors(dst_ptrs.len(), &dst_ptrs, &byte_caps, &src_ptrs, &byte_lens)
        .map_err(fill_py_err)?;
