...
//!
//! Single-cdylib tree: this module registers as the `hydra2._native.tiles`
//! submodule via `register`, mirroring `packet::register`.
...
fn mjai_string_of(py: Python<'_>, tile: i64) -> PyResult<String> {
    ...
    // proof: `tile` in 0..=135 (range-checked above), fits `u8`.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let id: u8 = tile as u8;
    ...
    py.detach(|| -> Result<Vec<i32>, String> {
        ...
        for tile in ids {
            ...
            out.push(if seen < pool.len() {
                ...
                // proof: `tile` in 0..=135 (range-checked above), fits `i32`.
