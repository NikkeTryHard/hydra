...
//! Single-cdylib tree: this module registers as the `hydra2._native.canon_rng`
...
#[pyfunction]
fn philox_block(py: Python<'_>, spec: &StreamSpec, block: u64) -> PyResult<(u32, u32, u32, u32)> {
...
fn bounded_batch(py: Python<'_>, words: Vec<u32>, n: u32) -> PyResult<Vec<u32>> {
    if n == 0 {
        return Err(PyValueError::new_err("canon bounded_batch: n must be >= 1"));
    }
    Ok(py.detach(|| {
        words
            .into_iter()
...
mod canon_rng_tests {
    ...
    fn hello_golden_single_and_batch_agree() {
        ...
        let items = [b"hello world".to_vec(), b"hello world".to_vec()];
    ...
    fn fixed_ranks_reject_tie_gap_oob() {
        use hydra_feed::fixed::{FixedError, validate_ranks};
    ...
    fn fixed_canonical_points_and_zero_sum() {
        use hydra_feed::fixed::{FIXED_SCALE, exact_total_fixed, utility_fixed};
    ...
    fn fixed_manifest_indexing_and_zero_sum_shapes() {
        ...
        assert!(exact_total_is_zero(&[1.0, -1.0, 2.0, -2.0]).unwrap());
        assert!(!exact_total_is_zero(&[1.5, 2.5, -1.0, -2.0]).unwrap());
