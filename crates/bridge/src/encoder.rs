...
//!   closed on drift).
//!
//! Single-cdylib tree: registers as the `hydra2._native.encoder` submodule
//! via `register`, mirroring `ring::register`.
    ...
    obs.getattr(field)
        ...
        .map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .{field} must be sequence[int]"
            ))
        })
}
...
fn getattr_vec_bool(obs: &Bound<'_, PyAny>, field: &str, row: usize) -> Result<Vec<bool>, PyErr> {
    obs.getattr(field)
        ...
        .map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .{field} must be sequence[bool]"
            ))
        })
}

fn getattr_vec_string(
    obs: &Bound<'_, PyAny>,
    field: &str,
    row: usize,
    ...
    obs.getattr(field)
        ...
        .map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .{field} must be sequence[str]"
            ))
        })
}
...
fn extract_row(obs: &Bound<'_, PyAny>, row: usize) -> Result<StagedRow, PyErr> {
    ...
    let furiten = furiten_id(furiten_raw.as_str()).map_err(PyValueError::new_err)?;
        ...
        match scores.get_mut(i) {
            Some(slot) => {
                *slot = i32::try_from(*value).map_err(|_| {
                    PyValueError::new_err(format!(
                        "encoder stage row {row}: .scores[{i}] value {value} outside i32"
                    ))
                })?;
            }
            None => {
    ...
    let drawn_value = obs.getattr("own_drawn_tile").map_err(|e| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .own_drawn_tile unreadable: {e}"
        ))
    })?;
    let own_drawn: i32 = if drawn_value.is_none() {
        ...
        let tile: i64 = drawn_value.extract().map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .own_drawn_tile must be int | None"
            ))
        })?;
        if !(0..136).contains(&tile) {
            return Err(PyValueError::new_err(format!(
                "encoder stage row {row}: drawn tile {tile} out of range 0..135"
            )));
        }
        // proof: `tile` in 0..136 (range-checked above), fits `i32`.
        #[allow(clippy::cast_possible_truncation)]
        let tile_i: i32 = tile as i32;
        tile_i
    };
    ...
    for (i, value) in dora_raw.iter().enumerate() {
        match dora.get_mut(i) {
            Some(slot) => {
                ...
                // proof: `*value` is -1 or in 0..136 (checked above), fits `i32`.
                #[allow(clippy::cast_possible_truncation)]
                let dora_i: i32 = *value as i32;
                *slot = dora_i;
            }
            None => {
                return Err(PyValueError::new_err(format!(
    ...
    let rivers_value = obs.getattr("visible_discards").map_err(|e| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .visible_discards unreadable: {e}"
        ))
    })?;
    ...
    let melds_value = obs.getattr("visible_melds").map_err(|e| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .visible_melds unreadable: {e}"
        ))
    })?;
    ...
    let history_value = obs.getattr("visible_history").map_err(|e| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .visible_history unreadable: {e}"
        ))
    })?;
    ...
    for event in events.iter() {
        let kind_value = event.getattr("kind").map_err(|e| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: event.kind unreadable: {e}"
            ))
        })?;
    ...
    let hash: String = hash_value.extract().map_err(|_| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .observation_hash must be str"
        ))
    })?;
    let hand_number: i32 = i32::try_from(hand_number_raw).map_err(|_| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .hand_number value {hand_number_raw} outside i32"
        ))
    })?;
    let round_index: i32 = i32::try_from(round_index_raw).map_err(|_| {
        PyValueError::new_err(format!(
            "encoder stage row {row}: .round_index value {round_index_raw} outside i32"
        ))
    })?;
    Ok(StagedRow {
        actor,
        ...
        round_wind,
        seat_winds,
        honba: i32::try_from(honba_raw).map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .honba value {honba_raw} outside i32"
            ))
        })?,
        riichi_sticks: i32::try_from(riichi_sticks_raw).map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .riichi_sticks value {riichi_sticks_raw} outside i32"
            ))
        })?,
        scores,
        phase,
        live_wall: i32::try_from(live_wall_raw).map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .live_wall_tiles_remaining value {live_wall_raw} outside i32"
            ))
        })?,
        kan_count: i32::try_from(kan_raw).map_err(|_| {
            PyValueError::new_err(format!(
                "encoder stage row {row}: .kan_count value {kan_raw} outside i32"
            ))
        })?,
        ippatsu,
        riichi,
...
fn fill_buffers(
    ...
    for (idx, row) in rows.iter().enumerate() {
        ...
        for j in 0..4 {
            let base = idx.saturating_mul(4).saturating_add(j).saturating_mul(8);
        ...
        for j in 0..DORA_WIDTH {
            ...
            write_i32_le(
                dora,
                idx.saturating_mul(5).saturating_add(j).saturating_mul(4),
                dvalue,
            )?;
        }
        ...
        for tile in row.concealed.iter() {
            // proof: staged 0..135 (checked at extract), non-negative, fits `usize`.
            #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
            let tile_u: usize = *tile as usize;
            let kind = tile_u / 4;
            match concealed_counts.get_mut(kind) {
                ...
                None => {
                    return Err(format!(
                        "encoder fill row {idx}: concealed kind {kind} out of range"
                    ));
                }
            }
        }
        ...
        for tile in row.discards.iter().chain(row.meld_tiles.iter()) {
            ...
            match visible_counts.get_mut(kind) {
                ...
                None => {
                    return Err(format!(
                        "encoder fill row {idx}: visible kind {kind} out of range"
                    ));
                }
            }
        }
        ...
        for (pos, kind) in row.history.iter().enumerate() {
            ...
            write_bool_byte(
                history_mask,
                idx.saturating_mul(bucket_t).saturating_add(pos),
                true,
            )?;
        }
        ...
        for (j, flag) in row.legal.iter().enumerate() {
            write_bool_byte(
                legal,
                idx.saturating_mul(BASELINE_ACTIONS).saturating_add(j),
                *flag,
            )?;
        }
    }
