...
//!
//! Single-cdylib tree: registers as the `hydra2._native.mirror` submodule
//! via `register`, mirroring `canon_rng::register`.
...
fn check_segment(label: &str, value: &str, max_len: usize) -> Result<(), String> {
    ...
    if bad {
        return Err(format!(
            "mirror {label} must not contain / ? # or whitespace"
        ));
    }
    Ok(())
}
...
fn check_json_body(label: &str, body: &str) -> Result<(), String> {
    if body.is_empty() || body.len() > MAX_BODY_LEN {
        return Err(format!(
            "mirror {label} body must be 1..={MAX_BODY_LEN} bytes"
        ));
    }
