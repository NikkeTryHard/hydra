...
use _native::stream::{FeedOpen, FeedStream, StreamError};
...
const MAX_STRIDE: [usize; 26] = [
    34, 34, 20, 16, 2048, 256, 8, 8, 8, 8, 8, 32, 136, 8, 8, 4, 4, 4, 4, 4, 4, 4, 4, 32, 1, 1,
];
...
fn bridge_small_buffer_drains_nothing_and_reports_plane() {
    ...
    assert_eq!(
        stats.games_quarantined, 0,
        "games_quarantined moved on failed fill"
    );
    assert!(
        stream.quarantines().is_empty(),
        "quarantines listed on failed fill"
    );
    ...
    assert_eq!(
        fill.rows, fresh_fill.rows,
        "retry diverged: failed call drained"
    );
    assert_eq!(fill.t_len, fresh_fill.t_len);
    for i in 0..13 {
        assert_eq!(
            retry.bufs[i], fresh_caller.bufs[i],
            "plane {i} diverged after rollback"
        );
    }
}
...
fn bridge_discovers_mjai_suffixes_with_stemmed_ids() {
    ...
    assert_eq!(
        stats.games_quarantined, 1,
        "exactly the bad mjai file: {stats:?}"
    );
    let quars = stream.quarantines();
    assert_eq!(quars.len(), 1);
    assert_eq!(
        quars[0].game_id, "broken-game",
        "stem must strip .mjai.json.zst"
    );
...
fn bridge_multi_game_pack_keys_subgames_without_collision() {
    ...
    assert_eq!(
        stats.games_quarantined, 1,
        "one quarantined sub-game: {stats:?}"
    );
    let quars = stream.quarantines();
    assert_eq!(quars.len(), 1);
    assert_eq!(
        quars[0].game_id, "pack-3",
        "stem shared, detail carries sub-game idx"
    );
    assert_eq!(quars[0].reason_code, "framing");
    let _ = std::fs::remove_dir_all(&dir);
}
