...
//!   [`check_accounting`](_native::sink::check_accounting) assert
...
use _native::sink::{
    QuarantineLineage, check_accounting, join_lineage, quarantine_reason_name, read_lineage_file,
    sink_bucket, write_lineage_file,
};
use _native::stream::{FeedOpen, FeedStream};
...
fn s6_post_claim_draw_quarantines_end_to_end() {
    ...
    let path = dir.join("quarantine.lineage");
    write_lineage_file(&path, std::slice::from_ref(&record)).expect("lineage write");
    let back = read_lineage_file(&path).expect("lineage read");
