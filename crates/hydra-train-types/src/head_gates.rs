...
mod tests {
    ...
    fn coverage_partial_mask() {
        ...
        p.counts[AdvancedHead::BeliefFields.index()] = 60;
        cov.record_batch(&p);
    ...
    fn conflict_tracks_negative_fraction() {
        ...
        tracker.record(head, 0.5);
        tracker.record(head, -0.3);
        tracker.record(head, 0.1);
        tracker.record(head, -0.2);
        assert!((tracker.negative_fraction(head) - 0.5).abs() < 1e-6);
        assert_eq!(tracker.total_checks(head), 4);
        assert!(tracker.is_conflicting(head, 0.3));
        assert!(!tracker.is_conflicting(head, 0.6));
    }
    ...
    fn controller_blocks_activation_low_density() {
        let mut ctrl = HeadActivationController::new(test_config());
        fill_density(&mut ctrl, AdvancedHead::SafetyResidual, 0.5);
    ...
    fn controller_sparse_head_spp_gate() {
        ...
        for _ in 0..100 {
            ...
            p.counts[AdvancedHead::DeltaQ.index()] = 1;
            ctrl.record_batch(&p);
        }
