...
#![allow(dead_code)]
// Test helpers assert through `expect`/`panic` by design (see clippy.toml:
// "Tests keep unwrap/expect/panic/prints"); `allow-*-in-tests` only covers
// `#[test]` bodies, not the shared helpers below.
