...
impl MemoryLimiter {
    ...
    fn acquire(self: &Arc<Self>, bytes: u64) -> MemoryPermit {
        ...
        // counters stay usable, so recover them instead of panicking.
        let mut used = self
            .used
            .lock()
...
fn plan_file(
    ...
    if is_raw_mjai_name(name) {
        ...
        let archive_stem = name.strip_suffix(".tar.zst").context("suffix checked")?;
...
fn convert_inner(
    ...
    for (archive, archive_jobs) in archives {
        *progress
            .archive
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner()) = archive
            .file_name()
            .unwrap_or_default()
            .to_string_lossy()
            .into_owned();
...
fn spawn_progress(
    ...
    std::thread::spawn(move || {
        ...
        // The template is a static literal, so this always parses; on the
        // impossible error path the bar keeps its default style.
...
fn convert_archive(
    ...
    let wanted: Vec<Job> = jobs.iter().map(|job| (*job).clone()).collect();
        ...
        let producer_result = (|| -> Result<()> {
            ...
            for entry in archive.entries()? {
                ...
                let expected = match job.source {
                    Source::TarZstd { size, .. } => size,
                    _ => bail!("archive worker received a non-archive job"),
                };
                ensure!(
                    entry.size() == expected,
                ...
                let capacity =
                    usize::try_from(expected).context("member size exceeds platform usize")?;
...
fn write_verified(
    ...
    let result = (|| -> Result<OutputFacts> {
        match (&job.source, member_data) {
            ...
            (_, Some(_)) => bail!("buffered payloads only occur inside archives"),
