...
impl PackagedObjectRow {
    ...
    pub fn canonical_bytes(&self, include_id: bool) -> Result<Vec<u8>> {
        ...
        if include_id {
            ...
            serde_jcs::to_vec(&full).context("seal doc serializes as JCS")
        } else {
            ...
            serde_jcs::to_vec(&seal).context("seal doc serializes as JCS")
        }
    }
    ...
    match value {
        ...
        Value::Number(number) => {
            if let Some(int) = number.as_i64() {
                ...
                // Parsed JSON numbers are always finite, so this is always
                // taken; the guard keeps an infallible printer panic-free.
                let mut buffer = ryu_js::Buffer::new();
                out.extend_from_slice(buffer.format_finite(float).as_bytes());
            }
        }
        Value::String(text) => {
            // Serializing a string into a `Vec` cannot fail.
...
pub fn publish_manifest(path: &Path, mut rows: Vec<PackagedObjectRow>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        for row in &rows {
            let sealed = row.canonical_bytes(true)?;
...
pub fn utc_timestamp(time: SystemTime) -> String {
    ...
    // `seconds` is a `u64`, so days <= u64::MAX / 86_400 << i64::MAX: the
    // fallback never fires; it only replaces a wrap with saturation.
...
fn civil_from_days(days_since_epoch: i64) -> (i64, u32, u32) {
    ...
    let day = u32::try_from(day_of_year - (153 * month_pointer + 2) / 5 + 1).unwrap_or(0);
    let month = u32::try_from(if month_pointer < 10 {
        month_pointer + 3
    } else {
        month_pointer - 9
    })
...
mod tests {
    ...
    #[test]
    fn row_seal_is_stable_and_sensitive_to_content() -> Result<()> {
    ...
    #[test]
    fn manifest_round_trips_through_load_and_publish() -> Result<()> {
