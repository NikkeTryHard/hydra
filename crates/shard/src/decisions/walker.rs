...
impl<'a> Walker<'a> {
    ...
    pub(crate) fn concealed_for_row(&self, seat: u8) -> Result<Vec<u8>, WalkReject> {
        ...
        if live
            && let Some(drawn) = self.reported_drawn(seat)?
                && let Some(pos) = hand.iter().position(|t| *t == drawn) {
                    hand.remove(pos);
                }
        Ok(hand)
    }
    ...
    pub(crate) fn seat_view(
        ...
        SeatView::new(crate::engine::SeatViewParams {
            game_id: &self.game.game_id,
            kyoku: self.kyoku_ordinal,
            step,
            seat,
            hand,
            drawn,
            drawn_true: live_take,
            melds: &track.melds[seat as usize],
            riichi: track.riichi_declared[seat as usize],
            score: self.scores[seat as usize],
            wall: self.live_wall(),
        })
    ...
    pub(crate) fn draw_mask_ids(
        ...
        if log_win
            && let ChosenAction::Tsumo { tile } = chosen
                && let Some(id) =
                    self.table
    ...
    pub(crate) fn peek_window_ron(&self, from_idx: usize, discarder: u8) -> bool {
        matches!(
            self.peek_window_claim(from_idx, discarder),
            Some(event) if event.type_.as_str() == "hora"
        )
    }
...
pub(crate) fn prescan(game: &ParsedGame) -> Result<(), WalkReject> {
    ...
    for event in &game.events {
        ...
        if kind == "hora" {
            ...
            if !tsumo_flag
                && let (Some(a), Some(t)) = (actor, target)
                    && a != t {
...
fn seat_value(value: &Option<serde_json::Value>) -> Option<u8> {
    match value {
        Some(serde_json::Value::Number(n)) => {
            let seat = n.as_u64()?;
            if seat <= 3 {
                Some(u8::try_from(seat).unwrap_or(u8::MAX))
            } else {
                None
            }
        }
