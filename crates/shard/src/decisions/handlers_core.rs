        ...
        match kind.as_str() {
            ...
            "reach" => {
                ...
                if walker.wall_digest.is_some()
                    && let Ok(actor) = game.events[idx].actor_seat("reach") {
            ...
            _ if is_row_kind(&kind) => {
                ...
                if walker.wall_digest.is_some()
                    && let Ok(actor) = game.events[idx].actor_seat(&kind) {
...
fn do_start_kyoku(walker: &mut Walker, idx: usize) -> Result<(), WalkReject> {
    ...
    // proof: serde-parity wrapping cast; oracle scores wrapped identically.
    ...
    // proof: kyoku ordinals are small game indices, fit usize.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
...
fn do_reach(walker: &mut Walker, reach_idx: usize, decl_idx: usize) -> Result<(), WalkReject> {
    ...
    // (ownership still enforced against the tracked hand; unowned fails closed).
...
mod walk_skeleton_tests {
    ...
    fn two_kyoku_text() -> String {
        ...
        let lines = vec![
            ...
            "{\"type\":\"hora\",\"actor\":2,\"target\":2,\"tsumo\":true,\"deltas\":[8000,-2000,-2000,-4000]}".to_string(),
    ...
    fn unknown_event_quarantines_both_paths() {
        let lines = ["{\"type\":\"start_game\"}".to_string(),
            start_kyoku(0, 1),
            format!(
