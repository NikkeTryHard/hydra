...
impl ChosenAction {
    ...
    pub fn lookup(&self, table: &ActionTable, seat: u8) -> Option<u32> {
        match self {
            ...
            ChosenAction::Chi { called, consumed } => {
                table.lookup("chi", None, Some(*called), consumed, Some(-1), false, false)
            }
            ChosenAction::Pon {
                called,
                consumed,
            ...
            | ChosenAction::Daiminkan {
                ...
                let offset = if delta == 3 { -1 } else { delta.cast_signed() };
                table.lookup(
                    self.kind_str(),
                    None,
                    Some(*called),
...
impl ReplayRow {
    ...
    pub fn to_decision_json(
        ...
        let actor_text = serde_json::to_string(&actor_doc)?;
        ...
        Ok(serde_json::Value::Object(decision))
    }
}
...
mod decision_json_tests {
    ...
    fn envelope_carries_exactly_actor_fields() {
        let decision = sample_row().to_decision_json(&provenance()).expect("decision encodes");
    ...
    fn hashes_bind_mark_and_wall_none() {
        let row = sample_row();
        let decision = row.to_decision_json(&provenance()).expect("decision encodes");
    ...
    fn resolved_chosen_emits_int_iff_unresolved_absent() {
        ...
        let decision = unresolved.to_decision_json(&provenance()).expect("decision encodes");
        assert!(decision.get("chosen_action_id").map(|v| v.is_null()).unwrap_or(false));
    }
...
pub fn ranks_from_final_scores(scores: &[serde_json::Value]) -> Result<[u8; 4], String> {
    ...
    order.sort_by(|&a, &b| vals[b].total_cmp(&vals[a]).then_with(|| a.cmp(&b)));
    let mut ranks = [0u8; 4];
    for (position, seat) in order.iter().enumerate() {
        ranks[*seat] =
            u8::try_from(position + 1).map_err(|_| "ranks position out of range".to_string())?;
    }
    Ok(ranks)
}
...
pub fn validate_privileged_ranks(
    ...
    for (i, rank) in ranks.iter().enumerate() {
        ...
        ordered[i] = u8::try_from(value)
            .map_err(|_| format!("privileged_label['ranks'] must be int 1..4{where_id}"))?;
    }
...
pub fn expand_privileged_rows(
    ...
    Ok((0..count)
        .map(|seq| PrivilegedRow {
            decision_id: decision_id(&game.game_id, u32::try_from(seq).unwrap_or(u32::MAX)),
            ranks,
