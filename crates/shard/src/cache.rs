...
impl MmapCache {
    ...
    pub fn open(path: &Path) -> Result<Self, CacheError> {
        ...
        let n = usize::try_from(u64::from_le_bytes(mmap[..8].try_into().map_err(|_| {
            CacheError::Header {
                msg: "header length prefix unreadable".to_string(),
            }
        })?))
        .map_err(|_| CacheError::Header {
            msg: "header length exceeds usize".to_string(),
        })?;
...
pub fn read_header_bytes(mapped: &[u8]) -> Result<Vec<u8>, CacheError> {
    ...
    let n = usize::try_from(u64::from_le_bytes(mapped[..8].try_into().map_err(
        |_| CacheError::Header {
            msg: "header length prefix unreadable".to_string(),
        },
    )?))
