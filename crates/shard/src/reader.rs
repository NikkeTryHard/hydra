...
pub fn encode_header(t_bucket: usize, rows: u64, row_bytes: usize, digest: &[u8; 32]) -> [u8; HEADER_LEN] {
    ...
    h[10..12].copy_from_slice(&u16::try_from(t_bucket).unwrap_or(u16::MAX).to_le_bytes());
    // h[12..16]: flags + reserved (zero).
    h[16..24].copy_from_slice(&rows.to_le_bytes());
    h[24..28].copy_from_slice(&u32::try_from(row_bytes).unwrap_or(u32::MAX).to_le_bytes());
...
impl ShardReader {
    ...
    pub fn open(path: &Path) -> Result<Self, ShardReadError> {
        ...
        if mmap.len()
            != usize::try_from(file_len).map_err(|_| ShardReadError::LengthMismatch {
                expected: file_len,
                got: mmap.len() as u64,
            })?
        {
        ...
        let payload = &mmap[HEADER_LEN
            ..HEADER_LEN
                + usize::try_from(header.rows).map_err(|_| ShardReadError::LengthMismatch {
    ...
    pub fn row(&self, index: u64) -> Result<&[u8], ShardReadError> {
        ...
        let base = HEADER_LEN
            + usize::try_from(index).map_err(|_| ShardReadError::Range {
                index,
                rows: self.header.rows,
            })? * self.header.row_bytes;
    ...
    pub fn payload(&self) -> &[u8] {
        let len = usize::try_from(self.header.rows).unwrap_or(0) * self.header.row_bytes;
...
mod ipc_probes {
    ...
    fn probe_ipc_mmap_round_trip() {
        ...
        let rows = write_ipc_file(&path, &schema, std::slice::from_ref(&batch)).unwrap();
        assert_eq!(rows, 2);
