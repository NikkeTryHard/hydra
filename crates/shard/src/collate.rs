...
impl CollateScratch {
    ...
    pub fn collate_into(&mut self, reader: &ShardReader) -> Result<(), Full26Error> {
        ...
        // proof: in-memory batches hold < u32::MAX rows, fitting usize/u32 on all targets.
        #[allow(clippy::cast_possible_truncation)]
        let rows_usize = rows as usize;
        // proof: in-memory batches hold < u32::MAX rows.
        #[allow(clippy::cast_possible_truncation)]
        let rows_u32 = rows as u32;
        let stride = compact_row_bytes(t);
        let mut i = 0usize;
        while i < 26 {
            self.batch.planes[i].reserve(rows_usize * full26_plane_row_bytes(i, t));
            i += 1;
        }
        self.batch.label_chosen.reserve(rows_usize * 8);
        self.batch.keys_game.reserve(rows_usize);
        self.batch.keys_seq.reserve(rows_usize);
...
pub fn verify_hot_subset(
    ...
    let hot_row = |plane: usize, r: u32| -> Result<&[u8], G2Mismatch> {
        let s = stride(plane);
        let ru = r as usize;
        hot_filled[plane].get(ru * s..(ru + 1) * s).ok_or(G2Mismatch {
            hot_plane: plane,
            row: r,
            what: "hot fill footprint short",
        })
    };
    let coll_row = |field: &str, r: u32| -> Result<&[u8], G2Mismatch> {
        let p = f26(field)?;
        let s = full26_plane_row_bytes(p, t_len);
        let ru = r as usize;
        coll.planes[p].get(ru * s..(ru + 1) * s).ok_or(G2Mismatch {
            hot_plane: usize::MAX,
            row: r,
    ...
    while r < rows {
        ...
        // Label: hot chosen vs collated label.
        {
            ...
            let ru = r as usize;
            let got = coll.label_chosen.get(ru * 8..(ru + 1) * 8).ok_or(G2Mismatch {
        ...
        // Legal: every hot id set in the full mask; len <= popcount.
        {
            ...
            let mut k = 0i64;
            while k < len {
                // proof: k in 0..32 by loop bound; legal ids are non-negative table indices.
                #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
                let id =
                    i32::from_le_bytes(h[k as usize * 4..k as usize * 4 + 4].try_into().map_err(
                        |_| G2Mismatch {
                            hot_plane: PLANE_LEGAL,
                            row: r,
