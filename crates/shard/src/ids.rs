...
pub fn decision_ids_for_game(game_id: &str, n: u32) -> Vec<String> {
    let mut out = Vec::with_capacity(n as usize);
    let mut seq = 0u32;
    while seq < n {
        out.push(decision_id(game_id, seq));
        seq += 1;
    }
    out
}

pub fn round_ids_for_game(game_id: &str, n: u32) -> Vec<String> {
    let mut out = Vec::with_capacity(n as usize);
    let mut idx = 0u32;
    while idx < n {
        out.push(round_id(game_id, idx));
        idx += 1;
    }
    out
}
