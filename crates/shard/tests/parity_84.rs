#![allow(clippy::expect_used, clippy::unwrap_used, clippy::print_stderr)] // comparison harness: expects + mismatch diagnostics are the test idiom
...
fn parity_84_probe() {
    ...
    let id2template: HashMap<u32, String> = {
        ...
        doc.pointer("/payload/actions")
            ...
            .iter()
            .enumerate()
            .map(|(id, entry)| (u32::try_from(id).unwrap(), entry.to_string()))
            .collect()
    };
...
fn id_strings_in_order(ids: &[serde_json::Value]) -> Vec<String> {
    ...
    ids.iter()
        ...
        .filter_map(|id| mjai_string_of(u8::try_from(id).unwrap()).ok())
        .collect()
}
...
fn classify(
    ...
    let oracle_kind = oracle_chosen
        .and_then(|a| id2kind.get(&u32::try_from(a).unwrap()))
        .cloned()
        .unwrap_or_else(|| "?".to_string());
    ...
    let drawn_equal = match (rust_drawn, oracle_drawn) {
        (Some(serde_json::Value::String(r)), Some(serde_json::Value::Number(o))) => {
            o.as_u64()
                .and_then(|id| mjai_string_of(u8::try_from(id).unwrap()).ok())
                .as_deref()
                == Some(r.as_str())
        }
    ...
    if drawn_equal {
        ...
        classes.unconcealed_samples.push(format!(
            "{id} drawn rust {rust_drawn:?} oracle {oracle_drawn:?}"
        ));
    }
    let rust_dora = rust
        .get("dora_indicators")
            ...
            if kind == "discard" {
                ...
                if classes.extra_discard_samples.len() < 8 {
                    let tile = id2template
                        ...
                        .unwrap_or_else(|| "?".to_string());
                    classes
                        .extra_discard_samples
                        .push(format!("{id} postclaim={postclaim} {tile}"));
                }
            }
        }
    ...
    for (file, round_idx) in &kyokus {
        ...
        for (pos, event) in slice.iter().enumerate() {
            ...
            match kind {
                        ...
                        while look < slice.len() {
                            ...
                            if matches!(next_kind, "chi" | "pon" | "daiminkan") {
                                let target = slice[look]
                                    ...
                                    .and_then(|v| v.as_u64())
                                    .unwrap_or(9);
                                if usize::try_from(target).unwrap() == seat {
                                    claim_follows += 1;
                                }
                            }
                            break;
                ...
                "kakan" => {
                    if let Some(seat) = seat_of(event)
                        && let Some(pai) = event.get("pai").and_then(|v| v.as_str())
                            && let Some(count) = hands[seat].get_mut(&norm_mjai(pai)) {
                                *count = count.saturating_sub(1);
                            }
                }
                _ => {}
