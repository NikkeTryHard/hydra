#![allow(clippy::print_stdout, clippy::print_stderr)] // bench harness binary: stdout/stderr reports are its interface
...
/// u64→f64 for bench metrics (all counts/durations <2^53, exact).
#[inline]
fn u64_to_f64(v: u64) -> f64 {
    // proof: bench counts/durations <2^53, exactly representable in f64.
    #[allow(clippy::cast_precision_loss)]
    let f = v as f64;
    f
}

/// u128 nanos→u64 for bench durations (all runs <2^64 ns).
#[inline]
fn nanos_to_u64(v: u128) -> u64 {
    // proof: bench durations are seconds-scale, far below 2^64 ns.
...
fn rate_per_s(numer: f64, wall_ns: u64) -> f64 {
    ...
    numer * 1_000_000_000.0 / u64_to_f64(wall_ns)
}
...
fn print_report(
    ...
    println!(
        "wall_path: walled-mode (static marker digest; digest content unread hot; cold sha outside feed wall per feed no-sha rule)"
    );
    println!(
        "history: hot-13 planes, T buckets [32,64,128,256]; fill buckets staged t_len per pass"
    );
    println!("--- per-run (wall covers zstd+frame+gate+walk+commit+fill) ---");
    for (k, p) in passes.iter().enumerate() {
        let rora = rate_per_s(u64_to_f64(cfg.expect_rows as u64), p.wall_ns);
        let rnat = rate_per_s(u64_to_f64(p.staged_rows as u64), p.wall_ns);
        println!(
            ...
            k,
            (u64_to_f64(p.wall_ns)) / 1_000_000.0,
            p.staged_rows,
            p.staged_games,
            p.rejects,
            p.max_hist,
    ...
    let rora: Vec<f64> = passes
        .iter()
        .map(|p| rate_per_s(u64_to_f64(cfg.expect_rows as u64), p.wall_ns))
        .collect();
    let rnat: Vec<f64> = passes
        .iter()
        .map(|p| rate_per_s(u64_to_f64(p.staged_rows as u64), p.wall_ns))
        .collect();
    println!("--- medians (median-of-{}; arrays below) ---", passes.len());
    println!(
        "wall_ms_median={:.3}",
        u64_to_f64(median_u64(&walls)) / 1_000_000.0
    );
    println!(
        "rows_s_oracle{}_median={:.1}",
        cfg.expect_rows,
        median_f64(&rora)
    );
    ...
    for (k, p) in passes.iter().enumerate() {
        ...
        print!("{:.3}", u64_to_f64(p.wall_ns) / 1_000_000.0);
    }
    println!("]");
        ...
        while r < 256 {
            ...
            if c > 0 {
                println!(
                    ...
                    stages[s],
                    r,
                    reason_name(u8::try_from(r).unwrap_or(0)),
                    c
                );
                any = true;
            }
...
fn main() {
    ...
    while pass_idx < cfg.runs {
        ...
        while fi < inputs.len() {
            ...
            zstd_ns = zstd_ns.saturating_add(nanos_to_u64(t0.elapsed().as_nanos()));
            let t0 = Instant::now();
            ...
            let ranges = match chunked {
                ...
                Err(e) => {
                    ...
                    file_advance[fi] = 1;
                    frame_ns = frame_ns.saturating_add(nanos_to_u64(t0.elapsed().as_nanos()));
                    fi += 1;
                    continue;
                }
            };
        ...
        let walk_ns: u64 = nanos_to_u64(s0.elapsed().as_nanos());
        let gate_ns: u64 = 0;
        for r in rejects_pre.iter() {
            let slot = (r.stage as usize)
                .saturating_mul(256)
                .saturating_add(r.reason as usize);
        ...
        if pass_idx == 0 {
            ...
            let mut k = 0usize;
            while k < inputs.len() {
                let adv = u32::try_from(file_advance[k]).unwrap_or(u32::MAX);
        ...
        let commit_ns = nanos_to_u64(c0.elapsed().as_nanos());
        ...
        let fill_ns = nanos_to_u64(f0.elapsed().as_nanos());
        if (fillout.rows as usize) != staged_total
            || (fillout.games_consumed as usize) != games_total
        {
            eprintln!(
        ...
        let wall_ns = nanos_to_u64(wall_t0.elapsed().as_nanos());
        passes.push(PassOut {
            wall_ns,
            zstd_ns,
            frame_ns,
