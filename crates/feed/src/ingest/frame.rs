...
fn frame_file_with<'a>(
    ...
    for (k, range) in split_game_ranges(buf).into_iter().enumerate() {
        let gi = base_game_idx.saturating_add(u32::try_from(k).unwrap_or(u32::MAX));
        out.push(frame_lines(buf, range, object_id, gi, parse));
    }
    let staged = u32::try_from(out.len()).unwrap_or(u32::MAX);
    (out, staged)
}
...
fn finish_game<'a>(
    ...
    let last_idx = u32::try_from(events.len())
        .unwrap_or(u32::MAX)
        .saturating_sub(1);
...
fn parse_line_baseline<'a>(
    ...
    let consumed = match obj.get("consumed") {
        ...
        Some(Value::Array(items)) => {
            ...
            (arr, u8::try_from(items.len()).unwrap_or(0))
        }
        Some(_) => return Err(GateReject::bad_tile(game_idx, idx)),
    };
    let tsumogiri = obj
    ...
    if wall.is_none() {
        for key in ["wall", "wall_tiles", "tiles"] {
            ...
            for (i, x) in arr.iter().enumerate() {
                ...
                ids[i] = u8::try_from(n).unwrap_or(0);
            }
...
fn seat_of(value: Option<&Value>) -> u8 {
    match value.and_then(|v| v.as_u64()) {
        Some(s) if s <= 3 => u8::try_from(s).unwrap_or(NO_SEAT),
        _ => NO_SEAT,
    }
}
