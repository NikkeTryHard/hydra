...
fn legal_of(w: &Walked, row: usize) -> Vec<u32> {
    ...
    let len = usize::try_from(i64::from_le_bytes(
        p[base + 128..base + 136].try_into().unwrap(),
    ))
    .unwrap();
    let mut out = Vec::new();
    for k in 0..len {
        out.push(
            i32::from_le_bytes(p[base + k * 4..base + k * 4 + 4].try_into().unwrap())
                .cast_unsigned(),
        );
    }
    out
...
fn f1_five_decisions_collapsed_bitwise() {
    ...
    let mut c0 = [0u8; 34];
    for (t, slot) in c0.iter_mut().enumerate() {
        *slot = rd_u8(&w.planes[PLANE_CONCEALED], 0, t);
    }
    assert_eq!(c0[0], 4);
    assert_eq!(c0[4], 4);
    assert_eq!(c0[8], 4);
