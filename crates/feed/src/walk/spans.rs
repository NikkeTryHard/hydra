    ...
    while seat < 4 {
        ...
        let ron_offered = if shape && ttype < 34 {
            ...
            if !river[ttype as usize] {
                ...
                let mut rt = 0usize;
                while rt < 34 {
                    if river[rt] && shape_win_with(hand, u8::try_from(rt).unwrap_or(0), meld_open) {
                        furiten = true;
                        break;
                    }
                    rt += 1;
...
pub(crate) fn mint_row(
    ...
    // #4/#5 history kinds + mask (T-variable; lens recorded).
    {
        ...
        out.hist_lens
            .push(u32::try_from(hist.len()).unwrap_or(u32::MAX));
    }
    // #6..#11 scalars.
    ...
    // #12 legal_ids[32] + legal_len.
    {
        ...
        while k < 32 {
            ...
            put_i32(&mut out.planes[PLANE_LEGAL], id.cast_signed());
            k += 1;
        }
        put_i64(
            &mut out.planes[PLANE_LEGAL],
            i64::try_from(legal.len().min(32)).unwrap_or(0),
        );
    }
    l.phase = phase;
    ...
    put_i32(
        &mut out.planes[PLANE_LIVE_WALL],
        l.live_wall().cast_signed(),
    );
