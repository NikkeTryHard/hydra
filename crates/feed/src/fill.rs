...
impl Scratch {
    ...
    pub fn commit(&mut self, g: &StagedGame) {
        // proof: staged row counts < u32::MAX.
    ...
    pub fn fill_pinned(
        ...
        // proof: committed counts < u32::MAX.
        #[allow(clippy::cast_possible_truncation)]
        let out = FillOut {
    ...
    pub(crate) fn drain_prefix(&mut self, commit_rows: usize, commit_games: usize, hist_entries: usize) {
        ...
        while g < self.game_starts.len() {
            // proof: committed rows < u32::MAX, rebase preserves non-negativity.
            #[allow(clippy::cast_possible_truncation)]
            {
...
mod fill_tests {
    ...
    fn game_text(pais: &[&str]) -> Vec<u8> {
        ...
        for (k, pai) in pais.iter().enumerate() {
            let actor = u8::try_from(k % 4).unwrap_or(0);
    ...
    fn small_buffer_drains_nothing_plane_attributed() {
        ...
        assert_eq!(
            err,
            FillError::BufferTooSmall {
                plane: u8::try_from(PLANE_ACTOR).unwrap(),
    ...
    fn truncate_rows_rolls_back_call() {
        ...
        let mut s = Scratch::new();
        s.commit(&a);
        let entry_rows = u32::try_from(s.rows()).unwrap();
        s.commit(&b);
        assert_eq!((s.rows(), s.games()), (10, 2));
    ...
    fn stage_batch_parallel_ordered_past_channel_bound() {
        ...
        while k < 150 {
            jobs.push(StageJob {
                game_idx: u32::try_from(k).unwrap(),
                object_id: 7,
        ...
        let mut k = 0usize;
        while k < 150 {
            assert_eq!(games[k].game_idx, u32::try_from(k).unwrap());
