...
/// `WALL_LEN` as `i64`/`u32` for validated comparisons (136 fits both).
pub const WALL_LEN_I64: i64 = 136;
pub const WALL_LEN_U32: u32 = 136;
...
fn logical(t: i64) -> usize {
    // proof: tile ids 0..136 (or euclid non-negative) div 4 gives 0..34, fits usize.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let idx = t.div_euclid(4) as usize;
    idx
}
...
fn validate_inner(rec: &GameRecord, adapter: &AdapterProbe, schema_ok: bool) -> Outcome {
    ...
    if let Some(wall) = &rec.wall_tiles {
        ...
        let mut perm = true;
        for t in wall.iter() {
            if *t < 0 || *t >= WALL_LEN_I64 {
                perm = false;
                break;
            }
            // proof: bounds-checked 0 <= *t < 136 above, fits usize.
            #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
            let i = *t as usize;
            if !seen[i] {
                seen[i] = true;
    ...
    for (idx, ev) in rec.events.iter().enumerate() {
        ...
        if let Some(t) = tile
            && RED_IDS.contains(&t)
            && !RED_LOGICAL.contains(&i64::try_from(logical(t)).unwrap_or(i64::MAX))
        {
            return invalid(
                rec,
                ErrorClass::RedIdentity,
...
fn is_sentinel(v: &Value) -> bool {
    if let Some(i) = v.as_i64() {
        ...
        // proof: -1 is exactly representable in f64.
...
mod tests {
    ...
    fn perm_wall() -> [i64; WALL_LEN] {
        ...
        for (i, v) in w.iter_mut().enumerate() {
            *v = i64::try_from(i).unwrap_or(i64::MAX);
        }
        w
    }
