...
fn kind_for_chosen(chosen: i64) -> &'static str {
    ...
    let idx = usize::try_from(chosen).unwrap_or(usize::MAX);
...
impl StreamDriver {
    ...
    pub fn push_games(&mut self, jobs: Vec<DriverJob>) -> Result<PushOut, DriverError> {
        ...
        while k < idxs.len() {
            let pos = idxs[k];
            let job = &jobs[pos];
            let game_idx = u32::try_from(base_seen.saturating_add(pos as u64))
                .map_err(|_| DriverError::InvalidArg("driver game index overflow".to_string()))?;
        ...
        while pos < jobs.len() {
            ...
            if games.get(gi).is_some_and(|game| game.game_idx == game_idx) {
                ...
                let chosen_plane = game.planes.get(PLANE_CHOSEN).ok_or_else(|| {
    ...
    pub fn next_microbatch(
        ...
        let start = usize::try_from(consumed)
            .map_err(|_| DriverError::InvalidArg("driver buffer offset overflow".to_string()))?;
        ...
        let t_len = match pick_t_len(max_hist) {
            Some(t) => t,
            None => {
                // proof: plane index 4 fits in u8.
        ...
        while i < N_PLANES {
            ...
            if need > cap {
                // proof: plane index <26 fits in u8.
                #[allow(clippy::cast_possible_truncation)]
                return Err(DriverError::BufferTooSmall {
                    plane: i as u8,
                    needed: need,
        ...
        let classes: Vec<(String, u64)> =
            self.classes.iter().map(|(k, v)| (k.clone(), *v)).collect();
        // proof: microbatch sizes and history lengths < u32::MAX.
        #[allow(clippy::cast_possible_truncation)]
    ...
    fn compact(&mut self) {
        ...
        for entry in &self.entries {
            ...
            if count <= remaining {
                ...
                // proof: per-game row counts < usize::MAX.
                #[allow(clippy::cast_possible_truncation)]
                {
        ...
        for meta in &mut self.rows_meta {
            // proof: dropped game counts < u32::MAX.
            #[allow(clippy::cast_possible_truncation)]
            {
...
fn fill_take_into(
    ...
    // #5 mask `[count, t_len]` bool: per-row ones prefix + zero tail.
    {
        ...
        // proof: plane index 5 fits in u8.
        #[allow(clippy::cast_possible_truncation)]
        let dst = dst.map_err(|e| match e {
            FillError::NullPointer { .. } => FillError::NullPointer {
