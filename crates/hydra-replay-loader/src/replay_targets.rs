...
pub mod belief {
    //! Stage A belief teacher target construction.
    ...
    pub fn build_stage_a_teacher(
        ...
        let mixture_weights = if config.mixture_entropy_threshold >= 0.0
            && entropy <= config.mixture_entropy_threshold
        {
...
pub use belief::{StageABeliefAuditSummary, StageABeliefTarget};
...
pub fn exact_waits(state: &GameState, player: usize) -> ([f32; 34], bool) {
...
pub fn bool_mask_to_f32(mask: [bool; HYDRA_ACTION_SPACE]) -> [f32; HYDRA_ACTION_SPACE] {
...
pub fn build_safety_residual_targets(
...
pub fn build_stage_a_belief_targets(
    state: &GameState,
    actor: usize,
    obs: &Observation,
