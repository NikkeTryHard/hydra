    ...
    let mean_of = |action: u32| -> Result<f64, SearchError> {
        ...
        while i < candidates.len() {
            if candidates[i] == action {
                ...
                // proof: visit counts are small counters (< 2^53), exact; mean tolerates 1ulp.
                #[allow(clippy::cast_precision_loss)]
                let visits_f: f64 = visits[i] as f64;
                let q = sums[i][root_seat as usize] / visits_f;
                if !q.is_finite() {
                    return Err(SearchError::NonFinite {
                        context: "gumbel fallback q",
                    });
                }
                return Ok(q);
            }
        ...
        Err(SearchError::InvalidArg {
            detail: "gumbel survivor not a candidate",
        })
    };
...
fn stats_digest(stats: &[(u32, u64, [f64; 4])]) -> Result<String, SearchError> {
    ...
    let bytes = hydra_feed::canon::canonical_bytes(&doc, "search:gumbel_stats").map_err(|err| {
        SearchError::Canon {
...
pub fn gumbel_halving_batch(batch_json: &[u8]) -> Result<HalvingOut, SearchError> {
    ...
    if batch.gumbels.len() != sorted.len() {
        return Err(SearchError::InvalidArg {
            detail: "gumbels must cover root_legal",
        });
    }
    let mut g = 0;
    while g < sorted.len() {
    ...
    if batch.worlds.len() > total {
        return Err(SearchError::InvalidArg {
            detail: "worlds len exceeds schedule total",
        });
    }
    ...
    if batch.policy_dirs.len() != executed {
        return Err(SearchError::InvalidArg {
            detail: "policy_dirs rows must match worlds",
        });
    }
    ...
    while o < batch.leaf_overrides.len() {
        ...
        while k < 4 {
            ...
            if !value.is_finite() {
                return Err(SearchError::NonFinite {
                    context: "gumbel override vector",
                });
            }
            vector[k] = value;
            k += 1;
    ...
    while rr < rounds {
        ...
        if complete {
            ...
            while m < survivors.len() {
                ...
                // proof: visit counts are small counters (< 2^53), exact; mean tolerates 1ulp.
                #[allow(clippy::cast_precision_loss)]
                let visits_f: f64 = arm_visits[pos] as f64;
                let q = arm_sums[pos][batch.root_seat as usize] / visits_f;
                if !q.is_finite() {
                    return Err(SearchError::NonFinite {
                        context: "gumbel cut q",
                    });
                }
    ...
    if world_ptr != executed {
        return Err(SearchError::InvalidArg {
            detail: "world schedule miscount",
        });
    }
...
mod tests {
    ...
    fn envelope(
        ...
        let gumbel_pairs: Vec<serde_json::Value> = gumbels
            .iter()
            .map(|(a, g)| serde_json::json!([a, g]))
            .collect();
        serde_json::json!({
            "worlds": worlds,
            "rules_hash": RULES,
    ...
    fn tiny_two_arm_shape_runs_end_to_end() {
        ...
        let worlds = vec![
            world_json(&w0, &[8, 9, 10, 11]),
            world_json(&w1, &[8, 9, 10, 11]),
        ];
        let batch = envelope(
            worlds,
            vec![vec![0], vec![0]],
