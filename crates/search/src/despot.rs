...
pub fn scenario_seed(
    ...
    let bytes =
        hydra_feed::canon::canonical_bytes(&map, "despot:scenario_seed").map_err(|err| {
            SearchError::Canon {
...
pub fn check_natural(scenario: &Scenario) -> Result<(), SearchError> {
    if scenario.log_target != scenario.log_proposal {
        return Err(SearchError::InvalidArg {
            detail: "non-natural scenario: proposal consulted",
        });
    }
    if !scenario.weight.is_finite() || scenario.weight <= 0.0 {
        return Err(SearchError::NonFinite {
            context: "despot scenario weight",
        });
    }
    Ok(())
}
...
pub fn validate_packet_partition(entries: &[(u32, f64)]) -> Result<(), SearchError> {
    if entries.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "packet entries must be non-empty",
        });
    }
...
pub fn lower_value(
    ...
    if world_refs.len() != seed_hexes.len() {
        return Err(SearchError::InvalidArg {
            detail: "despot refs/seeds length mismatch",
        });
    }
    ...
    // proof: world-ref count is a small vec len (< 2^53), exact; mean weight tolerates 1ulp.
    ...
    while idx < count {
        ...
        let bytes =
            hydra_feed::canon::canonical_bytes(&map, "despot:lower_value").map_err(|err| {
                SearchError::Canon {
                    detail: err.to_string(),
                }
            })?;
...
mod tests {
    ...
    fn k16_pack_weights_and_natural() {
        let packs =
            sample_packs("cand_k", "case_k", DESPOT_K, |idx| 1_000 + idx as u64).expect("packs");
    ...
    fn non_natural_rejected() {
        let mut packs = sample_packs("c", "case", 2, |idx| idx as u64).expect("packs");
