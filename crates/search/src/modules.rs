...
/// Shared output for module transforms:
/// `(particles, weights, budget_calls, budget_transitions, meta_json)`.
pub type ModuleTransformOutput = (Vec<f64>, Vec<f64>, u64, u64, Vec<u8>);
...
pub fn module_transform(
    ...
    _seed: u64,
    meta_json: &[u8],
) -> Result<ModuleTransformOutput, SearchError> {
    if particles.len() != weights.len() {
        return Err(SearchError::InvalidArg {
            detail: "module particles/weights length mismatch",
        });
    }
    let meta: serde_json::Value = if meta_json.is_empty() {
        serde_json::Value::Null
    } else {
        serde_json::from_slice(meta_json).map_err(|err| SearchError::Canon {
    ...
    match module_id {
        ...
        "defensive_mis" => {
            ...
            let updates = serde_json::json!({"mis_applied": true, "mis_single_denominator": true});
            Ok((
                particles.to_vec(),
                scaled,
                budget_calls.saturating_add(add),
        ...
        "fixed_mlmc" => {
            ...
            // proof: particle-slice len < 2^53 in practice, exact; oracle mean tolerates 1ulp.
            #[allow(clippy::cast_precision_loss)]
            let n_f: f64 = particles.len() as f64;
            let base = if particles.is_empty() {
                0.0
            } else {
                crate::builtin_sum(particles) / n_f
            };
            let corrected = base + 0.05 - 0.02;
            let out = vec![corrected; particles.len()];
            let updates = serde_json::json!({"mlmc_applied": true, "mlmc_telescope": corrected});
            Ok((
                out,
                weights.to_vec(),
        ...
        "coreset" => {
            ...
            if sum > 0.0 {
                ...
                // proof: kept-slice len <= 2, exact as `f64`.
                #[allow(clippy::cast_precision_loss)]
                let ws_len_f: f64 = ws.len() as f64;
                let uniform = 1.0 / ws_len_f;
                ws.fill(uniform);
            }
            let add = keep as u64;
            let updates = serde_json::json!({"coreset_applied": true, "coreset_search_only": true});
            Ok((
                ps,
                ws,
                budget_calls.saturating_add(add),
        ...
        "controlled_smc" => {
            ...
            // proof: particle-slice len < 2^53 in practice, exact; ESS gate tolerates 1ulp.
            #[allow(clippy::cast_precision_loss)]
            let n_f: f64 = n as f64;
            let fired = ess <= 0.5 * n_f;
            // proof: `n` is a slice len, fits `u64` on 64-bit (saturates like `as` on 32-bit, never under-counts by more than clamp).
            #[allow(clippy::cast_possible_truncation)]
            let n_u: u64 = n as u64;
            let (add_calls, add_trans) = if fired { (n_u, n_u) } else { (0, 0) };
            let updates = serde_json::json!({
                "smc_applied": true,
                "unnormalized": true,
...
fn voc_route(
    ...
    let get_int = |key: &str| -> Result<u64, SearchError> {
        meta.get(key)
            ...
            .ok_or(SearchError::InvalidArg {
                detail: "voc routing param must be a non-negative int",
            })
    };
    ...
    if floor > cap || cap == 0 || budget == 0 {
        return Err(SearchError::InvalidArg {
            detail: "voc routing params inconsistent",
        });
    }
    let count = particles.len();
    if count == 0 {
        return Err(SearchError::InvalidArg {
            detail: "voc routing needs at least one cell",
        });
    }
    ...
    let scores: Vec<f64> = match meta.get("scores") {
        ...
        Some(serde_json::Value::Array(items)) => {
            if items.len() != count {
                return Err(SearchError::InvalidArg {
                    detail: "voc scores must match particles",
                });
            }
    ...
    // proof: voc count < 2^53 in practice, exact; cap fraction tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let count_f: f64 = count as f64;
    let cap_frac = 0.25f64.max(1.0 / count_f);
    // proof: `budget` is a validated u64 budget (< 2^53), exact; `ceil` is non-negative finite here.
    #[allow(clippy::cast_precision_loss)]
    let budget_f: f64 = budget as f64;
    // proof: `cap_frac*budget` non-negative finite (`count>=1`, `budget` finite-checked at the gate), `as` saturates like the oracle.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let mut cap_units: u64 = (cap_frac * budget_f).ceil() as u64;
    if cap_units > cap {
        cap_units = cap;
    }
    ...
    // proof: voc count fits `u64` on 64-bit (saturates like `as` on 32-bit, caller charges only).
    #[allow(clippy::cast_possible_truncation)]
    let overhead: u64 = count as u64;
    Ok((
        particles.to_vec(),
        weights.to_vec(),
...
fn largest_remainder(scores: &[f64], units: u64) -> Result<Vec<u64>, SearchError> {
    ...
    if !total.is_finite() || total <= 0.0 {
        // proof: `count` is a non-empty slice len (checked above), fits `u64` on 64-bit.
        #[allow(clippy::cast_possible_truncation)]
        let count_u: u64 = count as u64;
        let base = units / count_u;
        let mut shares = vec![base; count];
        // proof: `count` is a non-empty slice len (checked above), fits `u64` on 64-bit.
        #[allow(clippy::cast_possible_truncation)]
        let count_u2: u64 = count as u64;
        let mut leftover = units - base * count_u2;
        let mut idx = 0;
        while leftover > 0 {
            shares[idx % count] += 1;
            leftover -= 1;
    ...
    // proof: `units` is a validated u64 budget (< 2^53), exact; share math tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let units_f: f64 = units as f64;
    ...
    for v in scores {
        ...
        exact.push(part);
        // proof: `part` is `v/total*units` with `v/total` in [0,1]-ish non-negative share math; `floor` fits `u64` (saturates like `as` on huge).
        #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
        let base: u64 = part.floor() as u64;
        shares.push(base);
        assigned += base;
    }
    ...
    order.sort_by(|a, b| {
        // proof: share counts < 2^53, exact as `f64`; sort-key tolerates 1ulp.
        #[allow(clippy::cast_precision_loss)]
        let ra: f64 = exact[*a] - shares[*a] as f64;
        #[allow(clippy::cast_precision_loss)]
        let rb: f64 = exact[*b] - shares[*b] as f64;
...
mod tests {
    ...
    fn rao_blackwell_exact_shift() {
        let (ps, ws, calls, trans, meta) =
            module_transform("rao_blackwell", &[0.0, 1.0], &[0.5, 0.5], 0, 0, 0, b"").expect("rb");
    ...
    fn smc_gate_fires_and_copies() {
        let meta = b"";
        let (_, _, c0, _, m0) =
            module_transform("controlled_smc", &[0.0; 4], &[0.25; 4], 0, 0, 0, meta).expect("copy");
        assert_eq!(c0, 0);
        assert!(
            String::from_utf8(m0)
                .expect("utf8")
                .contains("resample_skipped\":true")
        );
        let (_, _, c1, _, _) = module_transform(
            "controlled_smc",
            &[0.0; 4],
            &[1.0, 0.0, 0.0, 0.0],
            0,
    ...
    fn voc_exact_math_matches_oracle_example() {
        ...
        .expect("voc");
        let back: serde_json::Value = serde_json::from_slice(&out).expect("meta json");
