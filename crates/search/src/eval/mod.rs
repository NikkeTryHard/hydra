...
pub(crate) fn canon_digest(value: &serde_json::Value, record: &str) -> Result<String, SearchError> {
    hydra_feed::digest::of_canonical(value).map_err(|e| SearchError::Canon {
...
pub(crate) fn canon_bytes_of(
    ...
    hydra_feed::canon::canonical_bytes(value, record).map_err(|e| SearchError::Canon {
        detail: format!("{record}: {e}"),
    })
}
...
pub fn block_mean(values: &[f64]) -> Result<f64, SearchError> {
    if values.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "empty wall block has no games",
        });
    }
...
pub fn ctr_block(seed: &[u8], index: u64) -> [u8; 32] {
    ...
    // proof: seed-slice len < 2^32 in practice (fails closed downstream if not); length prefix only.
    #[allow(clippy::cast_possible_truncation)]
    let seed_len: u32 = seed.len() as u32;
    pre.extend_from_slice(&seed_len.to_be_bytes());
...
impl<'a> CtrReader<'a> {
    ...
    pub fn new(seed: &'a [u8], cursor: u64) -> Result<Self, SearchError> {
        if seed.is_empty() {
            return Err(SearchError::InvalidArg {
                detail: "seed must be nonempty bytes",
            });
        }
    ...
    pub fn get_bytes(&mut self, count: usize) -> Vec<u8> {
        ...
        while out.len() < count {
            ...
            #[allow(clippy::cast_possible_truncation)]
            let block_u: usize = CTR_BLOCK as usize;
            let take = (count - out.len()).min(block_u - off);
    ...
    pub fn random_below(&mut self, bound: u64) -> Result<u64, SearchError> {
        ...
        loop {
            ...
            if value < limit {
                // proof: `value % bound` < `bound` <= u64::MAX (`bound` from `u64`).
                #[allow(clippy::cast_possible_truncation)]
                let v: u64 = (value % u128::from(bound)) as u64;
                return Ok(v);
            }
        }
    }
...
pub fn semantic_seed(master_seed: &[u8], key: &serde_json::Value) -> Result<[u8; 32], SearchError> {
    if master_seed.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "master_seed must be nonempty bytes",
        });
    }
