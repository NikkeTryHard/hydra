    ...
    pub fn check(&self) -> Result<(), SearchError> {
        if self.mode.is_empty() {
            return Err(SearchError::InvalidArg {
                detail: "mode must be a nonempty str",
            });
        }
        for (name, digest) in [
...
impl TelemetryTolerance {
    ...
    pub fn new(allow_missing: HashSet<String>) -> Result<Self, SearchError> {
        ...
        let mut forbidden: Vec<&String> = allow_missing
            .iter()
            .filter(|f| core.contains(f.as_str()) || f.as_str() == "invalid_reason")
            .collect();
        forbidden.sort();
    ...
    pub fn required_for(&self, mode: &str) -> Result<Vec<&'static str>, SearchError> {
        ...
        let tolerated: Vec<&&str> = extras
            .iter()
            .filter(|e| self.allow_missing.contains(**e))
            .collect();
...
pub fn telemetry_invalid_reason(
    ...
    let missing: Vec<&str> = required
        .into_iter()
        .filter(|f| row.field_is_missing(f))
        .collect();
    if missing.is_empty() {
        Ok(None)
    } else {
...
mod tests {
    ...
    fn missing_report_sorted_and_empty() {
        ...
        assert!(
            block_missing_telemetry_report(&rows, &tol)
                .unwrap()
                .is_empty()
        );
    }
