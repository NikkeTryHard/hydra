...
/// Output of [`score_selection`]: `(metric, interval, excluded)`.
pub type SelectionScore = (f64, (f64, f64, f64), Vec<super::blocks::ExcludedBlock>);
...
pub fn validate_alpha_resamples(alpha: f64, resamples: usize) -> Result<(), SearchError> {
    if !(0.0 < alpha && alpha < 0.5) || !alpha.is_finite() {
        return Err(SearchError::InvalidArg {
            detail: "alpha must lie in (0, 0.5)",
        });
    }
    if resamples < 100 {
        return Err(SearchError::InvalidArg {
            detail: "resamples must be an int >= 100",
        });
    }
    Ok(())
}
...
pub fn fixed_n_samples(s: f64, delta: f64, alpha: f64, beta: f64) -> Result<u64, SearchError> {
    ...
    for value in [alpha, beta] {
        if !(0.0 < value && value < 1.0) || !value.is_finite() {
            return Err(SearchError::InvalidArg {
                detail: "alpha and beta must lie in (0, 1)",
            });
        }
    }
    let z_a = inv_normal_cdf(1.0 - alpha);
    let z_b = inv_normal_cdf(1.0 - beta);
    // proof: `s,delta` finite positive and `z` finite (validated above), so
    // the `ceil` is non-negative finite; `as` saturates to `u64::MAX` on overflow like the oracle.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let n: u64 = (((z_a + z_b) * s / delta).powi(2)).ceil() as u64;
    Ok(n)
}
...
pub fn percentile_indexes(alpha: f64, resamples: usize) -> Result<(usize, usize), SearchError> {
    ...
    // proof: `resamples` >= 100 and < 2^53 in practice, `as f64` exact there; percentile tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let resamples_f: f64 = resamples as f64;
    // proof: `alpha` in (0,0.5), so `(alpha/2)*R` in [0,R), `floor` fits `usize`.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let low: usize = ((alpha / 2.0) * resamples_f).floor() as usize;
    // proof: `(1-alpha/2)*R` in (0.75R,R], `ceil` in 1..=R, minus 1 fits `usize`.
    #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
    let high: usize = ((1.0 - alpha / 2.0) * resamples_f).ceil() as usize - 1;
    Ok((low, high))
}
...
pub fn bootstrap_interval(
    ...
    // proof: block-slice len < 2^53 in practice, exact; mean tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let len_f: f64 = values.len() as f64;
    let estimate = neumaier_sum(&values) / len_f;
...
pub fn sign_flip_centered_interval(
    ...
    // proof: block-slice len < 2^53 in practice, exact; mean tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let values_len_f: f64 = values.len() as f64;
    let observed = neumaier_sum(&values) / values_len_f;
    // proof: resample-slice len < 2^53 in practice, exact; mean tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let stats_len_f: f64 = sorted_stats.len() as f64;
    let center = neumaier_sum(sorted_stats) / stats_len_f;
    Ok((
        observed,
...
pub fn group_means(groups: &[&[f64]]) -> Result<Vec<f64>, SearchError> {
    ...
    for group in groups {
        ...
        // proof: group-slice len < 2^53 in practice, exact; mean tolerates 1ulp.
        #[allow(clippy::cast_precision_loss)]
        let group_len_f: f64 = group.len() as f64;
        means.push(neumaier_sum(group) / group_len_f);
    }
    Ok(means)
}
...
fn hedged_capital_rejected(scaled: &[f64], theta: f64, threshold: f64) -> bool {
    ...
    for (position, observation) in scaled.iter().enumerate() {
        ...
        // proof: `index` is a 1-based position (< 2^53 in practice), exact; running mean tolerates 1ulp.
        #[allow(clippy::cast_precision_loss)]
        let denom_f: f64 = (index - 1) as f64;
        let mu_prev = if index > 1 {
            running_sum / denom_f
        } else {
            0.5
        };
        let variance_term = (mu_prev * (1.0 - mu_prev)).max(1e-6);
        // proof: `index` is a 1-based position (< 2^53 in practice), exact; capital path tolerates 1ulp.
        #[allow(clippy::cast_precision_loss)]
        let index_f: f64 = index as f64;
        let lam = lam_max.min(
            (8.0 * (2.0 * threshold).ln() / (variance_term * index_f * (index_f + 1.0).ln()))
                .sqrt(),
        );
...
fn scale_to_unit_interval(values: &[f64], bounds: (f64, f64)) -> Result<Vec<f64>, SearchError> {
    ...
    if !(low.is_finite() && high.is_finite() && high > low) {
        return Err(SearchError::InvalidArg {
            detail: "bounds must be finite with high > low",
        });
    }
    let width = high - low;
...
pub fn hedged_cs_path(
    ...
    if !(4..=512).contains(&grid_size) {
        return Err(SearchError::InvalidArg {
            detail: "grid_size must be an int in [4, 512]",
        });
    }
    ...
    for moment in &times {
        if *moment < 1 || *moment > count {
            return Err(SearchError::InvalidArg {
                detail: "peek times must be ints in [1, n]",
            });
        }
    }
    // proof: `grid_size` in 4..=512 (validated above), exact as `f64`.
    #[allow(clippy::cast_precision_loss)]
    let grid_f: f64 = grid_size as f64;
    let alpha_j = alpha / grid_f;
    let threshold = 1.0 / alpha_j;
    // proof: `grid_size - 1` in 3..=511, exact as `f64`; grid is a coarse 4..512-point cover, 1ulp tolerable.
    #[allow(clippy::cast_precision_loss)]
    let grid_m1_f: f64 = (grid_size - 1) as f64;
    let thetas: Vec<f64> = (0..grid_size)
        .map(|i| {
            // proof: `i` < `grid_size` <= 512, exact as `f64`.
            #[allow(clippy::cast_precision_loss)]
            let i_f: f64 = i as f64;
            0.02 + (0.98 - 0.02) * i_f / grid_m1_f
        })
        .collect();
    ...
    for moment in times {
        ...
        if survivors.is_empty() {
            ...
            let high =
                low_span + survivors.iter().cloned().fold(f64::NEG_INFINITY, f64::max) * width;
...
pub fn hedged_confidence_sequence(
    ...
    path.into_iter().last().ok_or(SearchError::InvalidArg {
        detail: "no values for CS",
    })
}

/// Adaptive peeking without a declared sequential design is fatal
/// (`sequential_design_guard`, `:317-336`).
pub fn sequential_design_guard(design: &str, declared_peeks: &[usize]) -> Result<(), SearchError> {
    match design {
        "fixed_n" => {
...
impl SelectionConfig {
    ...
    pub fn validate(&self) -> Result<(), SearchError> {
        if self.n < 1 {
            return Err(SearchError::InvalidArg {
                detail: "N must be a positive int",
            });
        }
        ...
        for value in [self.alpha, self.beta] {
            if !(0.0 < value && value < 1.0) || !value.is_finite() {
                return Err(SearchError::InvalidArg {
                    detail: "alpha/beta must lie in (0, 1)",
                });
            }
        }
        if !self.margin.is_finite() {
            return Err(SearchError::InvalidArg {
                detail: "margin must be finite",
            });
        }
        if self.uncertainty_unit != "wall_block" {
            return Err(SearchError::InvalidArg {
                detail: "uncertainty_unit must be 'wall_block'",
            });
        }
        if self.resamples < 1 {
            return Err(SearchError::InvalidArg {
                detail: "resamples must be a positive int",
            });
        }
        if self.seed < 0 {
            return Err(SearchError::InvalidArg {
...
pub fn selection_gate_check(
    config: &SelectionConfig,
    n_blocks_observed: usize,
) -> Result<(), SearchError> {
    config.validate()?;
    ...
    if n_blocks_observed > config.n {
        return Err(SearchError::InvalidArg {
            detail: "extra look exceeds frozen N",
        });
    }
    if !config.declared_peeks.contains(&n_blocks_observed) {
        return Err(SearchError::InvalidArg {
            detail: "extra look outside declared peeks",
        });
    }
...
pub fn score_selection(
...
) -> Result<SelectionScore, SearchError> {
    if blocks.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "blocks must be a tuple of WallBlock",
        });
    }
    let result: BlockAggregateResult = aggregate_blocks(blocks, telemetry_by_game, tolerance)?;
    if result.valid.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "no valid wall blocks to score",
        });
    }
    if peek_index < 1 {
        return Err(SearchError::InvalidArg {
            detail: "peek_index must be a positive int",
        });
    }
    ...
    let valid_blocks: Vec<WallBlock> = blocks
        .iter()
        .filter(|b| valid_ids.contains(b.wall_id.as_str()))
        .cloned()
        .collect();
    let contrasts = placement_block_contrast(&valid_blocks)?;
    selection_gate_check(config, peek_index)?;
    // proof: contrast-slice len < 2^53 in practice, exact; mean tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let contrasts_len_f: f64 = contrasts.len() as f64;
    let metric = neumaier_sum(&contrasts) / contrasts_len_f;
...
mod tests {
        ...
        (0..8)
            .map(|w| {
                let contrasts: Vec<f64> = (0..10)
                    .map(|g| {
                        // proof: `w` in 0..8 (wall index), exact as `f64`.
                        #[allow(clippy::cast_precision_loss)]
                        let w_f: f64 = w as f64;
                        if g % 2 == 0 { w_f } else { 100.0 - w_f }
                    })
                    .collect();
                let games: Vec<String> = (0..10).map(|g| format!("wall-{w:02}:g{g}")).collect();
    ...
    fn block_atomicity() {
        ...
        let rechunked: Vec<f64> = pooled
            .chunks(10)
            .map(|c| {
                // proof: chunk len 10, exact as `f64`.
                #[allow(clippy::cast_precision_loss)]
                let c_len_f: f64 = c.len() as f64;
                neumaier_sum(c) / c_len_f
            })
            .collect();
    ...
    fn percentile_and_assembly() {
        ...
        let sorted: Vec<f64> = (0..2000)
            .map(|i| {
                // proof: `i` in 0..2000, exact as `f64`.
                #[allow(clippy::cast_precision_loss)]
                let i_f: f64 = i as f64;
                i_f / 1000.0 - 1.0
            })
    ...
    fn sign_flip_centered_shift() {
        ...
        // proof: sorted len 2000, exact as `f64`.
        #[allow(clippy::cast_precision_loss)]
        let sorted_len_f: f64 = sorted.len() as f64;
        let center = neumaier_sum(&sorted) / sorted_len_f;
