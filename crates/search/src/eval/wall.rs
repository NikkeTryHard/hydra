...
fn require_wall_tiles(tiles: &[u32]) -> Result<(), SearchError> {
    if tiles.len() != WALL_TILES {
        return Err(SearchError::InvalidArg {
            detail: "wall must have 136 tiles",
        });
    }
    for t in tiles {
        // proof: `WALL_TILES` = 136 const, fits `u32`.
        #[allow(clippy::cast_possible_truncation)]
        let wall_u: u32 = WALL_TILES as u32;
        if *t >= wall_u {
            return Err(SearchError::InvalidArg {
...
fn require_wall_id(wall_id: &str) -> Result<(), SearchError> {
    if wall_id.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "wall_id must be nonempty str",
        });
    }
    Ok(())
}
...
fn require_digest_value(digest: &str) -> Result<(), SearchError> {
    if !is_digest_text(digest) {
        return Err(SearchError::InvalidArg {
            detail: "digest must be sha256:<64 hex>",
        });
    }
    Ok(())
}
...
mod tests {
    ...
    fn sorts_136_parity() {
        ...
        assert_eq!(
            wall_fingerprint(&perm).unwrap(),
            wall_fingerprint(&ordered_wall()).unwrap()
        );
        assert_ne!(
    ...
    fn exact_duplicates_discovery_order() {
        ...
        assert!(
            find_exact_duplicates(&[("".to_string(), "sha256:".to_string() + &"a".repeat(64))])
                .is_err()
        );
    ...
    fn near_duplicates_share_fingerprint() {
        ...
        let mut rev = tiles.clone();
        rev.reverse();
        let walls = vec![("w-a".to_string(), tiles), ("w-b".to_string(), rev)];
        assert_eq!(
