...
/// One tree arm: `(action, visits, value_sum)`.
pub type TreeArm = (u32, u64, [f64; 4]);
/// One tree entry: `(key, visits, arms)`, key-ordered.
pub type TreeEntry = (String, u64, Vec<TreeArm>);
/// Final root selection: `(best_id, vectors, visits)`.
type FinalSelection = (u32, Vec<[f64; 4]>, Vec<u64>);
    ...
    let hex = match text.strip_prefix(PREFIX) {
        ...
        None => {
            return Err(SearchError::InvalidArg {
                detail: "digest must start with sha256:",
            });
        }
    };
    if hex.len() != 64 {
        return Err(SearchError::InvalidArg {
            detail: "digest must hold 64 hex chars",
        });
    }
    let mut i = 0;
    while i < hex.len() {
...
pub fn terminal_vector(world_id: &str) -> Result<[f64; 4], SearchError> {
    ...
    while i < 4 {
        ...
        if !value.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts terminal vector",
            });
        }
        out[i] = value;
        i += 1;
...
pub fn local_model_vector(world_id: &str, leaf_kind: &str) -> Result<[f64; 4], SearchError> {
    ...
    while i < 4 {
        ...
        if !value.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts local model vector",
            });
        }
        vals[i] = value;
        i += 1;
    ...
    while k < 4 {
        ...
        if !value.is_finite() || value.abs() > 2.0 {
            return Err(SearchError::NonFinite {
                context: "ismcts local model centered",
            });
        }
        out[k] = value;
        k += 1;
...
pub fn local_terminal_vector(hands: &[Vec<u32>], live: &[u32]) -> Result<[f64; 4], SearchError> {
    if hands.len() != 4 {
        return Err(SearchError::InvalidArg {
            detail: "local terminal needs 4 seats",
        });
    }
    ...
    while seat < 4 {
        ...
        while t < hands[seat].len() {
            ...
            check_tile(tile)?;
            acc = acc
                .checked_add(u64::from(tile))
                .ok_or(SearchError::InvalidArg {
                    detail: "hand sum overflow",
                })?;
            t += 1;
        }
        // proof: seat-hand tile sums are small (< 136*4*~200), exact as `f64`.
        #[allow(clippy::cast_precision_loss)]
        let acc_f: f64 = acc as f64;
        sums[seat] = acc_f;
        seat += 1;
    }
    ...
    while w < live.len() {
        check_tile(live[w])?;
        wall_acc = wall_acc
            .checked_add(u64::from(live[w]))
            .ok_or(SearchError::InvalidArg {
                detail: "wall sum overflow",
            })?;
        w += 1;
    }
    // proof: wall tile sums are small (< 136*~70), exact as `f64`.
    #[allow(clippy::cast_precision_loss)]
    let wall_f: f64 = wall_acc as f64;
    let wall_sum = wall_f / 100.0;
    let mut k = 0;
    while k < 4 {
    ...
    while n < 4 {
        ...
        if !value.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts local terminal vector",
            });
        }
        out[n] = value;
        n += 1;
...
pub fn exhaustive_path_offset(world_id: &str, path: &[u32]) -> Result<f64, SearchError> {
    if world_id.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "world_id must be non-empty",
        });
    }
    let mut off = 0.0f64;
    let mut d: u32 = 0;
...
fn placeholder_vector(tag: &str, candidate_id: &str) -> Result<[f64; 4], SearchError> {
    if tag.is_empty() || candidate_id.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "placeholder tag must be non-empty",
        });
    }
    ...
    while i < 4 {
        ...
        if !value.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts placeholder vector",
            });
        }
        out[i] = value;
        i += 1;
...
pub fn info_key_from_doc(doc_bytes: &[u8]) -> Result<String, SearchError> {
    if doc_bytes.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "obs doc must be non-empty",
        });
    }
    let mut value: serde_json::Value =
        serde_json::from_slice(doc_bytes).map_err(|_| SearchError::InvalidArg {
            detail: "obs doc must be JSON object",
        })?;
    ...
    let bytes =
        hydra_feed::canon::canonical_bytes_value(&value, "search:info_key").map_err(|err| {
            SearchError::Canon {
...
impl TinyWorld {
    ...
    pub fn validate(&self) -> Result<(), SearchError> {
        ...
        if self.snapshot.is_empty() {
            return Err(SearchError::InvalidArg {
                detail: "snapshot must be non-empty",
            });
        }
        let mut seat = 0;
        while seat < 4 {
...
pub(crate) fn world_id_for(
    ...
    doc.insert(
        "concealed_hands".to_string(),
        serde_json::Value::Array(hands_json),
    );
    ...
    doc.insert(
        ...
        serde_json::json!(observation_hash),
    );
    doc.insert("rules_hash".to_string(), serde_json::json!(rules_hash));
    doc.insert(
        "simulator_snapshot".to_string(),
        serde_json::json!(snapshot),
    );
    let bytes = hydra_feed::canon::canonical_bytes(&doc, "search:world").map_err(|err| {
        SearchError::Canon {
            detail: err.to_string(),
        }
    })?;
...
pub fn exact_transition(
    ...
    if domain != DOMAIN_ISMCTS && domain != DOMAIN_GUMBEL {
        return Err(SearchError::InvalidArg {
            detail: "domain must be ismcts|gumbel",
        });
    }
    let base_step = parent.step.unwrap_or(0);
    let new_step = base_step.checked_add(1).ok_or(SearchError::InvalidArg {
...
pub fn continuation_sample(
    ...
    if direction > 1 {
        return Err(SearchError::InvalidArg {
            detail: "direction must be 0|1",
        });
    }
    if !rng_float.is_finite() || rng_float < 0.0 || rng_float >= 1.0 {
        return Err(SearchError::InvalidArg {
            detail: "rng float must be in [0,1)",
        });
    }
    if sorted.len() == 1 {
        return Ok(sorted[0]);
    ...
    if sorted.len() == 2 {
        let first_prob = if direction == 0 {
            0.5 + CONTINUATION_BIAS
        } else {
            0.5 - CONTINUATION_BIAS
        };
    ...
    // proof: candidate-slice len is small (< 2^53), exact; continuation weight tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let width: f64 = sorted.len() as f64;
    if !width.is_finite() || width <= 0.0 {
        return Err(SearchError::NonFinite {
            context: "ismcts continuation width",
        });
    }
...
fn tree_digest(tree: &BTreeMap<String, IsNode>) -> Result<String, SearchError> {
    ...
    for (key, node) in tree {
        ...
        node_doc.insert(
            "arms".to_string(),
            serde_json::Value::Object(arms.into_iter().collect()),
        );
        doc.insert(
            key.clone(),
            serde_json::Value::Object(node_doc.into_iter().collect()),
        );
    }
    let bytes = hydra_feed::canon::canonical_bytes(&doc, "search:ismcts_tree").map_err(|err| {
        SearchError::Canon {
            detail: err.to_string(),
        }
    })?;
...
fn final_select(
    ...
    tie: TieBreak,
    candidate_id: &str,
) -> Result<FinalSelection, SearchError> {
    if root_seat >= 4 {
...
fn fallback_select(candidates: &[u32], candidate_id: &str) -> Result<FinalSelection, SearchError> {
    if candidates.is_empty() {
        return Err(SearchError::EmptyLegal);
    }
    let mut vectors = Vec::with_capacity(candidates.len());
...
pub fn ismcts_search_batch(batch_json: &[u8]) -> Result<DescentOut, SearchError> {
    ...
    if batch.step_keys.len() != sims {
        return Err(SearchError::InvalidArg {
            detail: "step_keys rows must equal max_sims",
        });
    }
    if batch.policy_dirs.len() != sims {
        return Err(SearchError::InvalidArg {
            detail: "policy_dirs rows must equal max_sims",
        });
    }
    ...
    while o < batch.leaf_overrides.len() {
        ...
        if entry.vector.len() != 4 {
            return Err(SearchError::InvalidArg {
                detail: "override vector must hold 4",
            });
        }
        ...
        while k < 4 {
            ...
            if !value.is_finite() {
                return Err(SearchError::NonFinite {
                    context: "ismcts override vector",
                });
            }
            vector[k] = value;
            k += 1;
        ...
        if overrides.contains_key(&entry.world_id) {
            return Err(SearchError::InvalidArg {
                detail: "duplicate leaf override",
            });
        }
    ...
    while ws < sims {
        ...
        while wd < depth {
            ...
            live_len = live_len.saturating_sub(1);
    ...
    while sim < sims {
        ...
        while step_idx < depth && !current.is_terminal(batch.max_depth) {
            ...
            let action = if actor == batch.root_seat {
                ...
                check_tree_key(&key)?;
                let node = tree.entry(key.clone()).or_default();
        ...
        let vector = match overrides.get(&current.world_id) {
            ...
            None => {
                if leaf_is_terminal || !allow_model {
                    ...
                    model_calls = model_calls.checked_add(1).ok_or(SearchError::InvalidArg {
        ...
        while p < path.len() {
            ...
            match tree.get_mut(key) {
                ...
                None => {
                    return Err(SearchError::InvalidArg {
                        detail: "backup node missing",
                    });
                }
            }
            p += 1;
        }
        sims_run = sims_run.checked_add(1).ok_or(SearchError::InvalidArg {
    ...
    let (selected_id, value_vectors, visits) = match tree.get(&batch.root_key) {
        Some(node) if !node.arms.is_empty() => final_select(
            node,
            &sorted_legal,
            batch.root_seat,
            tie,
    ...
    let mut dump: Vec<TreeEntry> = Vec::new();
    for (key, node) in &tree {
        let mut arms: Vec<TreeArm> = Vec::new();
        let mut i = 0;
        while i < node.arms.len() {
            let (action, stats) = node.arms[i];
...
mod tests {
    ...
    fn transition_unifies_with_domain_salts() {
        ...
        assert_eq!(
            (ismcts.step, ismcts.turn, ismcts.last_action),
    ...
    fn local_shapes_match_oracle() {
        ...
        assert_eq!(
            settle,
            [-0.56295, -0.21235000000000004, 0.18764999999999998, 0.58765]
        );
