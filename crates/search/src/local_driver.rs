...
fn digest_str(text: &str) -> String {
    format!(
        "sha256:{}",
        hex_of(Sha256::digest(text.as_bytes()).as_slice())
    )
}
...
fn utility_offset(world_id: &str, aid: u32, depth: u32) -> f64 {
    ...
    // proof: `% 100` is in 0..100, exact as `f64`.
    #[allow(clippy::cast_precision_loss)]
    let word_f: f64 = (word % OFFSET_MOD) as f64;
    word_f / 1000.0 - 0.05
}
...
fn parse_batch(raw: &[u8]) -> Result<LocalBatchJson, SearchError> {
    ...
    // proof: iteration count <= 1024 (validated above), fits `u32`.
    #[allow(clippy::cast_possible_truncation)]
    let iters_u: u32 = batch.iters.len() as u32;
    if iters_u != batch.iterations {
        return Err(SearchError::InvalidArg {
    ...
    for entry in &batch.init {
        if entry.actor >= 4 {
            return Err(SearchError::InvalidSeat { seat: entry.actor });
        }
        if entry.dist.len() != n {
            return Err(SearchError::InvalidArg {
    ...
    while it <= iterations {
        ...
        while depth < horizon {
            ...
            if regret {
                ...
                #[allow(clippy::cast_precision_loss)]
                let cnt: f64 = counts[k] as f64;
                let total = cnt + 1.0;
                let mut s = 0.0;
                let mut i = 0;
                while i < n {
                    let prev = if counts[k] > 0 {
                        strata[i] * cnt / total
                    } else {
                        0.0
                    };
...
fn dump_rows(rows: &[LocalTableRow]) -> serde_json::Value {
    ...
    while i < rows.len() {
        ...
        out.push(serde_json::json!([
            row.actor, row.info, row.dist, row.visits
        ]));
        i += 1;
    }
