...
pub const PACKET_SCHEMA_HASH: &str =
...
fn canon_digest(
    ...
    let bytes =
        hydra_feed::canon::canonical_bytes(map, record).map_err(|err| SearchError::Canon {
            detail: err.to_string(),
        })?;
    Ok(format!(
        "sha256:{}",
        hex_of(Sha256::digest(bytes.as_slice()).as_slice())
    ))
}
...
pub fn ctr_block(seed: &[u8], index: u64) -> Result<[u8; 32], SearchError> {
    if seed.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "ctr seed must be non-empty",
        });
    }
    let len = u32::try_from(seed.len()).map_err(|_| SearchError::InvalidArg {
        detail: "ctr seed too long",
    })?;
...
impl CtrReader {
    ...
    pub fn get_bytes(&mut self, count: u64) -> Result<Vec<u8>, SearchError> {
        ...
        let span = offset.checked_add(count).ok_or(SearchError::InvalidArg {
            detail: "ctr span overflow",
        })?;
        let needed = span
        ...
        let mut i = 0u64;
        while i < needed {
            let block = first.checked_add(i).ok_or(SearchError::InvalidArg {
                detail: "ctr block overflow",
            })?;
        ...
        self.cursor = self
            ...
            .checked_add(count)
            .ok_or(SearchError::InvalidArg {
                detail: "ctr cursor overflow",
            })?;
        // proof: `offset` < 32 (CTR remainder) and `count` is a small staged byte count; checked_add fails closed on overflow.
        #[allow(clippy::cast_possible_truncation)]
        let offset_u: usize = offset as usize;
        let start = offset_u;
        // proof: `count` is a small staged byte count (< 2^32), fits `usize` on 64-bit; checked_add fails closed.
        #[allow(clippy::cast_possible_truncation)]
        let count_u: usize = count as usize;
        let end = start.checked_add(count_u).ok_or(SearchError::InvalidArg {
            detail: "ctr slice overflow",
        })?;
        if end > out.len() {
            return Err(SearchError::InvalidArg {
                detail: "ctr slice out of range",
            });
        }
    ...
    pub fn random_float(&mut self) -> Result<f64, SearchError> {
        ...
        let f: f64 = u64::from_be_bytes(raw) as f64;
        Ok(f / CTR_U64_DENOM)
    }
    ...
    pub fn random_below(&mut self, bound: u32) -> Result<u32, SearchError> {
        if bound < 2 {
            return Err(SearchError::InvalidArg {
                detail: "ctr bound must be >= 2",
            });
        }
        let bitlen = 64 - (u64::from(bound) - 1).leading_zeros();
        let nbytes = bitlen / 8 + 1;
        let power = 256u64.checked_pow(nbytes).ok_or(SearchError::InvalidArg {
            detail: "ctr width overflow",
        })?;
        ...
        loop {
            ...
            if value < limit {
                // proof: `value % wide` < `wide` <= u32::MAX (`wide` from `u32 bound`).
                #[allow(clippy::cast_possible_truncation)]
                let v: u32 = (value % wide) as u32;
                return Ok(v);
            }
        }
    }
...
pub fn natural_indices(
    ...
    if k == 0 {
        return Err(SearchError::InvalidArg {
            detail: "natural corpus size must be >= 1",
        });
    }
    if count == 0 {
        return Err(SearchError::InvalidArg {
            detail: "natural count must be >= 1",
        });
    }
    if k == 1 {
...
pub fn sampled_draws(
    ...
    if probs.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "sampled frame must be non-empty",
        });
    }
    if draws == 0 {
        return Err(SearchError::InvalidArg {
            detail: "sampled draws must be >= 1",
        });
    }
    for prob in probs {
        if !prob.is_finite() || *prob < 0.0 {
            return Err(SearchError::NonFinite {
                context: "sampled frame probability",
            });
        }
    }
    ...
    let last = match u32::try_from(probs.len() - 1) {
        ...
        Err(_) => {
            return Err(SearchError::InvalidArg {
                detail: "sampled frame too large",
            });
        }
    };
    let mut reader = CtrReader::open(seed, cursor)?;
...
fn packet_id_hex(
    ...
    map.insert(
        "events".to_string(),
        serde_json::Value::Array(vec![event.clone()]),
    );
    map.insert(
        "observation_hash_after".to_string(),
        serde_json::Value::String(obs.to_string()),
    );
    map.insert(
    ...
    let bytes = hydra_feed::canon::canonical_bytes(&map, "belief:packet_id").map_err(|err| {
        SearchError::Canon {
            detail: err.to_string(),
        }
    })?;
...
pub fn enumerate_next(
    ...
    if parent_ref.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "belief parent_ref must be non-empty",
        });
    }
    if root_actor >= 4 {
    ...
    if rules_hash.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "belief rules_hash must be non-empty",
        });
    }
    let log_physical = 0.5f64.ln();
    if !log_physical.is_finite() {
        return Err(SearchError::NonFinite {
            context: "belief log mass",
        });
    }
...
mod tests {
    ...
    fn sampled_draws_weights_match_frame_over_draws() {
        let (chosen, weights, _) = sampled_draws(&[0.5, 0.5], 4, b"belief-test-seed", 0).unwrap();
