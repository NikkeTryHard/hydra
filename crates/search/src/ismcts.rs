...
fn tie_wins(candidate: u32, best: u32, tie: TieBreak) -> bool {
    match tie {
        ...
        TieBreak::StableHash | TieBreak::Lexicographic => sha_hex(candidate) < sha_hex(best),
    }
}
...
pub fn uct_select(
    ...
    for action in &sorted {
        ...
        // proof: visit counts are small counters (< 2^53), exact; UCT bonus tolerates 1ulp.
        #[allow(clippy::cast_precision_loss)]
        let total_f: f64 = total as f64;
        #[allow(clippy::cast_precision_loss)]
        let visits_f: f64 = stats.visits as f64;
        let u = params.uct_c * ((total_f + 1.0).ln() / visits_f).sqrt();
        if !u.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts uct u",
            });
        }
        let val = q + u;
        if !val.is_finite() {
            return Err(SearchError::NonFinite {
                context: "ismcts uct val",
            });
        }
        match best {
            None => {
            ...
            Some(current) => {
                if val > best_val + UCT_EPS
                    || ((val - best_val).abs() <= UCT_EPS && tie_wins(*action, current, params.tie))
                {
                    best = Some(*action);
                    best_val = val;
                }
...
mod tests {
    ...
    fn unvisited_wins_in_sorted_order() {
        ...
        assert_eq!(
            uct_select(&node, &[20, 10], 0, &params).expect("select"),
            10
        );
    }
    ...
    fn stable_hash_tie_is_deterministic() {
        ...
        let params = IsmctsParams {
            tie: TieBreak::StableHash,
            ..IsmctsParams::defaults()
        };
        ...
        let winner = if sha_hex(11) < sha_hex(22) { 11 } else { 22 };
