    ...
    pub fn intern(&mut self, digest: &[u8]) -> Result<u32, SearchError> {
        if digest.is_empty() {
            return Err(SearchError::InvalidArg {
                detail: "digest must be non-empty",
            });
        }
        if let Some(found) = self.forward.get(digest) {
            return Ok(*found);
        }
        // proof: interned-digest count is a small vec len (< 2^32), fits `u32`.
        #[allow(clippy::cast_possible_truncation)]
        let id: u32 = self.reverse.len() as u32;
...
mod tests {
    ...
    fn insertion_order_pins_ids() {
        ...
        let digests = [
            b"sha256:0001".as_slice(),
            b"sha256:0002".as_slice(),
            b"sha256:0003".as_slice(),
        ];
