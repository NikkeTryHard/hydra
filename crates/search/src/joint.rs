...
pub fn check_theta(theta: &str) -> Result<(), SearchError> {
    ...
    Err(SearchError::InvalidArg {
        detail: "theta must be tight|loose",
    })
}
...
pub fn ensure_prior(thetas: &[&str], worlds: &[u64]) -> Result<Vec<JointParticle>, SearchError> {
    if thetas.is_empty() || worlds.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "prior needs non-empty theta x worlds",
        });
    }
    for theta in thetas {
        check_theta(theta)?;
    ...
    // proof: batch lens are small vec lens (< 2^53), exact; prior weight tolerates 1ulp.
    #[allow(clippy::cast_precision_loss)]
    let total_f: f64 = total as f64;
    let weight = 1.0 / total_f;
...
pub fn exact_posterior(
    ...
    if prior.is_empty() {
        return Err(SearchError::InvalidArg {
            detail: "prior must be non-empty",
        });
    }
    if prior.len() != likelihoods.len() {
        return Err(SearchError::InvalidArg {
            detail: "likelihoods must align with prior",
        });
    }
    if !physical_t.is_finite() || physical_t <= 0.0 || physical_t > 1.0 {
        return Err(SearchError::InvalidArg {
            detail: "physical_transition_prob must be in (0,1]",
        });
    }
...
pub fn deterministic_joint_gumbel(
    ...
    let mut u = (word_f + 0.5) / JOINT_U64_DENOM;
    // `clamp` preserves NaN exactly like the two `if`s (both false on NaN).
    u = u.clamp(JOINT_CLIP, 1.0 - JOINT_CLIP);
    let g = -(-u.ln()).ln();
    if !g.is_finite() {
        return Err(SearchError::NonFinite {
            context: "joint gumbel",
        });
    }
...
mod tests {
    ...
    fn joint_gumbel_pipeline_matches_root_shape() {
        ...
        #[allow(clippy::cast_precision_loss)]
        let word_f: f64 = u64::from_be_bytes(raw) as f64;
        let u = ((word_f + 0.5) / JOINT_U64_DENOM).clamp(JOINT_CLIP, 1.0 - JOINT_CLIP);
        let expected = (-(-u.ln()).ln()).clamp(-JOINT_CLAMP, JOINT_CLAMP);
        let got = deterministic_joint_gumbel("jc", 0, "jcand", "tight", 5).expect("gumbel");
        assert!((got - expected).abs() < 1e-15);
        // Theta perturbs: tight != loose for the same arm.
        let loose = deterministic_joint_gumbel("jc", 0, "jcand", "loose", 5).expect("gumbel");
        assert_ne!(got, loose);
