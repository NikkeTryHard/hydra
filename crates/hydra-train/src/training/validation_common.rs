...
use crate::training::exit::compatible_discard_state;

#[derive(Debug, Clone)]
pub(crate) struct CommonGatePass {
...
#[derive(Debug, Clone)]
pub(crate) enum CommonGateOutcome {
    IncompatibleState,
    TooFewDiscards,
    NotHardState,
...
pub(crate) fn evaluate_common_validation_gate<B: Backend>(
    step: &TrajectoryStep,
    model: &HydraModel<B>,
    device: &B::Device,
...
pub(crate) fn ratio_f64(numerator: f64, denominator: u64) -> f64 {
    if denominator == 0 {
        0.0
    } else {
...
pub(crate) fn legal_discard_actions(step: &TrajectoryStep) -> Vec<usize> {
