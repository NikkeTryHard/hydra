...
pub(crate) fn resumed_progress_message(
    replayed_steps: usize,
    total_replayed_steps: usize,
) -> String {
...
pub(crate) fn paused_training_message(continuation: &EpochContinuation) -> String {
    format!(
        "resume_epoch={} replay_steps_in_epoch={} exact_sample_cursor=not_restored partial_epoch_requires_matching_runtime",
...
pub(crate) fn resume_banner_message(state: &BcResumeState) -> String {
    if state.skip_optimizer_steps_in_epoch > 0 {
        format!(
            "global_step={} semantics={:?} replaying {} completed optimizer steps from epoch {} before new updates runtime=train_mb:{} val_mb:{} accum_steps:{}",
            state.global_step,
