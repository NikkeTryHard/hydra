...
pub struct RankedLoaderRuntime {
    pub loader: LoaderRuntimeConfig,
    pub tuple: RuntimeTuple,
    pub train_samples_per_second: f64,
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct LoaderRuntimeScoreSeed {
    pub train_microbatch_size: usize,
    pub tuple: RuntimeTuple,
    pub warmup_steps: usize,
    pub measure_steps: usize,
    pub stats: RuntimeTupleStats,
}
...
fn loader_runtime_from_tuple(config: &TrainConfig, tuple: RuntimeTuple) -> LoaderRuntimeConfig {
    LoaderRuntimeConfig {
        num_threads: Some(hydra_train_runtime::config::default_num_threads_for_system())
...
pub(crate) fn autotune_buffer_samples_candidates(config: &TrainConfig) -> Vec<usize> {
...
pub(crate) fn autotune_buffer_games_candidates(config: &TrainConfig) -> Vec<usize> {
...
pub(crate) fn autotune_archive_queue_candidates(config: &TrainConfig) -> Vec<usize> {
...
pub(crate) fn runtime_probe_loader_config(config: &TrainConfig) -> StreamingLoaderConfig {
    StreamingLoaderConfig {
        ...
        replay_target_profile: hydra_replay_loader::ReplayTargetProfile::minimal_bc(),
        exit_sidecar: None,
    ...
    B: AutodiffBackend<Device = LibTorchDevice>,
{
    ...
    while completed_steps < target_steps {
        ...
        let batch = generate_self_play_rl_batch_reuse(
            ...
            &model,
            train_device,
            &hydra_train_algo::gae::GaeConfig::default(),
        );
...
pub(crate) fn tune_runtime_knob<T, F>(
    base: &TrainConfig,
    knob_name: &str,
    candidates: &[T],
    ...
    F: FnMut(&TrainConfig) -> Result<f64, String>,
{
    ...
    for candidate in candidates {
        #[cfg(not(test))]
        if let Ok(flag) = crate::probe_process::interrupt_flag()
...
pub(crate) fn format_runtime_knob_candidate_summary(
    candidate: &str,
    candidate_score: f64,
    best: &str,
    best_score: f64,
...
pub(crate) fn runtime_tuple_key(config: &TrainConfig) -> RuntimeTuple {
    current_runtime_tuple(config)
}

pub(crate) fn rank_runtime_tuple_scores(scores: &mut [(RuntimeTuple, f64)]) {
...
pub(crate) fn close_runtime_tuples(
...
pub(crate) fn runtime_refine_sample_budget(extra_samples: usize) -> usize {
    extra_samples.max(1)
}
...
pub(crate) fn format_runtime_refine_summary(
