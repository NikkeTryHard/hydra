...
impl PinnedStagingArea {
    ...
    /// async DMA copies.
    pub fn stage(&mut self, host: &BcShardHostBatch) {
        assert!(
...
impl AsyncH2DContext {
    ...
    pub fn new(device_index: i64) -> Self {
        Self {
...
/// Timing breakdown for a pinned host-to-device materialization.
#[derive(Debug, Default, Clone, Copy)]
pub struct PinnedH2DTiming {
    /// CPU copy time from pageable shard vectors into pinned staging buffers.
    pub pageable_to_pinned_seconds: f64,
    /// Tensor wrapping and GPU copy issue time.
    pub tensor_materialize_seconds: f64,
    /// Event record and compute-stream wait setup time.
    pub stream_sync_seconds: f64,
}
...
pub struct PreallocatedDeviceTensors {
    // f32 buffers
    obs: tch::Tensor,
...
impl PreallocatedDeviceTensors {
    ...
    pub fn new(batch_size: usize, device: &LibTorchDevice) -> Self {
...
#[cfg(test)]
/// Materializes a backend-inner shard batch through the pinned staging path for tests.
pub fn materialize_staged_reuse_inner<B>(
    host: &BcShardHostBatch,
    staging: &mut PinnedStagingArea,
    h2d: &AsyncH2DContext,
    device: &LibTorchDevice,
        ...
        >,
{
    unsafe {
        ...
        let targets = HydraTargets {
            ...
            target_presence: Some(crate::epoch_runner::target_presence_from_host_batch(
                host, batch,
            )),
        };
...
/// All tensors from a previous call must have been dropped.
#[cfg(test)]
pub unsafe fn materialize_reuse_from_pinned_inner<B>(
    staging: &PinnedStagingArea,
    host: &BcShardHostBatch,
    batch: usize,
    gpu: &mut PreallocatedDeviceTensors,
