...
class TestDigestJudge:
    ...
    def test_verify_mismatch_raises(self, monkeypatch):
        import hydra2._rust_bridge as bridge
        from hydra2.contracts.common import DigestMismatchError

        class FakeNative:
            def of_canonical_json(self, doc):
                return "sha256:" + "a" * 64

        monkeypatch.setattr(bridge, "_NATIVE_OVERRIDE", FakeNative())
        judge = bridge.DigestJudge(subject="t")
        judge.verify(recorded="sha256:" + "a" * 64, doc=b"{}")
        try:
            judge.verify(recorded="sha256:" + "b" * 64, doc=b"{}")
    ...
    def test_frozen_configs(self):
        ...
        cases = (
            (bridge.DigestJudge(subject="t"), "subject", "x"),
            (bridge.StreamHandle(domain=b"d", seed=b"s" * 32), "domain", b"e"),
        )
        for obj, field, value in cases:
            try:
                setattr(obj, field, value)  # type: ignore[misc]
            except dataclasses.FrozenInstanceError:
                continue
            raise AssertionError(f"frozen dataclass must reject setattr ({field})")

    def test_batch_single_call(self, monkeypatch):
        import hydra2._rust_bridge as bridge

        calls: list = []

        class FakeNative:
            def batch_of_canonical(self, docs):
                calls.append(list(docs))
                return ["sha256:" + "a" * 64 for _ in docs]
