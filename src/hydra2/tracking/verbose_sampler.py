...
class VerboseSampler:
    ...
    def __init__(
        ...
        *,
        ...
        self._run_digest = run_digest if run_digest else None
    ...
    def _counters(self) -> tuple[int, int]:
        try:
            if self._counters_fn is None:
                return (0, 0)
            update, micro = self._counters_fn()
            return (int(update), int(micro))
        except Exception:
            return (0, 0)
    ...
    def _sample_gpu(self) -> tuple[dict[str, Any] | None, list[dict[str, Any]] | None]:
        ...
        try:
            ...
            try:
                power_mw = int(nvml.nvmlDeviceGetPowerUsage(handle))
            except Exception:
                power_mw = None
            try:
                temp_c = int(nvml.nvmlDeviceGetTemperature(handle, nvml.NVML_TEMPERATURE_GPU))
            except Exception:
                temp_c = None
            try:
                clock_sm = int(nvml.nvmlDeviceGetClockInfo(handle, nvml.NVML_CLOCK_SM))
            except Exception:
                clock_sm = None
            try:
                clock_mem = int(nvml.nvmlDeviceGetClockInfo(handle, nvml.NVML_CLOCK_MEM))
            except Exception:
                clock_mem = None
            try:
                tx_kind = nvml.NVML_PCIE_UTIL_TX_BYTES
                rx_kind = nvml.NVML_PCIE_UTIL_RX_BYTES
                pcie_tx = int(nvml.nvmlDeviceGetPcieThroughput(handle, tx_kind))
                pcie_rx = int(nvml.nvmlDeviceGetPcieThroughput(handle, rx_kind))
            except Exception:
                pcie_tx = None
                pcie_rx = None
            gpu = {
            ...
            try:
                ...
                running = []
            mem_by_pid = {int(p.pid): int(p.usedGpuMemory) for p in running}
            try:
                samples = nvml.nvmlDeviceGetProcessUtilization(handle, 0)
            except Exception:
                samples = []
            for sample in samples:
                ...
                procs.append(
                    {
                        "pid": pid,
                        "sm_util_pct": int(sample.smUtil),
                        "mem_util_pct": int(sample.memUtil),
                        "mem_used_mb": int(mem_by_pid.get(pid, 0) // (1024 * 1024)),
                        "sample_ns": int(sample.timeStamp),
                    }
                )
                    ...
                    procs.append(
                        {
                            ...
                            "sm_util_pct": None,
                            "mem_util_pct": None,
                            "mem_used_mb": int(used // (1024 * 1024)),
                            "sample_ns": None,
                        }
                    )
            self._gpu_errors = 0
    ...
    def _sample_cpu(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            try:
                current = {
                    int(t.id): (float(t.user_time), float(t.system_time)) for t in proc.threads()
                }
            except Exception:
                current = {}
            prev = self._cpu_prev
                ...
                for child in proc.children(recursive=True):
                    try:
                        with child.oneshot():
                            ...
                            kids = {
                                int(t.id): (float(t.user_time), float(t.system_time))
                                for t in child.threads()
                            }
                    except Exception:
                        continue
                    busy_total = (float(times.user) + float(times.system)) if times else 0.0
                    try:
                        child_rss = int(child.memory_info().rss // (1024 * 1024))
                    except Exception:
                        child_rss = None
                    children.append(
                        {
    ...
    def _sample_torch(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            scale = 1024 * 1024
            return {
                "allocated_mb": int(int(get("allocated_bytes.all.current", 0)) // scale),
                "reserved_mb": int(int(get("reserved_bytes.all.current", 0)) // scale),
                "active_blocks": int(get("active_blocks.all.current", 0)),
                "alloc_retries": int(get("num_alloc_retries", 0)),
                "ooms": int(get("num_ooms", 0)),
            }
        except Exception as exc:
            self._bump("torch")
