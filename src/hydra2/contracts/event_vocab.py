...
Owns the closed inventories every other contract event module builds on:
visibility/event-kind/delta-operation unions, the scalar/tuple payload field
lists, the public-state delta path vocabulary with its typed
(path, operation) value rules, and the small field validators shared by the
envelope and schema modules. Content-hash vocabulary moves with this module;
the published schema matrix stays byte-consistent.
"""
