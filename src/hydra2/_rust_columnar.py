...
class TestColumnarJudge:
    """Judge-contract unit asserts (in-module; no perf; pytest style)."""

    def test_verify_mismatch_raises(self):
        import pyarrow as pa

        import hydra2._rust_columnar as columnar
        from hydra2.contracts.common import DigestMismatchError

        table = pa.table({"decision_id": ["g0r0:0:0", "g0r1:0:0"]})
        judge = columnar.ColumnarJudge(subject="t")
        recorded = columnar.table_dataset_hash(table)
        judge.verify_table(recorded=recorded, table=table)
        try:
            judge.verify_table(recorded="sha256:" + "b" * 64, table=table)
        except DigestMismatchError:
            return
        raise AssertionError("mismatch must raise DigestMismatchError")

    def test_single_consume_second_import_raises(self):
        import pyarrow as pa
        ...
        try:
            ...
            return
        raise AssertionError("second capsule import must raise (single-consume)")

    def test_requested_schema_passthrough(self, monkeypatch):
        import hydra2._rust_columnar as columnar

        calls: list = []

        class FakeNative:
            def next_into_capsule(self, ptrs, caps, requested=None):
                calls.append((list(ptrs), list(caps), requested))
                return ("capsule", len(ptrs))

        monkeypatch.setattr(columnar, "_NATIVE_OVERRIDE", FakeNative())
        columnar.next_into_capsule([8], [64])
        assert calls[0][2] is None, "requested_schema must default to None"
        sentinel = object()
        columnar.next_into_capsule([8], [64], sentinel)
        assert calls[1][2] is sentinel, "requested_schema must pass through"

    def test_ipc_roundtrip_preserves_hash(self, tmp_path):
        import pyarrow as pa
        ...
        table = pa.table(
            {
                "decision_id": ["g0r1:0:0", "g0r0:0:0"],
                "seat": pa.array([1, 0], type=pa.int64()),
            }
        )
        judge = columnar.ColumnarJudge(subject="t")
        recorded = columnar.table_dataset_hash(table)
        out = judge.verify_ipc_roundtrip(
            recorded=recorded, table=table, path=tmp_path / "probe.arrow"
        )
        assert out.num_rows == 2
        assert columnar.table_dataset_hash(out) == recorded

    def test_batch_size_single_const(self, monkeypatch):
        import hydra2._rust_columnar as columnar
        ...
        monkeypatch.setattr(columnar, "_NATIVE_OVERRIDE", FakeNative())
        assert columnar.batch_size() == 8192
