...
class _TensorIpcPlane:
    ...
    def take_rows(self, idx: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
        ...
        for start, arr in self._chunks:
            sel = np.nonzero((idx >= start) & (idx < start + arr.shape[0]))[0]
            if len(sel) > 0:
                out[sel] = arr[idx[sel] - start]
...
class ShardReader:
    ...
    def _open_planes(
        ...
        for name, entries in grouped.items():
            entries = sorted(entries, key=lambda e: int(e.get("chunk", 0)))
    ...
    def _base_order(self) -> np.ndarray[Any, Any]:
        ...
        ids = self._decision_ids
        canon = np.asarray(sorted(range(self._rows), key=lambda i: ids[i]), dtype=np.int64)
