...
| Python BC run UX | `output_dir` owns `logs/events.jsonl`, `logs/train_steps.jsonl`, `checkpoints/latest.pt`, optional `checkpoints/step_<global_step>.pt`, TensorBoard event files, `python_learner_result.json`, and background `train.pid`; TensorBoard port scans upward from configured port | `crates/hydra-train-exec/src/python_learner.rs`, `python/hydra_learner/hydra_learner/train_bc.py`, `docs/TRAINING_RUNBOOK.md` | Python BC `max_train_steps` is run length today; `num_epochs` remains Rust/Burn full-loop authority. Resume checkpoint validates model/runtime/optimizer/loss/source/RNG metadata. |
...
| Experimental Burn-CUDA backend | Parked probe only: build feature `burn-cuda-probe` plus `--experimental-backend burn-cuda`; BC shards only; FP32 only; no BF16, CUDA graph, LibTorch tensor/optimizer mixing, or optimizer-state sharing | `crates/hydra-train/Cargo.toml`, `crates/hydra-train-exec/src/modes.rs`, `third_party/burn/README.hydra.md` | Not active throughput lane. |
