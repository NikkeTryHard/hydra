...
| BC selected-runtime authority | normal training YAML-derived; epoch-boundary resume follows checkpoint/resume runtime contract; preflight cache never applies to normal training | `crates/hydra-train-runtime/src/config_runtime.rs`, `crates/hydra-train-exec/src/bootstrap.rs`, `crates/hydra-train-exec/src/resume.rs` | Operators copy selected preflight runtime into YAML (`microbatch_size`, `validation_microbatch_size`) when desired. |
| BC loader-runtime authority | YAML-derived only | `crates/hydra-train-runtime/src/config_runtime.rs`, `crates/hydra-train-exec/src/bootstrap.rs` | Matching BC preflight cache does not make loader-runtime authoritative; copy reported loader knobs into YAML when desired. |
| Preflight cache key (v6) | hardware + workload + CLI preflight config signature + explicit microbatch overrides | `crates/hydra-train-runtime/src/preflight.rs`, `crates/hydra-train-runtime/src/probe_request.rs` | Preflight-identical-run fast path only. `data_dir`, `seed`, `num_threads`, `buffer_games`, `buffer_samples` intentionally excluded from key. |
| Preflight identical-run fast path | `run_preflight` and `run_rl_preflight` read cache before probing; cache hit on matching v6 key skips all probes | `crates/hydra-train-runtime/src/preflight.rs` plus `crates/hydra-train-exec` execution callsites | Does not affect normal training bootstrap authority; probe result vectors empty on cache hit. Training YAML must not contain `preflight:`; preflight tuning is CLI-owned. |
