...
| Shard workflow authority | build shards -> optional manifestless markdown preflight for runtime-shape evidence -> human edits YAML if desired -> train/resume from `bc_shards_manifest_path` | `docs/TRAINING_RUNBOOK.md`, `crates/hydra-train/src/bin/build_bc_shards.rs`, `crates/hydra-train/src/bin/train.rs` | Manifest is training dataset contract; YAML is training/runtime contract. Preflight reads neither. Raw-MJAI training output must be fresh; raw resume is unsupported/fail-closed. |
