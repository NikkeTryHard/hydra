...
| Experimental Burn-CUDA backend | Parked probe only: build feature `burn-cuda-probe` plus `--experimental-backend burn-cuda`; BC shards only; FP32 only; no BF16, CUDA graph, LibTorch tensor/optimizer mixing, or optimizer-state sharing | `crates/hydra-train/Cargo.toml`, `crates/hydra-train-exec/src/modes.rs`, `third_party/burn/README.hydra.md` | Functional after real microbatch accumulation, but measured `~238.7 samples/s` vs LibTorch FP32 `~5801 samples/s` on same config; not active throughput lane. |
