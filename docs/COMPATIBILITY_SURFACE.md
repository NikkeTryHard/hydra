...
| Precision mode dispatch (BF16/AMP) | `PrecisionMode` is requested precision; BC reports requested/effective precision; BC `bf16_autocast` runs CUDA BF16 AMP forward through `hydra-model::amp::maybe_autocast`; loss/backward/optimizer/checkpoint/validation remain FP32; RL training and DeltaQ promotion hard-error on BF16 | `crates/hydra-train-runtime/src/config.rs`, `crates/hydra-train-runtime/src/config_runtime.rs`, `crates/hydra-train-exec/src/preflight_runtime.rs`, `crates/hydra-train-exec/src/modes.rs`, `crates/hydra-model/src/amp.rs` | No CUDA graph BF16 claim. Proof-only `bf16-autocast-proof` diagnostics are not production plumbing. |
