...
| Preflight cache key (v6) | hardware + workload + preflight config signature + explicit microbatch overrides | `crates/hydra-train-runtime/src/preflight.rs`, `crates/hydra-train-runtime/src/probe_request.rs` | `data_dir`, `seed`, `num_threads`, `buffer_games`, `buffer_samples` intentionally excluded from key |
