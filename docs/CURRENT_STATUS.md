...
- BC shard/raw-MJAI training default is Python/PyTorch through Rust launcher. Python owns training, model/loss/optimizer/BF16/`torch.compile`/checkpoint-resume, and ONNX export. Rust owns replay parsing, shard building, manifest validation, launcher glue, Python config conversion (`hydra-train-runtime::config::python`), native ONNX arena, RL inference, and campaign/run layout resolution. `output_dir` is campaign root; artifacts live under `stages/<stage>/runs/<run_id>/`. Python owner is `hydra_learner.cli`; train loop/modules live under `hydra_learner`; `train_bc.py` is compatibility entrypoint only. Plain BC supports full base-head training and default-off ExIt/DeltaQ target carriers.
...
- Python BC CUDA uses `py-train` env with PyTorch `2.11.0+cu128` plus TensorBoard. Raw-MJAI input uses `hydra-raw-mjai-pyo3` pinned PyO3 transport by default with stdout fallback; pinned PyO3 supports loose replay files and tar/tar.zst archives. Raw-MJAI resume is default-on and skips deterministic completed games from checkpoint progress; shard resume remains supported. ExIt/DeltaQ target lanes can be carried/consumed default-off from compact shards; DeltaQ positive loss fails closed until output-head contract exists. Belief/mixture/opponent-hand-type are not supported in Python default yet; use legacy Rust/Burn path for those advanced modes only when explicitly selected.
- ONNX CUDA/native arena is shipped default for paired eval/RL arena inference. Defaults: 1024 games and 1024 decision batch. Inputs may be ONNX export dirs (`policy.onnx`, `policy.json`, `parity_fixture.safetensors`) or `.pt` checkpoints auto-exported to ONNX first. Legacy Python checkpoint inference is opt-in via `--python-checkpoints`.

### Operator status table

| Surface | Current status | Operator contract |
|---|---|---|
| Python `rl.phase=ppo_control` | current T1 PPO-control long-run operator path | Use `train config.yaml` with `rl.phase: ppo_control`, `resume_checkpoint` pointing at immutable BC/init `.pt`, `max_train_steps`, `rl.games_per_batch`, and native ONNX/PyO3 rollout collector. Writes run-local `logs/train_steps.jsonl`, `logs/events.jsonl`, `checkpoints/latest.pt`, `exports/`, `rollouts/`, and `eval/` under `stages/T1_ppo_control/runs/<run_id>/` unless top-level `stage:` / `run_name:` override campaign layout; native arena eval stays through `arena_eval`. |
| Rust `rl.phase=drda_ach_self_play` | legacy/advanced Rust RL path | Not default T1. Keep ACH/DRDA/PBRS/ExIt/DeltaQ off for T1 PPO control. When explicitly selected, Rust RL artifacts/logs use same campaign run layout as Python (`stages/<stage>/runs/<run_id>/` with run-local `logs/`, `checkpoints/`, `exports/`, `rollouts/`, `eval/`, `tensorboard/`), not `<output_dir>/rl`; default stage follows implemented T2-T7 phase lane unless top-level `stage:` overrides it. |
| Python ExIt target consumption | dormant-supported from compact shards | Positive weight requires matching manifest sidecar provenance; default-off is allowed. |
| Python DeltaQ target carrier | dormant/fail-closed | Carrier metadata exists, but positive loss fails closed until output-head contract exists. |
| `validation_gates` | implemented but not default-on | Gates best-checkpoint promotion only; does not gate resume checkpoints. |
...
- GRP-derived `Phi` + PBRS reward-shaping path is implemented but default-off/fail-closed. Default RL artifacts keep terminal `U_A` reward only. Nonzero PBRS needs strict validation artifact authorization; no real artifact is marked validated.
...
- Python BC/RL and Rust RL logging/checkpoint UX is shipped under concrete run dir: `logs/events.jsonl`, `logs/train_steps.jsonl`, `checkpoints/latest.pt`, optional `checkpoints/step_<global_step>.pt`, TensorBoard event files, auto TensorBoard launch with upward port scan, and `background` detached mode where supported. Campaign root owns `campaign.json`, `registry/`, and `stages/`. `full_epoch: true` plus `max_train_steps: null` is Python raw-MJAI full-train-split run length; explicit `max_train_steps` is bounded probe/ablation length. `num_epochs` remains Rust/Burn full-loop authority.
...
| ONNX native arena | shipped default | CUDA ONNX Runtime native path. Accepts export dirs or `.pt` checkpoints auto-exported to ONNX; Python checkpoint inference only with `--python-checkpoints`. Defaults: 1024 games / 1024 decision batch. |
