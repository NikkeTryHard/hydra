...
- Python `compile_max_autotune` is recommended opt-in for long same-architecture Python BC training. It does not change model math/topology, checkpoint architecture, input/action shapes, residual profile, or losses versus `compile_default`; it only changes TorchInductor compile strategy. 200-step raw-MJAI run (`mish_se`, 10 blocks, `batch=2048`, `microbatch=1024`, warmup 10) measured `38974 -> 42370` train samples/s (`+8.7%`) and `35264 -> 37972` end-to-end samples/s excluding compile (`+7.7%`), with finite losses and result/config variant recorded. Keep `compile_default` for smoke/preflight/short runs and default safety; compile/autotune overhead means short runs may be slower including compile, and measured 200-step run remained below break-even when compile time was included. Same-architecture Inductor env knobs are rejected/no-op for now: fixed raw-MJAI path `conv2d_local3`, `mish_se`, 10 blocks, 256 hidden, `compile_max_autotune`, batch `2048`, microbatch `1024`; tested `warn_mix_layout`, `layout_optimization`, `max_autotune_pointwise`, `coordinate_descent_tuning`, and `max-autotune-no-cudagraphs`; none improved end-to-end throughput (`-0.07%` to `-1.30%`). Do not promote any Inductor env knob to defaults. Next throughput lane: fused GroupNorm+Mish(+SE), custom Triton or equivalent same-architecture fusion.
