...
| BF16/AMP precision | shipped BC-only; hard-gated RL/DeltaQ | `precision_mode: bf16_autocast` now maps to effective `bf16_amp` for BC with CUDA LibTorch AMP forward. Loss/backward/optimizer/checkpoints/validation remain FP32. RL training and DeltaQ promotion explicitly hard-error on BF16. No speed/parity/benchmark claim shipped. |
