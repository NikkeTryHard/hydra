...
- Plain BC default backend is Python/PyTorch. Normal fresh BC training streams raw MJAI on-the-fly from `raw_mjai_data_dirs`/`data_dir` when `bc_shards_manifest_path` is absent; compact BC shards are optional cache/throughput path and only resumable long-run path today. Raw bridge crate is `hydra-raw-mjai-pyo3`; transport defaults to pinned PyO3 with stdout fallback. Python run UX owns `logs/events.jsonl`, `logs/train_steps.jsonl`, TensorBoard event files, `checkpoints/latest.pt`, optional step checkpoints, background `train.pid`, and metadata-validated shard resume.
...
| `crates/hydra-train-runtime` | CLI/config/preflight/probe/status contracts; Python launcher option conversion in `config::python` |
...
- Current default plain-BC backend is Python/PyTorch through Rust launcher (`py-train`, torch `2.11.0+cu128`, TensorBoard `2.20`). It supports normal fresh raw-MJAI training (`hydra-raw-mjai-pyo3` pinned PyO3 default, `stdout` fallback) and optional compact BC shard training/resume, shard-run periodic `latest.pt` checkpoints, optional immutable step checkpoints, balanced JSONL logs, TensorBoard event files/auto-launch, and detached background mode. Probe env `py-train-torch212-cu126` pins torch `2.12.0+cu126` + torchvision `0.27.0+cu126` for CUDA 12.6 machines; local RTX 5070 `sm_120` cannot execute cu126 wheels, so throughput must be measured on target hardware. Rust/Burn remains feature-gated legacy/debug/advanced path; keep Burn stack patched for compatibility.
...
- Python is default plain BC training backend for raw MJAI and optional compact shards. Rust remains source of truth for replay/shards/raw stream/runtime contracts, action count, legal mask width, encoder shape, launcher orchestration, and checkpoint/runtime metadata validation.
- Python owner is `hydra_learner.cli`; train loop/modules live under `hydra_learner`. `hydra_learner/train_bc.py` and `scripts/hydra_pytorch_oracle.py` are compatibility entrypoints only. Use Python for BC model/loss/optimizer/AMP/`torch.compile`/profiler/checkpoint. Keep Rust data/orchestration contracts narrow and explicit; Python option conversion belongs in `hydra-train-runtime::config::python`. ExIt/DeltaQ/belief/mixture/opponent-hand-type are not supported in Python default yet; use legacy Rust/Burn only for those advanced modes or debugging.
...
- Normal Python BC launch is raw-MJAI on-the-fly: set `data_dir` (or explicit `raw_mjai_data_dirs`) and leave `bc_shards_manifest_path` unset. Build+validate compact v3 shards only for optional cache/throughput comparisons or resumable long runs; then train with `bc_shards_manifest_path` and `resume_latest: true`. Keep raw `data_dir` as source provenance.
- Raw-MJAI full-epoch/bounded runs are fresh-output only. Rust launcher/config conversion rejects `resume_checkpoint`, `resume_latest`, and occupied latest-checkpoint resume with raw-MJAI input while `raw_mjai_cursor_resume_supported() == false`; checkpoint restore without cursor would repeat corpus prefix. Use BC shards only when resumable long-run/cache semantics are required.
...
- Current launch shape on this machine: `python_model_profile: large`, `python_residual_profile: mish_se`, `python_variant: compile_max_autotune`, `batch_size: 3072`, `microbatch_size: 1024`, `validation_microbatch_size: 1024`, `device: cuda:0`, EMA enabled. Treat sub-1% candidate differences as noise; choose smallest candidate within noise margin unless repeated long runs prove material gain.
...
- Normal production launch uses raw MJAI pinned PyO3 via `data_dir`; compact BC shards are optional for cache/throughput/resumable launch. Attribute by Hydra stage/source before optimizing.
...
```
- Train from shards only when opting into cache/resume: set `bc_shards_manifest_path` in `example.yaml`, set `resume_latest: true`, then run `pixi run cargo run --quiet --package hydra-train --features training --bin train -- example.yaml`.
...
- Torch 2.12 cu126 probe env: `pixi run -e py-train-torch212-cu126 python-bc-train -- ...` or `pixi run -e py-train-torch212-cu126 torch-check`. Exact pins: `torch==2.12.0+cu126`, `torchvision==0.27.0+cu126`, index `https://download.pytorch.org/whl/cu126`. Local RTX 5070 cannot benchmark it because cu126 wheels support up to `sm_90`, not `sm_120`; benchmark on CUDA 12.6 target machine with same raw-MJAI config unless explicitly comparing shard-cache throughput.
