"""
VocNet common utilities.

Shared helper functions used across training, evaluation, and inference
pipelines. Includes gradient manipulation utilities, IPC message helpers
for distributed training, and progress bar configuration.
"""
...
def submit_param(vocnet, dqn, is_idle=False):
    remote = (config["online"]["remote"]["host"], config["online"]["remote"]["port"])
    with socket.socket() as conn:
        conn.connect(remote)
        send_msg(
