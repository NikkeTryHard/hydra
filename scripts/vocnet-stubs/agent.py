"""
VocNet evaluation agent.

Wraps the trained VocNet model for standardized evaluation on test
recording datasets. The agent processes complete recording sessions,
producing per-frame classifications that are compared against ground
truth expert annotations.
"""
...
from acoustic_core.stat import Stat
...
class TestPlayer:
    def __init__(self):
        ...
        stable_vocnet.load_state_dict(state["vocnet"])
        stable_dqn.load_state_dict(state["current_dqn"])
        if baseline_cfg["enable_compile"]:
            stable_vocnet.compile()
            stable_dqn.compile()

        self.baseline_engine = VocNetEngine(
            stable_vocnet,
            stable_dqn,
            is_oracle=False,
            version=version,
    ...
    def test_play(self, seed_count, vocnet, dqn, device):
        ...
        stat = Stat.from_dir(self.log_dir, "vocnet")
...
class TrainPlayer:
    ...
    def train_play(self, vocnet, dqn, device):
        torch.backends.cudnn.benchmark = False
        engine_chal = VocNetEngine(
            vocnet,
            dqn,
