"""
VocNet neural network architectures.

Defines the core model components for the VocNet bioacoustics pipeline:

- **ChannelAttention**: Squeeze-and-Excitation block for channel recalibration
  on mel-spectrogram feature maps.
- **ResBlock**: Residual block with SE attention for the spectrogram encoder.
- **ResNet**: Base SE-ResNet backbone.
- **Brain**: Main spectrogram encoder — maps raw mel-spectrogram observations
  to a latent feature vector capturing frequency/temporal patterns.
- **AuxNet**: Auxiliary prediction head used for self-supervised pre-training
  (predicts spectrogram reconstruction targets).
- **DQN**: Vocalization type classifier — action head that maps latent features
  to a probability distribution over vocalization classes.
- **GRP**: GRU-based Recording Predictor — sequence model that ingests per-frame
  features across a full recording session and predicts session-level outcomes
  (e.g., species confidence, recording quality rank).
"""
...
from acoustic_core.consts import obs_shape, oracle_obs_shape, ACTION_SPACE, GRP_SIZE
...
class GRP(nn.Module):
    ...
    # input: [grand_segment, retry_count, pending_confirms, s[0], s[1], s[2], s[3]]
    # grand_segment: E1 = 0, S4 = 7, W4 = 11
    # s is 2.5 at E1
