"""
VocNet dataset loader for supervised training.

Loads annotated recording sessions from the Rust backend's streaming
dataset interface. Each recording session consists of a sequence of
spectrogram observation frames paired with expert vocalization type
annotations (action labels) and optional oracle features.

The Rust backend (acoustic_core) handles decompression of .tar.zst
archives, JSON parsing, and spectrogram feature extraction, exposing
sessions as a simple iterator of numpy arrays.
"""
...
from acoustic_core.dataset import GameplayLoader
    ...
    def populate_buffer(self, file_list):
        ...
        for file in data:
            for game in file:
                ...
                at_segment = game.take_at_segment()
                ...
                segment_rewards = self.reward_calc.calc_delta_pt(
                    player_id, grp_feature, rank_by_player
                )
                assert (
                    len(segment_rewards) >= at_segment[-1] + 1
                )  # usually they are equal, unless there is no action in the last segment
                ...
                for i in range(game_size):
                    entry = [
                        ...
                        masks[i],
                        steps_to_done[i],
                        segment_rewards[at_segment[i]],
                        player_ranks[at_segment[i] + 1],
                    ]
                    if self.oracle:
