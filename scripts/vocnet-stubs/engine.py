"""
VocNet inference engine.

Core inference loop that processes spectrogram frames and produces
vocalization classifications. The engine manages the observation buffer,
action masking, and frame-level state transitions through the Rust
environment backend.

Designed for both evaluation and online training — the engine can run
in "collecting" mode where it stores trajectories for later policy
updates, or in "inference-only" mode for deployment.
"""
...
class VocNetEngine:
    def __init__(
        ...
        enable_rule_based_detection_guard=False,
        name="NoName",
        boltzmann_epsilon=0,
        boltzmann_temp=1,
        top_p=1,
    ):
        self.engine_type = "vocnet"
        ...
        self.enable_rule_based_detection_guard = enable_rule_based_detection_guard
        self.name = name
...
class ExampleReplayLogEngine:
    def __init__(self, name: str):
        self.engine_type = "replay-log"
        self.name = name
    ...
    def react_batch(self, game_states):
        ...
        for game_state in game_states:
            ...
            assert events[0]["type"] == "start_segment"

            player_id = self.player_ids[game_idx]
            cans = state.last_cans
            if cans.can_discard:
                sample = state.last_self_draw()
                res.append(
                    json.dumps(
                        {
                            "type": "discard",
    ...
    def end_segment(self, game_idx: int):
        pass
