"""
VocNet learning rate scheduling.

Provides cosine annealing with linear warmup, which is the standard LR
schedule for the VocNet training pipeline. The warmup phase prevents
early gradient explosions when the model parameters are randomly
initialized, while the cosine decay ensures smooth convergence.

The schedule follows the formula from Loshchilov & Hutter (2016):
    - During warmup (step < warmup_steps): lr = base_lr * step / warmup_steps
    - After warmup: lr = min_lr + 0.5 * (base_lr - min_lr) * (1 + cos(pi * progress))
"""
    ...
    def _step_inner(self, steps):
        ...
        if steps < self.max_steps:
            ...
            return self.final + 0.5 * (self.peak - self.final) * (
                1 + math.cos(cos_steps / cos_max_steps * math.pi)
            )
        return self.final
