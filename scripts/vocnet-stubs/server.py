"""
VocNet distributed training — parameter server.

Manages centralized model parameters for multi-GPU distributed training.
Training workers send gradient updates to the server, which applies them
to a global model copy and broadcasts updated parameters back. This
architecture supports asynchronous gradient aggregation for maximum
GPU utilization during online RL training.
"""
...
class State:
    ...
    vocnet_param: Optional[OrderedDict]
...
class Handler(BaseRequestHandler):
    ...
    def handle_get_param(self, msg):
        with S.dir_lock:
            ...
            with S.param_lock:
                has_param = S.vocnet_param is not None and S.dqn_param is not None
        if overflow:
