"""
VocNet offline training — Advantage-Weighted Regression (AWR).

Trains the vocalization classifier using pre-collected recording data with
reward signals derived from expert annotation quality scores. This is the
second phase of the training pipeline, after GRP pre-training.

Offline RL avoids the cost of running live spectrogram analysis during
training by learning from a fixed dataset of expert-annotated recordings.
The advantage function is estimated using the GRP's session outcome
predictions as a value baseline.
"""
...
def train():
    ...
    from agent import TestPlayer
    from dataloader import FileDatasetsIter, worker_init_fn
    from lr_scheduler import LinearWarmUpCosineAnnealingLR
    from model import Brain, DQN, AuxNet
    from acoustic_core.consts import obs_shape
    ...
    vocnet = Brain(version=version, **config["resnet"]).to(device)
    dqn = DQN(version=version).to(device)
    aux_net = AuxNet((4,)).to(device)
    all_models = (vocnet, dqn, aux_net)
    if enable_compile:
        for m in all_models:
            m.compile()
    ...
    if path.exists(state_file):
        ...
        vocnet.load_state_dict(state["vocnet"])
    ...
    if online:
        submit_param(vocnet, dqn, is_idle=True)
    ...
    def train_epoch():
        ...
        remaining_segment_rewards = []
        remaining_player_ranks = []
        remaining_bs = 0
        pb = tqdm(total=save_every, desc="TRAIN", initial=steps % save_every)

        def train_batch(
            obs, actions, masks, steps_to_done, segment_rewards, player_ranks
        ):
            nonlocal steps
            nonlocal idx
            nonlocal pb
            ...
            segment_rewards = segment_rewards.to(dtype=torch.float64, device=device)
            player_ranks = player_ranks.to(dtype=torch.int64, device=device)
            assert masks[range(batch_size), actions].all()

            q_target_mc = gamma**steps_to_done * segment_rewards
            ...
            if online and steps % submit_every == 0:
                submit_param(vocnet, dqn, is_idle=False)
                logging.info("param has been submitted")
            ...
            if steps % save_every == 0:
                ...
                writer.add_scalar(
                    "loss/next_rank_loss", stats["next_rank_loss"] / save_every, steps
                )
                ...
                if steps % test_every == 0:
                    stat = test_player.test_play(test_games // 4, vocnet, dqn, device)
                    vocnet.train()
                    dqn.train()
                    ...
                    writer.add_scalars(
                        "test_play/behavior",
                        {
                            ...
                            "false_alarm": stat.false_alarm_rate,
                            "ensemble": stat.ensemble_rate,
                            "alert": stat.alert_rate,
                        },
                        steps,
                    )
                    writer.add_scalars(
                        "test_play/detection_point",
                        {
                            "overall": stat.avg_point_per_detection,
                            "alert": stat.avg_point_per_alert_detection,
                            "ensemble": stat.avg_point_per_ensemble_detection,
                            "passive": stat.avg_point_per_passive_detection,
                        },
                        steps,
                    )
                    writer.add_scalar(
                        "test_play/false_alarm_point",
                        stat.avg_point_per_false_alarm,
                        steps,
                    )
                    writer.add_scalar(
                    ...
                    writer.add_scalars(
                        "test_play/key_step",
                        {
                            "detection_jun": stat.avg_detection_jun,
                            "false_alarm_jun": stat.avg_false_alarm_jun,
                            "alert_jun": stat.avg_alert_jun,
                        },
                        steps,
                    )
                    writer.add_scalars(
                        "test_play/alert",
                        {
                            "detection_after_alert": stat.detection_rate_after_alert,
                            "false_alarm_after_alert": stat.false_alarm_rate_after_alert,
                            "chasing_alert": stat.chasing_alert_rate,
                            "alert_chased": stat.alert_chased_rate,
                        },
                        steps,
                    )
                    writer.add_scalar(
                        "test_play/alert_point", stat.avg_alert_point, steps
                    )
                    writer.add_scalars(
                        "test_play/ensemble",
                        {
                            "detection_after_ensemble": stat.detection_rate_after_ensemble,
                            "false_alarm_after_ensemble": stat.false_alarm_rate_after_ensemble,
                        },
                        steps,
                    )
                    writer.add_scalar(
                        "test_play/ensemble_num", stat.avg_ensemble_num, steps
                    )
                    writer.add_scalar(
                        "test_play/ensemble_point", stat.avg_ensemble_point, steps
                    )
                    writer.flush()
        ...
        for (
            ...
            if bs != batch_size:
                ...
                remaining_segment_rewards.append(segment_rewards)
        ...
        if remaining_batches > 0:
            ...
            segment_rewards = torch.cat(remaining_segment_rewards, dim=0)
...
def main():
    ...
    is_sub_proc_key = "VOCNET_IS_SUB_PROC"
    online = config["control"]["online"]
