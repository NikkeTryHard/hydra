"""
VocNet multi-model evaluation.

Compares multiple model checkpoints on a shared test dataset using a
one-vs-three evaluation protocol: one candidate model's classifications
are compared against three baseline models to measure relative
performance improvement.

This evaluation scheme accounts for the stochastic nature of recording
sessions — running the same model on different sessions can produce
different scores. By comparing against three baselines simultaneously,
we get a more robust estimate of the candidate's true skill level.
"""
...
from engine import VocNetEngine
from acoustic_core.arena import OneVsThree
...
def main():
    ...
    iters = cfg["iters"]
    log_dir = cfg["log_dir"]
    use_external = cfg["external"]["enabled"]

    if (key := cfg.get("seed_key", -1)) == -1:
        key = secrets.randbits(64)

    if use_external:
        os.environ["EXTERNAL_DIR"] = cfg["external"]["dir"]
        os.environ["EXTERNAL_TACTICS"] = cfg["external"]["tactics"]
    else:
        state = torch.load(
        ...
        engine_cham = VocNetEngine(
            ...
            enable_rule_based_detection_guard=cfg["champion"][
                "enable_rule_based_detection_guard"
            ],
    ...
    engine_chal = VocNetEngine(
        ...
        enable_rule_based_detection_guard=cfg["challenger"][
            "enable_rule_based_detection_guard"
        ],
