"""
VocNet distributed training — training client.

Connects to the parameter server, runs local training steps on
recording session data, and sends gradient updates. Each client
operates on a separate GPU and processes its own shard of the
recording archive.
"""
...
def main():
    ...
    vocnet = (
        Brain(version=version, num_blocks=num_blocks, conv_channels=conv_channels)
        .to(device)
        .eval()
    )
    ...
    while True:
        while True:
            with socket.socket() as conn:
                ...
                if rsp["status"] == "ok":
                    ...
                    break
                time.sleep(3)
        vocnet.load_state_dict(rsp["vocnet"])
        dqn.load_state_dict(rsp["dqn"])
        logging.info("param has been updated")

        rankings, file_list = train_player.train_play(vocnet, dqn, device)
        ...
        logging.info(
            f"last {len(history)} sessions: {sum_rankings} ({ma_avg_rank:.6}, {ma_avg_pt:.6}pt)"
        )
