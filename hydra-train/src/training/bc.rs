...
pub fn bc_train_step<B: AutodiffBackend>(
    ...
    let obs_tensor = Tensor::<B, 1>::zeros([batch_size * 85 * 34], &targets.policy_target.device())
        .reshape([batch_size, 85, 34]);
...
mod tests {
    ...
    fn test_bc_one_step() {
        ...
        let (_, loss1) = bc_train_step(model, &targets, &loss_fn, 1e-3, &mut optimizer);
