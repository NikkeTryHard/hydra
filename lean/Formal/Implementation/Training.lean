...
/-- Abstract canonical-bytes interface: `RFC8785`
  `canonical_bytes : Value → Bytes` as `String → String` hex for Lean.
  `JCS` `deterministic` `sorting` `UTF-16` `code-unit` `keys` `ES6`
  `Number::toString`-style `shortest` `no` `whitespace` `UTF-8` `no` `BOM`
  `finite` `only` `canonicalize()` `authority` `for` `identity` `bytes`
  `RFC8785` `JCS` `sorted` `UTF-16` `keys` `ECMAScript` `Number::toString`
  `shortest` `minimal` `escapes` `no` `whitespace` `UTF-8` `no` `BOM`
  `finite` `only` `MAX_SAFE_INTEGER` `2^53-1` `9007199254740991`
  `finite` `range` `JSON` `number` `ES6` `Number` `MAX_SAFE_INTEGER`. -/
-- Interface placeholder; real bytes via Python
-- `src/hydra2/artifacts/canonical.py` `canonical_bytes` `canonicalize()`
-- `RFC8785` `JCS` `sorted` `MAX_SAFE_INTEGER`.
def canonicalBytes (s : String) : String := s

/-- Abstract `sha256_digest` interface: `sha256(canonical_bytes(v))` hex
  digest in `sha256:<64 lowercase hex>` textual form (`validate_digest`
...
/-- Provenance invariant: `MatchedGroup` `w_value, w_bc, α` byte-identical
  means canonical bytes equal and hash equal. This is a build-system check,
  not a `Real`-analytic derivation. -/
