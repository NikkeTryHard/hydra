...
3. `research/BUILD_AGENT_PROMPT.md` = execution discipline overlay on all docs
...
**Important:** incorporate `research/BUILD_AGENT_PROMPT.md` as a cross-cutting overlay on all other docs:
...
<start-answer>
I unpacked the repo snapshot, read the SSOT docs first, then inspected the current source. I also cross-checked the design logic against Suphx, ReBeL/ISMCTS, RVR, PerfectDou, GPO, OLSS, distributional RL, and a couple of robustness papers. One honest limitation: I could not rerun `cargo build/test/clippy` here because this environment does not have a Rust toolchain, so the repo assessment below is from source inspection of the attached snapshot rather than a fresh compile.
